...
To decide whether to cache an entity, the router honors the [`Cache-Control` header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cache-Control) returned with the subgraph response. Because `Cache-Control` might not contain a `max-age` or `s-max-age` option, a default TTL must be configured, either defined per subgraph configuration or inherited from the global configuration.
...
A subgraph might return a response with the header `Cache-Control: private`, indicating that it contains user personalized data. Although this usually forbids intermediate servers from storing data, the router may actually be able to recognize different users and store their data in different parts of the cache.

To set up private information caching, you can configure the `private_id` option. `private_id` is a string pointing at a field in the request context, which contains data used to recognize users (for example, user id, or `sub` claim in JWT).

As an example, if you are using the router's JWT authentication plugin, you can configure the `private_id` option in the `accounts` subgraph to point to the `user_id` key in context, and use a rhai script to set that key from the JWT's `sub` claim:
...
Because the router doesn't know in advance if a particular query will return private data, it implements the following sequence:
...
- When the router sees the query a subsequent time:
