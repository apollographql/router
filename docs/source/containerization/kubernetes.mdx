...
subtitle: Self-hosted deployment of GraphOS Router in Kubernetes
description: Deploy the Apollo GraphOS Router in Kubernetes using Helm charts. Customize configurations, enable metrics, and choose values for migration.
---
...
Learn how to deploy a self-hosted GraphOS Router in Kubernetes using Helm charts.
...
flowchart LR;
  ...
  router_a(["<b>GraphOS Router</b>"]);
  coproc_a(["<b>Coprocessor</b>"]);
  router_b(["<b>GraphOS Router</b>"]);
...
* Read [Managing GraphOS Router resources in Kubernetes](/technotes/TN0016-router-resource-management/).
...
When [migrating from `@apollo/gateway` to the GraphOS Router](../migrating-from-gateway), consider the following tips to maximize the compatibility of your router deployment.
