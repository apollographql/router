...
Rhai comes with a string interner, which caches frequently used strings to reduce allocations. This interner is protected by a lock to make the Rhai engine safe to share across threads.

Threads that encounter new strings must acquire a write lock. Under high concurrency, the write lock can be under high contention, which serializes those operations across all concurrent requests.

String interning is enabled by default. You can disable it with the `intern_strings` configuration option:
...
- Eliminating lock contention on the string interner can meaningfully improve throughput for deployments processing many concurrent requests through Rhai scripts.
