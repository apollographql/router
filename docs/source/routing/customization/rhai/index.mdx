...
fn http_service(service) {
  let request_callback = |request| {
      print("HTTP service: Raw request received");
  };
  let response_callback = |response| {
      print("HTTP service: Raw response ready to send");
  };
...
Before you build your Rhai script, it helps to understand how the router handles each incoming GraphQL request. The router first receives a **raw HTTP request** at the **HTTP layer**. Your Rhai scripts can define `fn http_service(service)` to run at this stage (see [Service descriptions](#service-descriptions) below). You can also use [native plugins](/graphos/routing/customization/native-plugins) (via the `http_service` hook) or the [coprocessor](/graphos/routing/customization/coprocessor) (HttpRequest/HttpResponse stages). The request is then converted to a GraphQL/router request and flows through the services below. During each request's execution, different **services** in the router communicate with each other as shown:
...
sequenceDiagram
    ...
    Client->>HttpLayer: Sends raw HTTP request
    HttpLayer->>RouterService: Converts to GraphQL/JSON request
    ...
    RouterService-->>HttpLayer: Returns HTTP response
    HttpLayer-->>Client: Returns HTTP<br/>response
```
...
Your Rhai scripts can hook into any combination of the services in the diagram above (if you are using the Apollo Router Core v1.30.0 and later), including the HTTP layer via `http_service`. The scripts can modify the request, response, and/or related metadata as they're passed along.
...
Runs first; the request is raw HTTP (method, URI, headers, body as string). Define `http_service` if your customization needs to see or mutate the raw HTTP request or response before it is converted to a GraphQL/router request. You can use `service.map_request` and `service.map_response` like the other hooks. The request and response objects expose `method`, `uri`, `headers`, and `body` (as a string; e.g. JSON). See the [Rhai API reference](/graphos/reference/router/rhai/#http-layer-request-and-response) for details.
...
Runs at the very beginning and very end of the GraphQL request lifecycle (after the HTTP layer).
...
For **router_service**, **supergraph_service**, **execution_service**, and **subgraph_service**, request and response data use structures that contain:
...
The **HTTP layer** (`http_service`) uses a simpler shape: request and response expose `method`, `uri`, `headers`, and `body` (as a string). There is no context object at this stage. See the [Rhai API reference](/graphos/reference/router/rhai/#http-layer-request-and-response).
...
* `map_request` callbacks are called in each service as execution proceeds "to the right." The **HTTP layer** runs first (before the request is converted to GraphQL); then the flow continues through the router and downstream:

    ```mermaid
    graph LR;
      client(Client);
      client --> http(http_service);
      http --> router(router_service);
      subgraph Router
...
* `map_response` callbacks are called in each service as execution proceeds back "to the left" from subgraphs resolving their individual sub-operations, and finally at the **HTTP layer** before the response is sent to the client:
    ...
    Finally, callbacks for `http_service` are passed the raw HTTP response (e.g. body as string) before it is sent to the client.
