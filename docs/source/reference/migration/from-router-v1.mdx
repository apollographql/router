...
<details>
<summary>Renée Drafting</summary>
...
Upgrading the router from v1.x to v2.x requires the following steps.

### Enforce introspection depth limit

To protect against abusive requests, the router v2.x enforces a depth limit on introspection queries by default.
...
**Upgrade step**: if you access context entries in a custom plugin, Rhai script, coprocessor, or telemetry selector, update your context keys to account for the new names:
...
In router v2.x, the path used for selecting data from a client request body for [header propagation](/docs/graphos/routing/header-propagation#insert) now must comply with the [JSONPath](https://www.ietf.org/archive/id/draft-goessner-dispatch-jsonpath-00.html) spec. This means a `$` is now required to select the root element.
...
Starting in v1.x, the router introduced telemetry that followed OpenTelemetry (OTLP) conventions. That evolution toward OTLP continues in router v2.x.
...
The architectural improvements in router v2.x affect most custom Rust plugins. 
...
For `Context` values carried by request and response types of the router request lifecycle, context keys added by the router have been renamed in v2.x to prevent collisions with user-provided context keys.

**Upgrade step**: if you are using any router-provided context keys in your plugins, see the [context keys section][TODO] of the upgrade guide.
...
Make sure that you are referencing the correct router release: **v{products.router.version("latest-2x").version}**
