---
title: Managed federation with the Apollo Router
sidebar_title: Managed federation
---

This article describes using the Apollo Router with [managed federation](https://www.apollographql.com/docs/federation/managed-federation/overview/) and Apollo Studio.

## Connecting to Apollo Uplink

[Uplink](https://www.apollographql.com/docs/federation/managed-federation/uplink/) is the Apollo-hosted endpoint that a managed router polls to fetch its latest supergraph schema. To authenticate the Apollo Router with Studio:

1. [Create a graph API key](https://www.apollographql.com/docs/studio/api-keys/#graph-api-keys) with the `Contributor` key role.
2. In the router's environment, set the API key as the value of the `APOLLO_KEY` environment variable.
3. Also set the `APOLLO_GRAPH_REF` environment variable to match whichever Studio graph and variant the router is using (e.g., `my-graph-id@production`).

After you set these values, the Apollo Router automatically starts polling Uplink on startup. Setting these values also enables [usage reporting](#usage-reporting-agent-spaceport) by default.
