...
To serve MCP requests, enable the [Apollo MCP Server](/apollo-mcp-server) using the `MCP_ENABLE` environment variable. You'll also need to export container port `5000` for Streamable HTTP connections to the MCP server, using the `-p 5000:5000` flag.
