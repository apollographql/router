...
Enable and configure the [OpenTelemetry](https://www.opentelemetry.io/) exporter for metrics in the Apollo Router for use with [Datadog](https://www.datadoghq.com/).
...
To configure the router, enable [OpenTelemetry metrics exporter](./opentelemetry#configuration) and set both `temporality: delta` and `endpoint: <datadog-agent-endpoint>`. For example: 
