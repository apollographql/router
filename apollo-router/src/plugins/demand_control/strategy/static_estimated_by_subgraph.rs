...
pub(crate) struct StaticEstimatedBySubgraph {
    ...
    fn on_execution_request(&self, request: &execution::Request) -> Result<(), DemandControlError> {
        self.cost_calculator
            ...
            .and_then(|cost_by_subgraph| {
                ...
                request
                    .context
                    .insert_cost_strategy("static_estimated_by_subgraph".to_string())?;
...
mod tests {
    ...
    use super::StaticEstimatedBySubgraph;
    ...
    async fn load_config_and_extract_strategy(
        config: &'static str,
    ) -> Result<StaticEstimatedBySubgraph, BoxError> {
        let schema_str =
        ...
        let StrategyConfig::StaticEstimatedBySubgraph {
            list_size,
            max,
            ref subgraphs,
        } = plugin.config.strategy
        else {
            panic!("must provide static_estimated_by_subgraph config");
        };
        let strategy = plugin
            .strategy_factory
            .create_static_estimated_by_subgraph_strategy(list_size, max, subgraphs);
        Ok(strategy)
    }
