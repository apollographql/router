...
The router always indicates it accepts compressed responses via the `accept-encoding` header, but it only compresses its own request bodies when you explicitly configure the `compression` option.
