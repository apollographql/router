...
By default, the router enables _only_ GraphOS Studio to initiate browser connections to it. If your supergraph serves data to other browser-based applications, you need to do one of the following in the `cors` section of your router's [YAML config file](/router/configuration/overview/#yaml-config-file):
...
If your router requires requests to [a device or service on a private network](https://wicg.github.io/private-network-access/), you need to enable this behaviour in your CORS configuration. Without this change, the browser will not allow any requests to be sent to a device on the private network.

To enable private network access with CORS, the [`Access-Control-Allow-Private-Network`](http://wicg.github.io/private-network-access/#http-headerdef-access-control-allow-private-network) HTTP header needs to be returned with `true`.

You can enable private network access by including `private_network_access` in the policies of your `cors` config. While optional, specifying the subfields, `access_id` and `access_name`, of `private_network_access` clarify what will be accessed on the private network when the browser prompts the user to allow the request.
...
When specified, the `access_id` and `access_name` fields return the `Private-Network-Access-ID` and `Private-Network-Access-Name` HTTP headers, respectively, along with the specified strings.
