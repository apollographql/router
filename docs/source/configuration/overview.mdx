...
When migrating types and fields between subgraph schemas with the [`@override`](/federation/federated-types/federated-directives/#override) directive, you can configure the router to override fields or types in a progressive or gradual fashion by enabling the `progressive_override` option:
...
Learn more with the [Incremental migration with `@override`](/federation/entities-advanced/#incremental-migration-with-override) guide.
