...
GraphOS Router and Apollo Router Core support a basic HTTP-level health check. It is enabled by default and is served on port `8088` at the URL path `/health`. A `200` status code is returned if the HTTP server is successfully serving. The health check is a **separate route** and doesn't pass through the [RouterHttp pipeline](/graphos/routing/request-lifecycle#request-path), so [Rhai](/graphos/routing/customization/rhai/) scripts, native plugins, and license enforcement don't run on health requests.
