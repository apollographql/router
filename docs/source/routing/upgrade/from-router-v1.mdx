...
Measuring latency in the router is difficult because it processes in parallel. Realistically, the way to monitor router latency is by comparing the 'differences' between `http.server.request.duration` and `http.client.request.duration`. If server durations spike, and client durations remain steady, it's a good indicator that the router itself is slower than usual. If a subgraph is slow, you'll probably see a spike in both http.server.request.duration and http.client.request.duration.
