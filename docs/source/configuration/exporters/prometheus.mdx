...
To export metrics to Prometheus, in `router.yaml` you must enable the Prometheus endpoint and set its address and path. For example:
...
You can use [Prometheus with Grafana](https://prometheus.io/docs/visualization/grafana/) to collect metrics and visualize the router metrics.
