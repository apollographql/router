...
limits:
  ...
  connector:
    all:
      ...
      max_mapping_errors: 100 # at most 100 `->withConnectorError` errors per response
    sources:
      products.rest:
...
Limits how many errors a connector's response mapping may report in the response's `extensions.connectorErrors` using the [`->withConnectorError` method](/graphos/connectors/responses/error-handling#report-errors-without-failing-a-field).

A `->withConnectorError` call inside a `->map` records one error per element, so a mapping over a large API response can contribute one error per row. This limit caps that contribution.
...
This limit applies only to errors the mapping declares with `->withConnectorError`. Diagnostics recorded with `->withError`, and the mapping language's own problems such as a path that matched nothing, are never sent to clients and are unaffected.
