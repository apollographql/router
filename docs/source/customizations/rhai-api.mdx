...
This reference documents the symbols and behaviors that are specific to [Rhai customizations](/router/customizations/rhai/) for the GraphOS Router and Apollo Router Core.

## Entry point hooks

Your Rhai script's [main file](/router/customizations/rhai/#the-main-file) hooks into the individual services of the router's [request-handling lifecycle](/router/customizations/rhai/#router-request-lifecycle). To do so, it defines whichever combination of the following entry point hooks it requires:
...
Within each hook, you define custom logic to interact with the current active request and/or response as needed. This most commonly involves using methods of the provided `service` object to [register service callbacks](/router/customizations/rhai/#service-callbacks), like so:
