...
It also includes other improvements that make it more likely that two operations that only vary slightly appear as the same normalized operation in GraphOS Studio.
...
Once you enable this configuration, operations with enhanced signatures appear with different operation IDs than they did previously in Studio.
