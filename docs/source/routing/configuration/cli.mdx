...
If set, the router watches for changes to its configuration file and any supergraph file passed with `--supergraph` and reloads them automatically without downtime. This setting only affects local files provided to the router. The supergraph and configuration provided from GraphOS via Launches (and delivered via Uplink) are _always_ loaded automatically, regardless of this setting.<br/><br/>

For details on hot reload support for different schema sources and performance considerations, see [Hot Reload](/graphos/routing/configuration/hot-reload).
...
For information on finding OCI artifact references, see [OCI Artifact References](/graphos/routing/configuration/oci-artifacts).<br/><br/>
