...
pub(crate) struct CacheInvalidationSpecDefinition {
    url: Url,
    minimum_federation_version: Version,
}

impl CacheInvalidationSpecDefinition {
    pub(crate) fn new(version: Version, minimum_federation_version: Version) -> Self {
        Self {
            url: Url {
                identity: Identity::cache_invalidation_identity(),
                version,
            },
    ...
    fn directive_specification(&self) -> Box<dyn TypeAndDirectiveSpecification> {
        // TODO: Port the JS federation PR, once Rust composition is implemented.
        Box::new(DirectiveSpecification::new(
            ...
            true, // composes
            Some(&|v| CACHE_INVALIDATION_VERSIONS.get_dyn_minimum_required_version(v)),
            None,
        ))
    }
}

impl SpecDefinition for CacheInvalidationSpecDefinition {
    fn url(&self) -> &Url {
        &self.url
    }
...
pub(crate) static CACHE_INVALIDATION_VERSIONS: LazyLock<
    SpecDefinitions<CacheInvalidationSpecDefinition>,
> = LazyLock::new(|| {
    let mut definitions = SpecDefinitions::new(Identity::cache_invalidation_identity());
    definitions.add(CacheInvalidationSpecDefinition::new(
