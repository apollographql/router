...
Then the body of the co-processor response should be valid graphql data that the client is expecting. If the Break code is a "failure" code, then the body of the co-processor response should contain a GraphQL errors array as shown in the following code snippet.
...
then the router would stop processing the request and return a response to the client with an HTTP status code of 403 and graphql response defined in the body.
