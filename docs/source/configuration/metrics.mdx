...
The Apollo Router exports metrics for collection by [Prometheus](#using-prometheus) and [OpenTelemetry Collector](#using-opentelemetry-collector), with support for [Datadog via OpenTelemetry]().

In `router.yaml`, you configure router metrics within the following settings:
...
### Customizing `apollo_router_http_requests` instrument
...
OpenTelemetry includes many [standard attributes](https://opentelemetry.io/docs/specs/semconv/attributes-registry/) that you can use these via custom [instruments](../instruments.mdx). 
...
The Apollo Router exports metrics for:

* [Prometheus](#prometheus)
* [OpenTelemetry Collector](#opentelemetry-collector)
* [Datadog via OTLP](#datadog-otlp)
...
To export metrics to OpenTelemetry Collector, in `router.yaml` you must enable the OpenTelemetry exporter and optionally customize its configuration. For example: 
