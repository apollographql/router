...
description: Create and customize instruments to collect data and report measurements from the Apollo Router's pipeline services.
---
...
For more information, see the [Open Telemetry semantic conventions for HTTP metrics](https://opentelemetry.io/docs/specs/semconv/http/http-metrics/).
...
When defining a custom instrument, make sure to reference the [OpenTelemetry semantic conventions](https://opentelemetry.io/docs/specs/semconv/general/metrics/), as it will help guide you to:



In the example configuration below, three custom instruments are defined (`acme.request.duration`, `acme.graphql.requests`, `acme.graphql.subgraph.errors`), one for each service of the router pipeline (`router`, `supergraph`, `subgraph`):
...
Make sure to follow naming conventions for instruments. For reference, read the [Open Telemetry semantic conventions](https://opentelemetry.io/docs/specs/semconv/general/metrics/). It will help guide you to:
...
The `router`, `supergraph` and `subgraph` sections are used to define custom instruments for the `router`, `supergraph` and `subgraph` services of the router, respectively.
...
* `custom` - a custom value extracted from the pipeline service. See [selectors](selectors.mdx) for more information.
...
Values of custom metrics can be extracted from the pipeline using [custom attributes](custom-attributes.mdx). For example, to sum the contents of a request header, create a counter with value set as the request header:
