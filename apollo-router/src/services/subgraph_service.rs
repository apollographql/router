...
pub(crate) struct SubgraphCreator {
    pub(crate) http_services: Arc<HashMap<String, Arc<dyn MakeSubgraphHTTPService>>>,
...
impl SubgraphCreator {
    pub(crate) fn new(
        ...
        http_services: Vec<(String, Arc<dyn MakeSubgraphHTTPService>)>,
        plugins: Arc<Plugins>,
    ) -> Self {
        SubgraphCreator {
            services: Arc::new(services.into_iter().collect()),
            http_services: Arc::new(http_services.into_iter().collect()),
            plugins,
        }
    }
}
...
impl SubgraphServiceFactory for SubgraphCreator {
    ...
    fn create(&self, name: &str) -> Option<Self::SubgraphService> {
        self.http_services
            ...
            .and_then(|subgraph_http_service| {
