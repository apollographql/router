...
| `apollo.router.operations.response_cache.cdn_tag_header.error` | Errors while emitting the header, labeled by `reason`: `invalid_header_name` or `invalid_header_value`. | `{error}` |
