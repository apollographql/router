...
impl JSONSelection {
    ...
    // support).
    pub fn apply_to(&self, data: &RawJSON) -> (Option<RawJSON>, Vec<ApplyToError>) {
    ...
    pub fn apply_with_vars(
        &self,
        data: &RawJSON,
        vars: &IndexMap<String, RawJSON>,
    ) -> (Option<RawJSON>, Vec<ApplyToError>) {
        let safe_data: JSON = data.into();
        let vars = vars
            .iter()
            .map(|(var_name, var_data)| (var_name.clone(), var_data.into()))
            .collect::<IndexMap<String, JSON>>();
        let vars: IndexMap<&String, &JSON> = vars.iter().collect();
                ...
                if let Some(child) = data.get_key(name) {
                    ...
                    if let Some(selection) = selection {
                        ...
                        if let Some(value) = value {
                            output = Some(JSON::Object(JSONMap::from_iter([(
                                output_name.to_string().into(),
                                value,
                            )])));
                        }
                    } else {
                ...
                if let Some(alias) = alias {
                    ...
                    match value_opt {
                        Some(JSON::Object(map)) => {
                            output = Some(JSON::Object(map.clone())); // HERE
                        }
                        Some(JSON::Null) => {
        ...
        match self.as_ref() {
            ...
            PathList::Key(key, tail) => {
                ...
                if let JSON::Array(array) = data {
                    ...
                    let self_with_empty_tail = WithRange::new(
                        PathList::Key(key.clone() /* HERE */, empty_tail),
                        key.range(),
                    );
                    ...
                    if let Some(child) = data.get_key(key.as_str()) {
...
impl ApplyToInternal for WithRange<LitExpr> {
    fn apply_to_path(
        ...
        match self.as_ref() {
            LitExpr::String(s) => (Some(JSON::String(s.clone().into())), vec![]), // HERE
            ...
            LitExpr::Object(map) => {
                ...
                for (key, value) in map {
                    ...
                    if let Some(value_json) = value_opt {
                        output.insert(SafeString::Unsafe(key.as_str().to_string().into()), value_json);
                    }
                }
...
mod hack {
    ...
    fn hack() {
        let s = selection!("
            slashes
            commas
            safeSlashes: slashes->urlSafe
            safeCommas: commas->urlSafe
            list: $([$.slashes, $.commas])
            listSafe: $([$.slashes->urlSafe, $.commas->urlSafe])
            join: $([$.slashes, $.commas])->join(',')
            joinSafe: $([$.slashes, $.commas])->join($(',')->urlSafe)
            joinEachSafe: $([$.slashes->urlSafe, $.commas->urlSafe])->join($(',')->urlSafe)
            joinAllSafe: $([$.slashes, $.commas])->join(',')->urlSafe
        ");
        let d = json!({
            "slashes": "a/b",
            "commas": "a,b",
        });
        let (r, v) = s.apply_to(&d);
        assert_eq!(v, vec![]);
