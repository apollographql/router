...
To control how many traces are sent to GraphOS, use the common tracing sampler (which applies to every exporter) and the GraphOS exporter sampler (which applies only to traces sent to GraphOS):
...
Whatever these samplers let through is then subject to a back-stop that further reduces GraphOS trace volume — see [Reducing the volume of traces sent to GraphOS](#reducing-the-volume-of-traces-sent-to-graphos) below.
...
#### Reducing the volume of traces sent to GraphOS

To keep trace volume manageable for high-traffic graphs, the router applies a back-stop that further reduces the number of traces it sends to GraphOS. This runs *after* the `telemetry.apollo.sampler` and `telemetry.exporters.tracing.common.sampler` head samplers, so it only ever further reduces the traces those samplers already selected. It acts as a safeguard against sending too many traces to GraphOS even when a high sample rate is configured, and it affects only the GraphOS pipeline — traces sent to your own OTLP collector or to Datadog are unaffected.

Choose the strategy with `telemetry.apollo.tracing.throttle`:
...
- **`representative_traces`** (default): Sends at most one *representative* trace per minute for each distinct combination of operation, client, latency bucket, error status, and operation type. Traces that duplicate a combination already seen within the current minute are dropped. Error traces are sampled at a higher rate so that failures remain visible. This gives broad coverage of distinct behaviors while dramatically reducing volume for repetitive traffic.
- **`rate_limited`**: Sends every trace but caps the export rate at a fixed maximum of 100 traces per second, per router instance. This is cheaper in CPU and memory than `representative_traces` and is a good fallback if representative sampling is too resource-intensive for your workload. The rate is not configurable.

<Note>

This back-stop always applies to GraphOS traces. To reduce trace volume upstream of it, lower `telemetry.apollo.sampler` (GraphOS only) or `telemetry.exporters.tracing.common.sampler` (all exporters). To stop sending traces to GraphOS altogether, set `telemetry.apollo.sampler: always_off`.
