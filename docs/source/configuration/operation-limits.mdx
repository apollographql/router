...
If you have enabled operation limits to reject any operations that exceeed the configureed limits, clients will get an `HTTP 400` response with a GraphQL error.
...
If you have enabled `warn_only` mode, the response will return `HTTP 200` and execute as normal with just the added logs.
