...
In most supergraphs, this represents a large percentage of all fields in the schema. [See an example of a non-deferrable field.](#non-deferrable-fields)
...
As this query shows, if you defer an entity field (such as `User.name`), you can also defer _subfields_ of that field (such as `Name.firstName` and `Name.lastName`).
...
In this case, the router initially responds with the result of `Query.products`, and it later completes the response with the result of `Query.users`.
