...
Enable and configure the [Open Telemetry](https://www.opentelemetry.io/) exporter for metrics in the Apollo Router for use with [Datadog](https://www.datadoghq.com/).
...
To configure the router, enable [Open Telemetry metrics exporter](./opentelemetry.mdx#configuration) and set both `temporality: delta` and `endpoint: <datadog-agent-endpoint>`. For example: 
