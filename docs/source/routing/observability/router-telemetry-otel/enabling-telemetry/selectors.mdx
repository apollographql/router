...
The `router_overhead` and `active_subgraph_requests` selectors allow you to extract router processing overhead metrics. These selectors are available on the router service and can be used with custom instruments to create detailed overhead metrics with custom attributes.
    ...
    instruments:
      router:
        apollo.router.overhead:
          attributes:
            custom:
              # Add the count of active subgraph requests as an attribute
              active_requests_count:
                active_subgraph_requests: count
              # Add a boolean indicating if any subgraph requests were active
              has_active_requests:
                active_subgraph_requests: bool
```
