...
flowchart TB;
  ...
  subgraph " "
  httpLayer("<code>HTTP layer</code><br/>HttpRequest / HttpResponse");
  ...
  subgraphs[[Subgraphs]];
  client --"<b>1.</b> Sends request"--> httpLayer;
  httpLayer <-."<b>2.</b> Can send request<br/>or response details<br/>to coprocessor".-> coprocessing;
  httpLayer --"<b>3</b>"--> routerService;
  routerService <-."<b>4.</b> Can send request<br/>details to coprocessor<br/>and receive modifications".-> coprocessing;
  routerService --"<b>5</b>"--> supergraphService;
  supergraphService <-."<b>6.</b> Can send request<br/>details to coprocessor<br/>and receive modifications".-> coprocessing;
  supergraphService --"<b>7</b>"--> executionService;
  executionService <-."<b>8.</b> Can send request<br/>details to coprocessor<br/>and receive modifications".-> coprocessing;
  executionService --"<b>9</b>"--> subgraphService;
  subgraphService <-."<b>10.</b> Can send request<br/>details to coprocessor<br/>and receive modifications".-> coprocessing;
  subgraphService -- "<b>11</b>"--> subgraphs;
...
The **HTTP layer** runs first: the router receives the raw HTTP request and can call your coprocessor at **HttpRequest** (before the request is converted to a GraphQL/router request) and **HttpResponse** (after the pipeline, when sending the raw HTTP response). The payload shape for HttpRequest and HttpResponse is the same as RouterRequest and RouterResponse (for example, `body` is a string). See [example requests by stage](/graphos/routing/customization/coprocessor/reference#example-requests-by-stage) for HttpRequest and HttpResponse.

As shown in the diagram above, the `RouterService`, `SupergraphService`, `ExecutionService`, and `SubgraphService` steps of the [request-handling lifecycle](/graphos/routing/customization/rhai/#router-request-lifecycle) can also send these POST requests (also called **coprocessor requests**).
...
### Configure the HTTP layer (http_service) stage

You can run the coprocessor at the **HTTP layer** (before RouterService) by configuring `http_service`:

- **`coprocessor.http_service.request`** – Same structure as `router.request`: `headers`, `body`, `context`, `path`, `method`, `sdl`, `condition`, and optional `url`. When set, the router sends a coprocessor request at the **HttpRequest** stage when it receives the raw HTTP request.
- **`coprocessor.http_service.response`** – Same structure as `router.response`: `headers`, `body`, `context`, `sdl`, `status_code`, `condition`, and optional `url`. When set, the router sends a coprocessor request at the **HttpResponse** stage when it has the raw HTTP response to send to the client.
...
See the [coprocessor reference](/graphos/routing/customization/coprocessor/reference#example-requests-by-stage) for example HttpRequest and HttpResponse request bodies.
