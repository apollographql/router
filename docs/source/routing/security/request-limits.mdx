...
limits:
  router:
    ...
    max_non_local_selections: 100000 # Default value
...
Limits an estimate of the work needed to plan an operation across subgraphs. The estimate accounts for selections that require the planner to explore fetch options and the possible starting points for those options. It can exceed the number of fields in the operation.

When the estimate exceeds this limit, the router rejects the operation with a 400 HTTP status code and a GraphQL error with `"extensions": {"code": "QUERY_PLAN_COMPLEXITY_EXCEEDED"}`. The router enforces this limit in [`warn_only` mode](#warn_only-mode) too.

The default value is `100000`. This limit is available on all plans and does not require a license.

Raise `max_non_local_selections` to allow legitimate operations that exceed the default. Setting the `APOLLO_ROUTER_DISABLE_SECURITY_NON_LOCAL_SELECTIONS_CHECK` environment variable to `true` disables this check entirely, leaving query planning without this protection.
...
In [`warn_only` mode](#warn_only-mode), violations of `max_depth`, `max_height`, `max_aliases`, `max_root_fields`, and `max_recursive_selections` allow execution to continue. Other limits, including `max_non_local_selections`, still reject operations that exceed them.
