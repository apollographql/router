...
- `RouterHttpRequest`: The **RouterHttp** pipeline has received a raw HTTP request, before the Router pipeline. This runs at the earliest possible point, analogous to Rhai's `router_http` hook.
...
<Caution>
Using the **RouterHttp** stage (`RouterHttpRequest` / `RouterHttpResponse`) means your coprocessor runs *before* traffic shaping, limits, CSRF, and other router protections. Only use the **RouterHttp** stage if you accept that risk and your coprocessor can scale to handle the load.
</Caution>
