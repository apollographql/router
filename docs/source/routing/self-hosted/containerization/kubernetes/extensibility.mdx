...
The router supports three extensibility options to customize the router's behavior. The extensibility features are:

- [WebAssembly plugins](/graphos/routing/customization/wasm)
...
## Deploy with WebAssembly plugins

Package your WebAssembly component in the router image or mount it into the router container as a read-only file. Set each plugin's `source.path` to the component's path inside the container and configure a `source.digest` to verify the deployed bytes.

The router loads and compiles enabled components when it starts or reloads its configuration. Every router replica therefore needs access to the same component files at the configured paths.

See [WebAssembly plugin configuration](/graphos/routing/customization/wasm/#configure-the-plugin) for a complete example.
