...
telemetry:
  logging:
    ...
    supergraph:
      include_attributes: # Available attributes are request|response_body|response_headers|operation_name
        - request # Display the request at the supergraph level
    subgraph:
      all:
        include_attributes: # Available attributes are request|response_body|response_headers
