...
> **This Apollo Router feature is currently experimental.** As long as it's experimental, its behavior might change, or it might be removed entirely in a future release. [Learn about experimental features.](/resources/product-launch-stages#experimental-features)
>
...
The Apollo Router provides experimental support for request authentication via [JSON Web Token (JWT)](https://www.rfc-editor.org/rfc/rfc7519). This feature uses the popular [jsonwebtoken](https://crates.io/crates/jsonwebtoken) Rust library, which supports a variety of signature algorithms.
