...
When you set this option, the router watches for changes to its configuration file and any supergraph file passed with `--supergraph`. It reloads them automatically without downtime. This setting only affects local files provided to the router. GraphOS automatically reloads supergraphs and configurations provided via Launches (delivered via Uplink), regardless of this setting.
