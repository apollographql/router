...
Use Rhai to propagate extensions from subgraph responses to the client response. The following example demonstrates how to safely collect extensions from subgraph responses and merge them into the final response.

<Note>

**Important considerations:**

- **Race conditions**: Subgraph requests execute in parallel, so their responses might arrive in any order. Use defensive coding to handle concurrent updates.
- **Key conflicts**: Different subgraphs might use the same extension keys. The last subgraph response to complete will overwrite earlier values for conflicting keys.
...
fn supergraph_service(service) {
  let f = |response| {
    // Check if the router collected any extensions from subgraphs
...
2. If the hook finds extensions, it stores them in the request context (shared across all services).
3. The `supergraph_service` hook runs after the router collects and merges all subgraph responses.
4. It retrieves all stored extensions from the context and adds them to the final response's `extensions` field.

### Customization options

Modify the script to control extension merging, handle key conflicts, or filter specific extensions:
    ...
    if response.context["extensions"]?.len > 0 {
      ...
      for extension_map in response.context["extensions"] {
        for key in extension_map.keys() {
          ...
          // Or: Add your own conflict resolution logic here
