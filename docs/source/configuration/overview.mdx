...
#
# graphql: Configuration of the GraphQL server
#
graphql:
...
By default, the router doesn't answer to introspection queries. You can override this behavior to enable the introspection like so:
...
By default, the router doesn't display a landing page if you access its endpoint path via your browser. You can override this behavior to enable the landing page like so:

```yaml title="router.yaml"
#
# router: Configuration of the GraphQL server
#
sandbox:
  enabled: true
```
