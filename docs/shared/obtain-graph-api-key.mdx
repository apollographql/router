...
3. Open your graph's Settings page and scroll down to the API Keys section. Either copy an existing key or click **Create New Key**. Give your key a name, such as `Production`. This helps you keep track of each API key's use.
    ...
    > If you don't see the API Keys section, you don't have sufficient permissions for your graph. Only organization members with the `Org Admin` or `Graph Admin` role can manage graph API keys. [Learn more about member roles.](/graphos/platform/access-management/member-roles)
