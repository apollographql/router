...
When active, the `required_to_start` option prevents the router from starting if it cannot connect to Redis. By default, the router starts without a connection to Redis, which would result in only using response caching sending the requests to subgraphs undisturbed:
