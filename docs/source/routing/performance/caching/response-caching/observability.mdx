...
- `experimental.apollo.router.operations.response_cache.maintenance.deduplicated_commands`: Redis `ZREMRANGEBYSCORE` commands avoided per drain cycle because the same cache tag appeared multiple times in the maintenance queue. Non-zero values indicate the deduplication logic is reducing Redis pressure — expected at high insert rates when many requests write to the same subgraph simultaneously.
