...
The Apollo Router takes an optional YAML configuration file as input via the `--config` option. This file enables you to customize the router's behavior in many ways:
...
See [OpenTelemetry in the Apollo Router](./opentelemetry/).
...
The Apollo Router automatically supports APQ via its in-memory cache. **No configuration options are supported at this time.**

The following APQ features currently available in Apollo Server might be supported in a future release of the Apollo Router:

* Support for external data stores like Redis and Memcached
* Automatic subgraph `@cacheControl` aggregation, so you don’t have to manually propagate `cache-control` headers from your subgraphs
