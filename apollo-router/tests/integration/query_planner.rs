...
#[tokio::test(flavor = "multi_thread")]
async fn progressive_override_with_legacy_qp_change_to_new_qp_updates_config() {
    if !graph_os_enabled() {
        return;
    }
    let config = format!("{PROMETHEUS_METRICS_CONFIG}\n{LEGACY_QP}");
...
#[tokio::test(flavor = "multi_thread")]
async fn progressive_override_with_legacy_qp_reload_to_both_best_effort_updates_config() {
    if !graph_os_enabled() {
        return;
    }
    let config = format!("{PROMETHEUS_METRICS_CONFIG}\n{LEGACY_QP}");
