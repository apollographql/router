...
The [`limits.http_max_request_bytes`](/graphos/routing/security/request-limits#http_max_request_bytes) setting applies to the GraphQL operations field (the query and variables) of the multipart request, not to the uploaded file data itself.
File data is bounded by `protocols.multipart.limits.max_file_size`.
