...
* The `public_url` must include the configured `path` on the Router. For example, given `path` = `/my_callback` then your `public_url` must be something like `http://localhost:8080/my_callback`.
If you have a proxy in front of the Router that handles the redirection to the right `path` configured in the Router then you can specify another path for the `public_url`, for example `http://localhost:8080/external_path`.
Under the hood in the Router we will take this URL and add the subscription id in the url to finally have `http://localhost:8080/external_access/{subscription_id}` and pass it directly to your subgraphs.
If you didn't specify the `path`, by default it's `/callback` so you'll have to specify it in `public_url`.
...
subscription:
  ...
  mode:
    callback:
      public_url: https://example.com:4000/callback # The router's public URL, the one on which your subgraphs can have access to the Router, this must include the path configured on the Router
