...
| `response_header`  | Yes         |                                                       | The name of a response header                                                     |
| `is_primary_response` | No       | `true`                                                  | Boolean indicating whether the current response chunk is the primary one (as opposed to a deferred or subscription event). At per-chunk scope (for example, on an event-level instrument), returns `true` for the primary chunk and `false` for subsequent chunks. See [splitting metrics between primary and deferred chunks](#splitting-metrics-between-primary-and-deferred-chunks). |
