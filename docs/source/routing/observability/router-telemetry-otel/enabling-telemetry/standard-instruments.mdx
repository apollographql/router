...
These attributes represent how a span spends time (in nanoseconds) over its lifetime and how it contributes to the total request overhead. Your APM provider can likely use this trace data to generate synthetic metrics, which you can then create an approximation view of how particular spans affect the router performance.
...
- What should our target be for "optimal" router overhead?
  - Apollo can provide all of the best practices to monitor your overhead, however, to many other factors determine what your overhead will be for a given supergraph, subgraphs, and its request traffic. Instead we reccomend tracking the delta or percentage change of the metric in your production deployment rather than a fixed number or specific variance.
- What should our target be for "optimal" router CPU utilization?
  - Using the various metrics, you can now have a better understanding of what goes into the CPU usage. What is optimal for your team will depend on your expected load, spikes in traffic, time to scale infrastructure, etc. We can not provide generalized recommendations but encourage the community to do their own validations and share their results with the [Community](https://community.apollographql.com/).
...
- How do we measure the subgraph response time?
...
- How do I find the overhead for a particular operation type (Query, Mutation, Subscriptions) or operation name?
  - To filter overhead by specific operation type or even operation id, you will need to look at trace and span attributes and use your APM to create specific metric views for the `busy_ns` and `idle_ns`. Since the `apollo.router.overhead` is a histrogram metric, it is collected and joined together over all operations.
