...
Add the `--dev` flag is the equivalent runnning `./router --hot-reload` with this configuration:
...
If you need some of these features enabled in production, specify them directly in your configuration instead of using `--dev`.
