...
limits:
  ...
  # New middleware-level header limits
  http_max_request_headers: 100    # Limit total number of headers in a request
  http_max_header_list_items: 10   # Limit number of values per header field
...
```yaml title="router.yaml"
limits:
  http_max_header_list_items: 5  # Allow up to 5 values per header
```
  ...
  "errors": [
    {
      ...
      "extensions": {
        "details": "Request header list too many items",
        "code": "INVALID_GRAPHQL_REQUEST"
      }
    }
  ]
