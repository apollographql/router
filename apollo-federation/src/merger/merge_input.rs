  ...
  pub(crate) fn merge_input(
    ...
    for (dest_field, subgraph_fields) in added {
        ...
        let is_inaccessible = if let Some(dest_field_def) = dest_field_def {
            if let Some(inaccessible_directive) = &self.inaccessible_directive_name_in_supergraph {
        ...
        if !is_inaccessible && Self::some_sources(&subgraph_fields, |field, _idx| field.is_none()) {
            ...
            if !non_optional_subgraphs.is_empty() {
                let non_optional_subgraphs_str = non_optional_subgraphs.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(", ");
                let missing_subgraphs_str = missing_subgraphs.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(", ");
                ...
                let present_subgraphs_str = present_subgraphs.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(", ");
                let missing_subgraphs_str = missing_subgraphs.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(", ");
   ...
   fn add_fields_shallow(
    ...
    sources: &Sources<Node<InputObjectType>>, 
    dest: &InputObjectTypeDefinitionPosition
) -> IndexMap<crate::schema::position::InputObjectFieldDefinitionPosition, Sources<Component<InputValueDefinition>>> {
    
    let mut added: IndexMap<crate::schema::position::InputObjectFieldDefinitionPosition, Sources<Component<InputValueDefinition>>> = IndexMap::default();
    let mut fields_to_add: IndexMap<usize, HashSet<Option<Component<InputValueDefinition>>>> = IndexMap::default();
    let mut extra_sources: Sources<Component<InputValueDefinition>> = IndexMap::default();
    
    // Process each subgraph
    for (source_index, source) in sources.iter() {
        let fields_set = fields_to_add.entry(*source_index).or_insert_with(HashSet::new);
...
mod tests {
    ...
    fn create_input_object_type(name: &str, field_definitions: &[(&str, &str, bool)]) -> Node<InputObjectType> {
        ...
        for (field_name, field_type, is_required) in field_definitions {
            ...
            input_object_type.fields.insert(
                Name::new_unchecked(field_name),
                Component::new(field_def),
            );
        }
    ...
    fn test_input_object_type_creation() {
        let input1 = create_input_object_type("UserInput", &[
            ("name", "String", true),
            ("email", "String", false),
        ]);
        assert_eq!(input1.fields.len(), 2);
        assert!(input1.fields.contains_key(&Name::new("name").expect("Valid name")));
        assert!(input1.fields.contains_key(&Name::new("email").expect("Valid name")));
        
        // Check that name field is required (NonNull)
        let name_field = input1.fields.get(&Name::new("name").expect("Valid name")).unwrap();
        assert!(name_field.ty.is_non_null());
        
        // Check that email field is optional (nullable)
        let email_field = input1.fields.get(&Name::new("email").expect("Valid name")).unwrap();
    ...
    fn test_merge_input_combines_all_fields() {
        ...
        let input2 = create_input_object_type("UserInput", &[
            ("name", "String", true),
            ("age", "Int", false),
        ]);
        ...
        let final_input = dest.get(merger.merged.schema()).expect("input in supergraph");
        
        // Only 'name' should remain as it's present in both subgraphs
        assert_eq!(final_input.fields.len(), 1);
        assert!(final_input.fields.contains_key(&Name::new("name").expect("Valid name")));
    }
    ...
    fn test_merge_input_identical_fields_across_subgraphs() {
        ...
        assert!(final_input.fields.contains_key(&Name::new("name").expect("Valid name")));
        assert!(final_input.fields.contains_key(&Name::new("email").expect("Valid name")));
