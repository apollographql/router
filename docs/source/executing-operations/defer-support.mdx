---
title: GraphOS Router Support for @defer
subtitle: Improve performance by delivering fields incrementally
description: Improve your GraphQL query performance with GraphOS Router's support for the @defer directive. Incrementally deliver response data by deferring certain fields.
minVersion: 1.8.0
---

Queries sent to the GraphOS Router can use the `@defer` directive to enable the incremental delivery of response data. By deferring data for some fields, the router can resolve and return data for the query's _other_ fields more quickly, improving responsiveness.

The GraphOS Router's `@defer` support is compatible with all [federation-compatible subgraph libraries](/federation/building-supergraphs/supported-subgraphs/). That's because the router takes advantage of your supergraph's existing [entities](/federation/entities/) to fetch any deferred field data via followup queries to your subgraphs.
...
To respond incrementally, the GraphOS Router uses a multipart-encoded HTTP response. To use `@defer` successfully with the GraphOS Router, a client's GraphQL library must _also_ support the directive by handling multipart HTTP responses correctly.

The GraphOS Router's `@defer` support is compatible with all [federation-compatible subgraph libraries](/federation/building-supergraphs/supported-subgraphs/), because the deferring logic exists entirely within the router itself.
...
The remainder of this article covers the GraphOS Router's defer implementation in greater depth.

## Executing a `@defer` query

To execute a deferred query on the GraphOS Router, a GraphQL client sends an HTTP request with _almost_ the exact same format that it uses for usual query and mutation requests.
...
## How does the GraphOS Router defer fields?

As discussed in [this article](/graphos/operations/defer/#which-fields-can-my-router-defer), the GraphOS Router can defer the following fields in your schema:
...
A query's `@defer` fragment might include fields that the GraphOS Router _can't_ defer. The router handles this case gracefully with the following logic:
...
The GraphOS Router supports the `@defer` directive as it's documented in [these edits to the RFC](https://github.com/graphql/graphql-spec/pull/742), according to the state of those edits on 2022-08-24.

## Disabling `@defer`

Defer support is enabled in the GraphOS Router by default. To _disable_ support, add `defer_support: false` to your router's [YAML config file](../configuration/overview/#yaml-config-file) under the `supergraph` key:
