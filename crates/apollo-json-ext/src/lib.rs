...
pub mod prelude {
    pub use super::{Object, Path, ValueExt};
...
#[derive(Debug, Error)]
pub enum Error {
    #[error("could not find path in JSON")]
    PathNotFound,
    #[error("attempt to flatten on non-array node")]
    InvalidFlatten,
}
...
pub trait ValueExt {
    ...
    #[track_caller]
    fn get_at_path<'a, 'b>(&'a self, path: &'b Path) -> Result<Vec<&'a Self>, Error>;

    /// Get a mutable reference to the value(s) at a particular path.
    #[track_caller]
    fn get_at_path_mut<'a, 'b>(&'a mut self, path: &'b Path) -> Result<Vec<&'a mut Self>, Error>;
        ...
        for path_element in path.iter() {
            current = match path_element {
                PathElement::Flatten => current
                    .into_iter()
                    .map(|x| x.as_array().ok_or(Error::InvalidFlatten))
                    .collect::<Result<Vec<_>, _>>()?
                    .into_iter()
                    .flatten()
                    .collect::<Vec<_>>(),
                PathElement::Index(index) if !(current.len() == 1 && current[0].is_array()) => {
                    vec![current.into_iter().nth(*index).ok_or(Error::PathNotFound)?]
                }
                path_element => current
                    ...
                    .map(|value| match (path_element, value) {
                        ...
                        (PathElement::Key(_), _) => Err(Error::PathNotFound),
    ...
    fn get_at_path_mut<'a, 'b>(&'a mut self, path: &'b Path) -> Result<Vec<&'a mut Self>, Error> {
        ...
        for path_element in path.iter() {
            current = match path_element {
                PathElement::Flatten => current
                    .into_iter()
                    .map(|x| x.as_array_mut().ok_or(Error::InvalidFlatten))
                    .collect::<Result<Vec<_>, _>>()?
                    .into_iter()
                    .flatten()
                    .collect::<Vec<_>>(),
