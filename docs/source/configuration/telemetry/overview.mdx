---
title: GraphOS Router Telemetry
subtitle: Collect observable data to monitor your router and supergraph
description: Observe and monitor the health and performance of GraphQL operations in the GraphOS Router by collecting and exporting telemetry logs, metrics, and traces.
---

import TelemetryPerformanceNote from '../../../shared/telemetry-performance.mdx';

In this overview, learn about:
- How GraphOS Router telemetry enables supergraph observability and debuggability
...
The GraphOS Router provides the necessary data to monitor its health and troubleshoot issues. The router's observability is critical for maintaining a healthy, performant supergraph and minimizing its [mean time to repair (MTTR)](https://en.wikipedia.org/wiki/Mean_time_to_repair). 
...
The GraphOS Router collects different types of telemetry, including:
...
Logs record **events** in the GraphOS Router. Examples of logged events include:
...
Traces monitor the flow of a request through the GraphOS Router. A trace is composed of [**spans**](./instrumentation/spans). A span captures a request's duration as it flows through the router request lifecycle. Spans may include contextual information about the request, such as the HTTP status code, or the name of the subgraph being queried.
...
The GraphOS Router exports its collected telemetry in formats compatible with industry-standard APM tools. The router supports logging, metrics, and tracing exporters for a variety of tools, including:
...
You can annotate events, metrics, and spans with **attributes**. Attributes are key-value pairs that add contextual information about the GraphOS Router pipeline to telemetry. You can then use these attributes to filter and group data in your APMs.
