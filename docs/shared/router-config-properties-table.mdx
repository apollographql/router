...
Configuration for the experimental diagnostics plugin (dashboard and API for configuration, system info, memory profiling, and export). See [Diagnostics (experimental)](/graphos/routing/self-hosted/diagnostics).
...
limits:
  router:
    ...
    max_recursive_selections: 10000000
    max_aliases: null
    max_depth: null
    max_height: null
...
Configuration for router reload retry behavior. See [Configuring reload retries](/router/configuration/hot-reload-schema#configuring-reload-retries) for details.
