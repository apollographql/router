...
The easiest way to see GraphQL operation data without enabling additional telemetry is through [GraphOS Insights](/graphos/platform/insights). GraphOS Studio lets you analyze data from  failed requests, such as client IDs and [exact GraphQL errors](/routing/graphos-reporting#errors).
