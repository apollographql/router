    ...
    async fn create<'a>(
                ...
                .find(|factory| factory.name == "apollo.telemetry")
        {
            ...
            if let Some(plugin_config) = &mut telemetry_config {
                ...
                let telemetry_init = match previous_telemetry_config {
                    Some(prev_config) => PluginInit::builder()
                        .config(plugin_config.clone())
                        .previous_config(Some(prev_config))
...
pub(crate) async fn add_plugin(
    ...
    let plugin_init = match previous_plugin_config {
        Some(prev_config) => PluginInit::builder()
            .config(plugin_config.clone())
            .previous_config(Some(prev_config.clone()))
