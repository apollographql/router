...
- `apollo.router.query_planning.warmup.operations` - operations processed during warm-up, attributed by `outcome` (`success`, `planner_error`, `timeout`) and `source` (`persisted_query`, `cache`).
