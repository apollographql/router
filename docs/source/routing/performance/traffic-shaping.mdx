...
The router maintains a pool of HTTP connections to each subgraph. Two options let you control how idle connections are managed.
...
#### Idle connection eviction mode

Use `pool_idle_eviction_mode` to control when expired connections are actually closed:

- `lazy` (default): connections are only evicted at checkout time. When the router fetches a connection from the pool to make a request, it discards any connection that has been idle longer than `pool_idle_timeout` and opens a fresh one. No TCP closes are sent between requests.
