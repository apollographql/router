...
- `sampler` sets the sampling rate as a decimal percentage, `always_on`, or `always_off`. For example, setting `sampler: 0.1` samples 10% of your requests.
  - [`always_off`](https://opentelemetry.io/docs/specs/otel/trace/tracestate-probability-sampling/#always-off-sampler) means that no spans will make it to your APM. It doesn't mean that sampling is disabled
