...
The `Authorization error` log event for a refused operation now appears in the `execution` span instead of the `query_planning` span. Update log or trace filters that match this event by span name.
