...
Using Rhai can be quite limiting and is best suited for simple modifications — such as altering headers, modifying context values, or lightweight payload transformations.

For more advanced functionality — including network requests, writing to disk, using RUST Crates injecting custom span data, or accessing external data — prefer using YAML configuration or [external co-processing](/router/customizations/coprocessor/) (an [Enterprise feature](/router/enterprise-features)).
