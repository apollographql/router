...
- `apollo.router.graphql_error` - counts GraphQL errors in responses. Also counts errors which occur during the response validation phase, which are represented in client responses as `extensions.valueCompletion` instead of actual GraphQL errors. Attributes:
  ...
  - Errors a connector's response mapping declares with `->withError` are counted here too, even though clients read them in `extensions.connectorErrors` rather than in `errors`. They are counted at the connector, so `include_subgraph_errors` withholding an error from a client does not also suppress its metric.
