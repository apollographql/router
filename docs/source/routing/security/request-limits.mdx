...
If you'd rather not configure `limits` at all yet — or you want to capture the offending operations along with attributes like client name and operation name in your logs or APM — see [Logging operations that exceed a candidate limit](#logging-operations-that-exceed-a-candidate-limit) below.
...
### Logging current values of operations
...
Repeat the block — with a different event name, selector, and threshold in `condition.gt` — for each of `depth`, `height`, and `root_fields` you want to track. Once you're confident in the values, you can switch over to enforcing them with `limits.max_*`.
