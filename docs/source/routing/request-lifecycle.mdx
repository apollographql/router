...
* The **RouterHttp** pipeline runs first at the raw HTTP layer. Rhai scripts, native plugins, and built-in features (such as license enforcement) can hook in here before the request reaches the Router service. Static pages and health endpoints also pass through this layer.
...
2. The request passes through the **RouterHttp** pipeline at the raw HTTP layer, where plugins (Rhai, native, license enforcement) can run. The HTTP server then transforms the HTTP request into a `RouterRequest` containing HTTP headers and the request body as a stream of byte arrays.
