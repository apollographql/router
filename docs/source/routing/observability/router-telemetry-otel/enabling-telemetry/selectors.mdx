...
The `http_client` service only supports the `request_header` and `response_header` selectors. Unlike other services, conditions and the `static` selector are **not** supported. If you attempt to configure them, your router fails to start.
