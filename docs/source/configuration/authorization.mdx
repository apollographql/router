...
When using subscriptions with `@policy` authorization, make sure to handle the special case of restarting subscription events.

Because subscription events restart from the `ExecutionService` of the router request lifecycle, if the authorization status of the subscription session changes, the session should be closed so that it doesn't go through query planning again. 

To handle this case, a policy should reevaluate at the `ExecutionService` level. If a reevaluation detects a changed authorization status, a policy should throw an error to stop the subscription.

##### Example with a Rhai script

As an example, the Rhai script below reevaluates a policy in the `ExecutionService` level and throws an error to stop a subscription:
