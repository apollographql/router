...
Define `router_http` if your customization must run at the earliest possible point—for example, very early request checks or logic that applies to static pages and health endpoints as well as GraphQL. It uses the same `service` API as other hooks (`map_request`, `map_response`).
