...
The Apollo Router enables JSON-formatted logging only if `stdout` is _not_ a TTY.
...
More specifically, each subgraph's configuration includes a `layers` array that lists a sequence of transformations for the router to perform (in order) before sending a request:
...
See [Managed federation with the Apollo Router](/studio/).
