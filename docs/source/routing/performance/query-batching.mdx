...
- If [query deduplication](/router/configuration/traffic-shaping/#query-deduplication), [response caching](/router/configuration/response-caching), or [entity caching](/router/configuration/entity-caching) are enabled, they will not apply to batched queries. Batching will take precedence over these features. Query deduplication, response caching, and entity caching will still be performed for non-batched queries.
