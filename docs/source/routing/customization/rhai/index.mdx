...
- Most beneficial for deployments processing many concurrent requests through scripts that access a large number of distinct string values (such as long context keys, header names, and template-interpolated values).

**Cons of disabling string interning:**
- Repeated identical strings are each allocated separately rather than sharing a single allocation. This increases memory pressure and allocation frequency, which may be noticeable in scripts that produce a high volume of repeated short strings.
...
- For low-concurrency deployments or scripts with limited string variety, the default of 256 interned strings may offer better overall performance by amortizing allocation costs.

Benchmark your specific script workload under realistic concurrency before changing this setting.
