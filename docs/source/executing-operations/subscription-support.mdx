...
>
> For cloud routers, [subscription support is included with a GraphOS Serverless plan](/graphos/operations/subscriptions).
    ...
    passthrough:
      ...
      subgraphs: # Overrides subscription settings for individual subgraphs
        reviews: # Overrides settings for the 'reviews' subgraph
          ...
          protocol: graphql_transport_ws # The WebSocket-based subprotocol to use for subscription communication (Default: graphql_transport_ws)
```
...
By default, the router uses the `graphql_transport_ws` protocol option for all subgraphs. You can change this global default and/or override it for individual subgraphs by setting the `protocol` key as shown above.
...
>    * `graph-ws` (with a hyphen `-`) is the name of the [library](https://github.com/enisdenjo/graphql-ws) that uses the recommended `graphql_transport_ws` WebSocket subprotocol. 
>     * `graph_ws` (with an underscore `_`) is the router's configuration value to use the legacy subprotocol of the `subscriptions-transport-ws` library.
