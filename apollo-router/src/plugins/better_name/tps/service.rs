...
impl<S, Request> Service<Request> for TpsLimit<S>
where
    S: Service<Request>,
    S::Error: Into<tower::BoxError>,
{
    ...
    fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        let time_unit = self.tps.interval().as_millis() as u64;
            ...
            self.window_start
                .fetch_update(Ordering::SeqCst, Ordering::SeqCst, |window_start| {
                    let duration_now = SystemTime::now()
                        ...
                        .as_millis() as u64;
                    if duration_now - window_start > self.tps.interval().as_millis() as u64 {
                        Some(duration_now)
                    } else {
                        None
                    }
        ...
        if estimated_cap as u64 > self.tps.capacity() {
            tracing::trace!("rate limit exceeded; sleeping.");
            return Poll::Ready(Err(TpsLimited::new().into()));
        }
