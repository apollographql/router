            ...
            .map_response(move |response: SupergraphResponse| {
                response.map_stream(move |mut graphql_response: graphql::Response| {
                    for error in &mut graphql_response.errors {
                        // If no subgraph name is found then we'll use the default rules
                        let subgraph_name = error.subgraph_name().unwrap_or_default();
                        // Get the effective config for this specific subgraph, or use default
                        let effective_config = config.subgraphs.get(&subgraph_name).unwrap_or(&config.default);

                        if !effective_config.include_errors {
                            tracing::debug!("Redacting errors for subgraph '{}' based on config: include_errors=false", subgraph_name);
