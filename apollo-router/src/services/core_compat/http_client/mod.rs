...
pub(super) use apollo_router_core::services::http_client::{Request as CoreRequest, Response as CoreResponse};
...
impl From<RouterHttpRequest> for CoreRequest {
    fn from(mut router_request: RouterHttpRequest) -> Self {
        ...
        let core_body = router_body.map_err(|err| -> BoxError { err.into() }).boxed_unsync();
