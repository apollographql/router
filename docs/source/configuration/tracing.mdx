...
telemetry:
  tracing:
    trace_config:
      service_name: "${MY_ENV_VARIABLE:router}"
      service_namespace: "apollo"
      # Optional. Either a float between 0 and 1 or 'AlwaysOn' or 'AlwaysOff'
      sampler: AlwaysOn
...
```
Specifying explicit propagation is generally only required if you are using an exporter that supports multiple trace ID formats. For example OpenTelemetry collector or Jaeger.
