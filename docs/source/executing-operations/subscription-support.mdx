...
description: Real-time updates with GraphOS Enterprise
---
...
With [GraphOS Enterprise](/graphos/enterprise/), the Apollo Router provides support for GraphQL subscription operations:
...
You can add `Subscription` fields to any subgraph that supports common WebSocket protocols for subscription communication.

## What are subscriptions for?

GraphQL subscriptions enable clients to receive continual, real-time updates whenever new data becomes available. Unlike queries and mutations, subscriptions are long-lasting, which means a client can receive multiple updates from a single request:
...
Subscriptions are _best_ suited to use cases that involve frequently-changing, time-sensitive data (stock prices, sensor readings, live chat, and so on).
...
1. A client executes a subscription operation against your router _over HTTP:_
   ...
   - **The client does _not_ use a WebSocket protocol!** Instead, it uses [HTTP with chunked multipart responses](https://github.com/graphql/graphql-over-http/blob/main/rfcs/IncrementalDelivery.md).
...
2. Your router executes the _same_ subscription against whichever subgraph defines the requested field (`stockPricesChanged` in the example above).
...
3. Whenever your router receives updated subscription data from the subgraph, it returns that data to the client in an additional HTTP response "chunk".
   - If the subscription includes federated entity fields that are defined in _other_ subgraphs, the router first fetches those fields by querying the corresponding subgraphs (such as **Portfolios** in the diagram above). These queries use HTTP as usual.

> [Walk through an example.](#example-execution)

## Prerequisites

⚠️ **Before you add `Subscription` fields to your subgraphs,** do _all_ of the following _in the order shown_ to prevent composition errors:
...
4. Modify your subgraph schemas to use Apollo Federation 2.4 or later:
    ...
    - You _only_ need to modify subgraph schemas that define `Subscription` fields.
...
In your router's [YAML config file](../configuration/overview/#yaml-config-file), you configure how the router communicates with each subgraph when executing GraphQL subscriptions.

The Apollo Router supports two popular [WebSocket protocols](#websocket-setup) for subscriptions, and it also provides preview support for an [HTTP-callback-based protocol](#http-callback-setup-preview). **Your router must use whichever protocol is expected by each of your subgraphs!**

### WebSocket setup

Here's an example configuration that sets up subgraph subscriptions over WebSocket:
...
This example enables subscriptions in **passthrough mode**, which uses long-lived WebSocket connections. The following WebSocket subprotocols are supported:

- [`graphql_ws`](https://github.com/enisdenjo/graphql-ws)
...
By default, the router uses `graphql-ws` for all subgraphs. You can change this global default and/or override it for individual subgraphs by setting the `protocol` key as shown above.
...
Here's an example configuration that sets up subgraph subscriptions over HTTP via callbacks:
...
**[Callback mode](#callback-mode-preview) (preview):** The router doesn't open a long lived connection from the router to subgraph and instead ask to the subgraph to send an HTTP request to the router with a specific payload each time he has an event.
...
- The operations have identical GraphQL selection sets (i.e., requested fields).
...
Note that this is a _global_ setting (not per-subgraph).
...
By default, if you've configured your router to [propagate](../configuration/header-propagation/) HTTP `Authorization` headers to your subgraph, then the router automatically sets `connectionParams` when initiating a WebSocket connection.

For example, when your router sends the [`connection_init` message](https://github.com/enisdenjo/graphql-ws/blob/master/PROTOCOL.md#connectioninit), it includes a payload like this to provide the `Authorization` header:
...
> The value of `queue_capacity` corresponds to the _maximum number of enqueued subscription events_, not the _total size_ of those events.
...
Client subscriptions are [long-lived HTTP(S) connections](#how-it-works), which means they might remain open indefinitely. You can limit the number of simultaneous client subscription connections in your router's YAML config file, like so:
...
If a client attempts to execute a subscription on your router when it's already at `max_open_subscriptions`, the router rejects the client's request.
