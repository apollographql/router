...
Apollo's legacy reporting didn't include input and enum references, meaning you couldn't see input and enum field usage in GraphOS insights and some operation checks may have flagged non-issues.
Beginning in v1.50.0, you can configure the router to report input and enum usage.
...
Once you enable this configuration, you can view enum and input usage along with other types of [field usage in GraphOS Studio](/graphos/metrics/field-usage). To-do: Information about enhanced checks + link to relevant GraphOS docs.

<Note>

Once you enable `extended` reference reporting, you won't see an immediate change in your operations check behavior. This is because operation checks use historical client operation data and need some historical operations with the `extended` reporting for comparison.
