...
impl FieldRoutingSearchSpace {
    ...
    pub(super) fn commit_choice(
        ...
        if matches!(choice, RoutingChoice::RefetchAncestor) {
            return if self.commit_refetch_ancestor(state, pending) {
                Ok(())
            } else {
                Err(FederationError::internal(
                    "ancestor refetch not viable at this position",
                ))
            };
        }
    ...
    fn subgraph_root_kind(
        ...
        let subgraph_schema = self.qg().schema_by_source(subgraph)?;
        Ok([
    ...
    fn intersection_filter_for_field(
        ...
        let target_data = self.qg().node_weight(target_node)?;
    ...
    fn field_in_multiple_subgraphs(
        ...
        let source_data = self.qg().node_weight(source_node)?;
        ...
        let mut count = 0u32;
        for (_source, schema) in self.qg().subgraph_schemas() {
    ...
    fn ensure_abstract_typename(
        ...
        if let Selection::Field(_) = &pending.selection {
            let target_data = self.qg().node_weight(target_qg_node)?;
    ...
    pub(super) fn dispatch_sub_selections(
        ...
        // ancestor resolution.
        let target_node_data = self.qg().node_weight(target_qg_node)?;
        ...
        // (see `refetch_ancestor_candidate`).
    ...
    fn child_provides_anchor(
        ...
        let source_is_copy = self
            ...
            .provide_id
            .is_some();
        let target_is_copy = self.qg().node_weight(target_qg_node)?.provide_id.is_some();
