...
When you use the JWT authentication plugin, the router requests keys from the [JWKS URL you configure](/graphos/routing/security/jwt#jwks) and caches them on start up.
It then polls the JWKS endpoint at the [configured interval](/graphos/routing/security/jwt#jwks) and caches any updates.
...
sequenceDiagram
  ...
  Router->>JWKS: Fetch key set at startup
  JWKS->>Router: Return public keys
  loop Periodically
    Router->>JWKS: Poll for updated keys
    JWKS->>Router: Update cached keys (if any)
  end
```
...
When the router receives a request, it uses the cached keys to validate any JWT requests.
...
sequenceDiagram
  ...
  note over Router: Validate JWT using cached keys
