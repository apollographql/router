...
fn test_request_response_type_aliases() {
    ...
    let body = UnsyncBoxBody::new(
        Full::new(Bytes::from("test")).map_err(|_: std::convert::Infallible| unreachable!()),
    );
    ...
    let body = UnsyncBoxBody::new(
        Full::new(Bytes::from("response")).map_err(|_: std::convert::Infallible| unreachable!()),
    );
    let _response: Response = http::Response::builder().status(200).body(body).unwrap();
}
