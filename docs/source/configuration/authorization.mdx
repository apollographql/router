...
The `reject_unauthorized` option configures whether to reject an entire query if any authorization directive failed, where any part of it is filtered by authorization directives. When enabled, a response contains the list of paths that are affected.
...
You can configure `response` to set where to place filtered paths in a GraphQL response:
