...
| `static_estimated.actual_cost_mode`               | `by_subgraph`                      | `by_subgraph` | `by_subgraph` calculates the cost of an operation as the sum of the cost of each subgraph response.                                |
...
The `actual_cost_mode` option determines how the actual cost of an operation is calculated.
