...
fn url_safe_method(
    ...
    _method_args: {_method_args:?} \n
    data: {data:?} \n
    _vars: {_vars:?} \n
    _input_path: {_input_path:?} \n
    ");
    if let JSON::String(_) = data {
        (Some(data.to_owned()), vec![])    
    } else {
        (Some(JSON::String("_".into())), vec![])
    }
}
...
mod tests {
    mod url_safe {
        ...
        #[test]
        fn safe_join() {
            let applied = selection!("$->joinNotNull(','->urlSafe)").apply_to(&json!(json!(["a","b","c"]))).0;
            assert_eq!(applied, Some(json!("a,b,c")));
        }

        #[test]
        fn non_safe_join() {
            let applied = selection!("$->joinNotNull(',')").apply_to(&json!(json!(["a","b","c"]))).0;
