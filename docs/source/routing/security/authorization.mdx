...
If _every_ requested field requires authentication and a request is unauthenticated, the router returns `data: null` and reports errors according to the `errors` configuration below.
...
### Authorization on root operation types

To scope authorization on `Query`, `Mutation`, and `Subscription` reliably, apply `@authenticated`, `@requiresScopes`, or `@policy` to each field rather than to the root operation type itself:

```graphql
type Mutation {
  bookHotel: BookedHotel @authenticated
}
```

Root operation types are shared merged types in a federated graph. Every subgraph that contributes fields to `Query`, `Mutation`, or `Subscription` shares the same composed root type in the supergraph. An authorization directive applied to the root type composes into that shared root and applies to every field on it, including fields that other subgraphs contribute.

The following pattern is valid composition, but produces cross-subgraph enforcement that is rarely intended:
...
Every field on `Mutation` across every subgraph now requires authentication, not only `bookHotel` in the subgraph that declared the directive.

Keep `@authenticated`, `@requiresScopes`, and `@policy` on individual fields in every subgraph that contributes to a root operation type, so each subgraph enforces only what it owns.
...
## `@include`, `@skip`, and authorization directives

GraphQL's built-in `@include` and `@skip` directives let a client conditionally request a field. When the condition is a literal `true` or `false` value, for example `@include(if: false)`, the router excludes that field from the query plan entirely, so it never reaches a subgraph.

**A field the router excludes this way doesn't produce an authorization error**, even if it's protected by `@requiresScopes`, `@authenticated`, or `@policy` and the request doesn't meet that requirement. Because the field is never resolved, there's nothing to authorize.

This only applies to a literal `true` or `false` value. When `@include` or `@skip` uses a variable, for example `@include(if: $showDetails)`, the router can't know its value until execution, so the field is still checked against your authorization directives and can still produce a `UNAUTHORIZED_FIELD_OR_TYPE` error.
...
By default, when a query is filtered by authorization (either partially or entirely), the list of filtered paths is added to the response and logged by the router. You can customize this behavior for your needs.
