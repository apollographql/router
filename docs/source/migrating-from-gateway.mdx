...
Apollo Gateway runs it's core GraphQL server using Apollo Server and an user configured HTTP server. Apollo Router is built as an all-in-one product with the GraphQL and HTTP server bundled together. With this change there may be some slight difference to the logic of HTTP status codes or GraphQL error extension codes.

Refer the full list of [Apollo Router error codes](https://www.apollographql.com/docs/router/errors) for any changes to your Gateway implementation and if you need to customize the responses, learn more in [Apollo Router customizations](https://www.apollographql.com/docs/router/customizations/overview).
