...
### Configure caching for Apollo Connectors

To cache responses from [Apollo Connectors](/graphos/connectors), add a `connector` block alongside the `subgraph` block. Connector sources are identified using the `subgraph_name.source_name` format.
...
response_cache:
  ...
  connector:
    all:
      enabled: true
      ttl: 10m # Fallback TTL when responses don't include Cache-Control headers (optional; defaults to 24h)
    ...
    # Configure overrides for specific connector sources. A listed source inherits `all`'s
    # cache_key_headers unless it sets its own.
    sources:
      "products.my_rest_api":
        ttl: 5m # Override TTL for this source
      "inventory.stock_api":
...
The `connector` configuration block mirrors the `subgraph` block, but uses `sources` (keyed by `subgraph_name.source_name`) instead of `subgraphs` (keyed by subgraph name).
...
The `apollo.router.response.cache` metric tracks cache hits and misses for both subgraph and connector requests. Custom attributes and selectors, like `supergraph.operation.name` above, apply only to subgraph requests — connector requests always report a fixed set of attributes: `graphql.type.name`, `cache.hit`, and `subgraph.name` (the connector source name).

You can use this metric to create a dashboard similar to the following example:
...
<Note>
    This example uses a simple, non-nullable `@key`. Simple, non-nullable `@key`s improve reliability and performance, but the router also supports compound, non-nullable `@key`s. For details, see the [FAQ](/routing/performance/caching/response-caching/faq)
</Note>
...
If you execute an example query twice in Apollo Sandbox with [cache debugger](./observability#cache-debugger) enabled you should see all the entries coming from `cache` in `source` column:

<img
  className="screenshot"
  alt="Response cache debugger example"
  src="../../../../images/response-cache/debugger-cache-query-ttl.png"
/>

### Invalidation

With cached data based on TTL, you might want to increase these TTLs and automatically invalidate specific data when you know it has changed. Response caching provides multiple ways to invalidate data. For more details, see [Invalidation](./invalidation). This quickstart uses cache tags, which work like surrogate cache keys for REST APIs. You can tag data in your schema to enable targeted invalidation.
...
For connector sources, use `"sources"` instead of `"subgraphs"` and the `subgraph_name.source_name` format:
...
curl --request POST \
	...
	--header 'content-type: application/json' \
	--url http://localhost:4000/invalidation \
	--data '[{"kind":"cache_tag","sources":["products.my_rest_api"],"cache_tag":"user-42"}]'
```
...
If you execute an example query twice in Apollo Sandbox with [cache debugger](./observability#cache-debugger) enabled you should see all the entries coming from `cache` in `source` column and now you should see the generated cache tags for entities and root field `posts` in `Cache tags` column:

<img
  className="screenshot"
  alt="Response cache debugger example"
  src="../../../../images/response-cache/debugger-cache-query-tags.png"
/>

To explore response caching and see what data has been cached in a query, use the [cache debugger](./observability#cache-debugger).
