...
Th main goal of response caching is to both reduce the load on your subgraphs and also improve overall latencies on your requests from clients. Using the router to cache data is better than using a CDN because it's a GraphQL aware cache. The router caches at the entity/query level which is GraphQL native and reusable across users and requests.
