...
- [**WebAssembly plugins**](/graphos/routing/customization/wasm/)
  - Run portable custom logic inside the router in a capability-restricted sandbox.
  - Explicit permissions and resource limits control what each component can access and how it executes.
...
Apollo Router Core supports WebAssembly plugins and [Rhai scripts](/graphos/routing/customization/rhai/).

Because [Rhai scripts](/graphos/routing/customization/rhai/) are easier to deploy, we recommend using them if they support your use case. Use a WebAssembly plugin for portable, sandboxed request logic that benefits from in-process execution. Use external co-processing if your customization needs to do any of the following (which Rhai scripts and WebAssembly plugins _don't_ support):

- Read or write to disk
- Make network requests
- Use libraries or frameworks that require operating-system or network capabilities unavailable in the Wasm sandbox
...
To learn how to hook into the various lifecycle stages, including example customizations, refer to the [WebAssembly plugins](/graphos/routing/customization/wasm/), [Rhai scripts](/graphos/routing/customization/rhai/), and [external coprocessing](/router/customizations/coprocessor/) docs.
