...
This feature is experimental. Your questions and feedback are highly valued—don't hesitate to get in touch with your Apollo contact. You can also discuss this feature in the [Apollo Community](https://discord.com/invite/apollo).

</ExperimentalFeature>

The Apollo Router includes an experimental diagnostics plugin that collects system and memory information. Use it to diagnose issues such as memory leaks or configuration problems. 

It has minimal overhead and can be enabled in production, but it exposes potentially sensitive information. Enable it only when needed and strictly control access. For example, you can use port-forwarding to access the diagnostics port to avoid exposing it to the network.
...
After startup, the router logs the diagnostics URL, for example:

```
Diagnostics endpoints at http://127.0.0.1:8089/diagnostics
```

The interactive dashboard is at `http://<listen>/diagnostics` (e.g. `http://127.0.0.1:8089/diagnostics`).
...
This endpoint does not include live memory or CPU data. Use it when you need build/version/deployment metadata. It is only available after the router has completed startup.
...
Use this when debugging resource usage or preparing a snapshot for support. The content is time-dependent and changes each time you fetch it.
...
Memory profiling helps diagnose memory leaks. For example, tracking growth over time after schema reload. It is not intended for diagnosing sudden out-of-memory (OOM) events or peak memory usage from normal allocations.
...
The flamegraph view is the easiest to interpret. It shows what has been allocated and not released between the base and actual profiles.
...
The callgraph is harder to interpret than the flamegraph. It can help when a leak has multiple stack paths and the flamegraph does not show a single clear culprit. It shows the percentage of allocation by function. High-memory nodes are the ones to focus on; in a leak scenario they are often highlighted in red.

### Next steps

After isolating the area of a leak, you will need to correlate it with the Router source code and may need to add instrumentation to confirm the finding.
...
The archive may contain sensitive information (for example, if secrets are hardcoded in your config). Always review the contents before sharing the file with anyone.
