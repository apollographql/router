...
To run the native query planner with the best performance and resource utilization, configure your router with the following option:

```yaml title="router.yaml"
experimental_query_planner_mode: new
```

Setting `experimental_query_planner_mode: new` not only enables native query planning, it also disables the V8 JavaScript runtime used by the legacy query planner. Disabling V8 frees up CPU and memory and improves native query planning performance.
