...
- With **Apollo Server 4+**, the [inline trace plugin](/apollo-server/api/plugin/inline-trace) is **not installed by default**. Explicitly add and configure `ApolloServerPluginInlineTrace` in your Apollo Server setup for the subgraph to emit federated trace data.

Also if your subgraph uses the Ariadne Python library, this [demo repository](https://github.com/apollosolutions/ariadne-federated-traces) shows how to enable support for federated tracing.
