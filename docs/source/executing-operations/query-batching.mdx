...
- The router can be configured to support batching for either all subgraphs or each subgraph, but not both.
...
##### Example: federated subgraph batching

This example shows how the router might batch subgraph requests for a federated graph, where the client batch contains a query for an entity.
