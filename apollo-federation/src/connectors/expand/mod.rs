...
mod helpers {
    ...
    impl<'a> Expander<'a> {
        ...
        fn copy_interface_object_keys(
            ...
            // Copy all @key directives from the original schema
            // For @interfaceObject types, mark them as resolvable: false
            // For regular types, keep them resolvable
                ...
                .filter(|d| d.name == self.key_name)
            {
                ...
                // Only set resolvable: false for @interfaceObject types
    ...
    /// Legacy expander for ConnectSpec v0.2 compatibility
    ///
    /// This implementation is frozen and uses the old visitor-based selection walking.
    /// It will be removed when v0.2 support is dropped.
    ...
    impl<'a> LegacyExpander<'a> {
        ...
        /// Build an expanded subgraph for the supplied connector using legacy visitor-based walking
        pub(super) fn expand(
            &self,
            connector: &Connector,
        ) -> Result<FederationSchema, FederationError> {
