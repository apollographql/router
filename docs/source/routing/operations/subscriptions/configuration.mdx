...
If a client attempts to execute a subscription on your router when it's already at `max_opened_subscriptions`, the router rejects the client's request with an error.
