...
With router v0.16.0 and later, and `@apollo/gateway` v2.5.4 and later, you can pass the following headers to return the query plans in the GraphQL response extensions:

- Including the `Apollo-Expose-Query-Plan` header with the following supported options:
  - A value of `true` both returns the query plan in the response and executes it
  - In router v1.61.0 and later, a value of `dry-run` returns the query plan in the response but doesn't execute it
