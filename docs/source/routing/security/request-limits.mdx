...
limits:
  router:
    ...
    http2_max_headers_list_bytes: 48KiB # Default value: 16KiB
    ...
    # Available on all plans (no license required)
    max_recursive_selections: 10000000 # Default value

  # Subgraph response size limits
  subgraph:
    all:
      http_max_response_size: 10MiB # 10MiB for all subgraphs
    subgraphs:
      products:
        http_max_response_size: 20MiB # per-subgraph override (20MiB)

  # Connector response size limits
  connector:
    all:
      http_max_response_size: 5MiB # 5MiB for all connector sources
    sources:
      products.rest:
        http_max_response_size: 10MiB # per-source override (10MiB)
```
...
Limits the number of fields included in an operation, including fields of fragments. Within each selection set, fields with the same name or alias count once. Distinct aliases count separately.

The `GetUser` operation below has height four:
...
Each counted field increments an operation's height by one, regardless of that field's return type (scalar, object, or list).
...
Limits the total number of selections encountered when recursively expanding fragment spreads in an operation. This protects against extremely large or deeply nested operations that consume excessive resources during query planning.

The default value is `10000000` (10 million). This limit is available on all plans and does not require a license.

### Non-local-selections limit

The router estimates the work needed to plan an operation across subgraphs and rejects the operation if that estimate exceeds a limit. The estimate accounts for selections that require the planner to explore fetch options and the possible starting points for those options, so it can exceed the number of fields in the operation.

When the estimate exceeds the limit, the router rejects the operation with a 400 HTTP status code and a GraphQL error with `"extensions": {"code": "QUERY_PLAN_COMPLEXITY_EXCEEDED"}`. The router enforces this limit in [`warn_only` mode](#warn_only-mode) too. This limit is available on all plans and does not require a license.

Unlike the limits above, this one is not a YAML config option. The default is `100000`; set the `APOLLO_ROUTER_SECURITY_NON_LOCAL_SELECTIONS_LIMIT` environment variable to raise it for operations that legitimately exceed the default:

```bash
export APOLLO_ROUTER_SECURITY_NON_LOCAL_SELECTIONS_LIMIT=250000
```

Setting the `APOLLO_ROUTER_DISABLE_SECURITY_NON_LOCAL_SELECTIONS_CHECK` environment variable to `true` disables this check entirely, leaving query planning without this protection.

### `warn_only` mode

In `warn_only` mode, the router executes operations that exceed `max_depth`, `max_height`, `max_aliases`, `max_root_fields`, or `max_recursive_selections` and emits a `WARN` trace with the exceeded limits:
...
If you'd rather not configure `limits` at all yet — or you want to capture the offending operations along with attributes like client name and operation name in your logs or APM — see [Logging operations that exceed a candidate limit](#logging-operations-that-exceed-a-candidate-limit).
...
In [`warn_only` mode](#warn_only-mode), violations of `max_depth`, `max_height`, `max_aliases`, `max_root_fields`, and `max_recursive_selections` allow execution to continue. Other limits, including the [non-local-selections limit](#non-local-selections-limit), still reject operations that exceed them.
...
#### Logging operations that exceed a candidate limit

If you'd like to identify which operations — and which clients — would exceed a limit before enforcing it, you can configure a [custom event](/graphos/reference/router/telemetry/instrumentation/events#custom-events) with a [`gt` condition](/graphos/reference/router/telemetry/instrumentation/conditions#gt) on the `query` selector. The router only logs the event when an operation exceeds the threshold, so you don't need to set `limits` or enable `warn_only` mode.

This approach also lets you attach the client name and version (from the `apollographql-client-name` and `apollographql-client-version` headers) so you can see which clients are sending the offending operations:
...
telemetry:
  instrumentation:
    events:
      supergraph:
        OPERATION_OVER_ALIAS_LIMIT:
          message: "operation exceeded candidate alias limit"
          on: response
          level: warn
          condition:
            gt: # fire only when aliases > 50 — replace with your candidate limit
              - query: aliases
              - 50
          attributes:
            graphql.operation.name: true
...
Repeat the block — with a different event name, selector, and threshold in `condition.gt` — for each of `depth`, `height`, and `root_fields` you want to track. After you're confident in the values, you can switch over to enforcing them with `limits.max_*`.
...
`GET` requests encode the query, variables, and other parameters in the URL's query string instead of a request body. For these requests, the limit applies to the byte length of the encoded query string.

Requests over the limit are rejected with a `413 Payload Too Large` response for `POST` requests, or a `414 URI Too Long` response for `GET` requests.
...
Limits the total size of the header list for your incoming HTTP/2 requests.
The default value is 16KiB.

If the router receives a request whose headers exceed the allowed size, Apollo Router rejects the request with `431 Request Header Fields Too Large`.
...
limits:
  subgraph:
    ...
    subgraphs:
      products:
        http_max_response_size: 20MiB # 20MiB override for 'products'
```
...
This histogram is disabled by default. Once enabled, use it to observe the 95th or 99th percentile of response sizes across your subgraphs, then set `http_max_response_size` above that value with enough headroom for legitimate variation (for example, large page sizes in paginated queries).
...
limits:
  connector:
    ...
    sources:
      products.rest:
        http_max_response_size: 10MiB # 10MiB override for 'products.rest'
```
