...
1. **If you compose your router's supergraph schema with GraphOS** (instead of with the Rover CLI), [update your build pipeline](/graphos/graphs/updating#2-update-your-build-pipeline) to use Apollo Federation 2.4 or later.
    - Each package that makes up Apollo Federation needs to be updated to version 2.4 or later:
      - `@apollo/gateway`
...
Currently, [Apollo Server 4.9 and later](https://github.com/apollographql/apollo-server/releases/tag/%40apollo%2Fserver%404.9.0) supports this protocol. If you're implementing support in a subgraph library, please [create a GitHub discussion](https://github.com/apollographql/router/discussions/).
