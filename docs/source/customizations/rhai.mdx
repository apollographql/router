...
We might introduce breaking changes to Rhai script functionality prior to the full release of the Apollo Router. However, any required updates to your existing scripts will probably be minor.
...
Similar to native Rust plugins, Rhai scripts can hook into the Apollo Router's [four services](./overview/#how-router-customizations-work) that handle requests. _Unlike_ native Rust plugins, Rhai scripts use two different hooks for each service, one for handling request data and one for handling response data:
