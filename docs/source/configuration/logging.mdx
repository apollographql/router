...
subtitle: Logging in the Apollo Router
...
Logging to stdout can be configured to output in different formats.
...
The format can be explicitly set in `router.yaml` with `telemetry.logging.stdout.format`:
