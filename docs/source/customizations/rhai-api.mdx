...
All callback functions registered via `map_request` are passed a `request` object that represents the request sent by the client. These objects provide the following fields, any of which a callback can modify in-place (read-write):

```
request.context
request.headers
request.body (router_service only)
request.body.query (not on router_service)
request.body.operation_name (not on router_service)
request.body.variables (not on router_service)
request.body.extensions (not on router_service)
request.uri.host
request.uri.path
```
...
At the router_service, the `router_body` is simply a String and can be manipulated as such. This is not so convenient as working with GraphQL objects, such as are available at the `supergraph_service`.
...
response.context
response.headers
response.body (router_service only)
response.body.label (not on router_service)
response.body.data (not on router_service)
response.body.errors (not on router_service)
response.body.extensions (not on router_service)
```
