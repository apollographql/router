...
limits:
  ...
  connector:
    all:
      ...
      max_mapping_errors: 100 # at most 100 `->withError` errors per response
    sources:
      products.rest:
...
## Connector mapping error limits

### `max_mapping_errors`

Limits how many errors a connector's response mapping may report in the response's `extensions.connectorErrors` using the [`->withError` method](/graphos/connectors/responses/error-handling#report-errors-without-failing-a-field).

A `->withError` call inside a `->map` records one error per element, so a mapping over a large API response can contribute one error per row. This limit caps that contribution.

There is no default limit; every declared error is reported unless you configure this option, matching how the router passes through subgraph errors.

Identify sources by `subgraph_name.source_name`, the same key format used by `http_max_response_size`. You can set a global default and optionally override it per source:
...
limits:
  connector:
    ...
    sources:
      products.rest:
        max_mapping_errors: 20 # override for 'products.rest'
```

The per-source entry under `sources` takes precedence over `all`.

Errors beyond the limit are replaced by a single summary error carrying the `CONNECTORS_TOO_MANY_ERRORS` code and stating how many were dropped, so a truncated list is visible in the response rather than silent.

This limit applies only to errors the mapping declares with `->withError`. Mapping *problems* — the mapping language's own diagnostics, such as a path that matched nothing — are never sent to clients and are unaffected.

### Monitoring exceeded limits

When a connector response's mapping errors are truncated, the router increments the `apollo.router.limits.connector_mapping_errors.exceeded` counter with a `connector.source` attribute identifying the affected source.
