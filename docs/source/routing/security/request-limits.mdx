...
server:
  http:
    ...
    max:
      ...
      # Primarily affects HTTP/1.1 connections  
      headers: 250
    ...
    # Header read timeout (applies to all HTTP versions)
...
- Set per-header size limits with `max.header_size` (similar to Node.js `--max-http-header-size`)
- Control header count limits with `max.headers` for HTTP/1.1
- Set total header payload limits with `max.header_list_size` for HTTP/2
- Configure header read timeouts consistently across all HTTP versions

#### Deprecated Field Names

For backward compatibility, the old flat field names are still supported but deprecated:

```yaml title="router.yaml (deprecated)"
server:
  http:
...
The new nested structure takes precedence when both are specified.
...
The Apollo Router enforces header limits at the HTTP server level using both the new `server.http.*` configuration and legacy `limits.http1_*` settings. These limits are protocol-specific and enforced early in the request processing pipeline for optimal performance.
