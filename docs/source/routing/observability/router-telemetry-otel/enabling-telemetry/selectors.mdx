...
| `response_cache_control`    | No          | `max_age` \| `public`\| `private`\| `no_store`    | Provides data from the computed `Cache-Control` header, such as `max_age` or scope (`public`/`private`).  |
