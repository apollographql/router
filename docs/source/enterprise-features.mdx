...
1. Configure the router to use a local supergraph schema by setting either the [`--s/-supergraph` CLI option](./configuration/overview/#-s----supergraph) or the [`APOLLO_SUPERGRAPH_PATH` environment variable](). 

1. (**Recommended**) Configure the router to report usage metrics to GraphOS in a best-effort basis by setting the 
