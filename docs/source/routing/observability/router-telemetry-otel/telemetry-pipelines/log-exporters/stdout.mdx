...
| `expand_json_string_values`| `true`\|`false` | `false` |               | When `true`, string attribute values that are valid JSON objects or arrays are emitted as native JSON rather than quoted strings. Useful for log aggregators (e.g. Splunk) that index nested fields. |
