...
Cached query plans are not invalidated on schema refresh, which can quickly fill a distributed cache when combined with [cache-warm up](/router/configuration/in-memory-caching#cache-warm-up) and frequent schema publishes. 

Test your cache configuration with expected queries and consider decreasing the [TTL](/router/configuration/in-memory-caching#ttl) to prevent cache overflow.
