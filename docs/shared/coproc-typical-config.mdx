...
coprocessor:
  ...
  http_service: # This coprocessor hooks into the HTTP layer (before RouterService)
    request: # HttpRequest stage: when the router receives the raw HTTP request
      headers: true
      body: false
      context: false
      sdl: false
      path: false
      method: false
    response: # HttpResponse stage: when the router is about to send the raw HTTP response
      headers: true
      body: false
      context: false
      sdl: false
