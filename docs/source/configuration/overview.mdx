...
Apollo's legacy reporting doesn't include data about enum values and input object fields, meaning you can't view enum and input object field usage in GraphOS Studio.
Legacy reporting also causes some [operation checks to flag non-issues](#enhanced-operation-checks).
...
Once you enable extended reference reporting, you can view enum and input object field usage along with the other types of [field usage in GraphOS Studio](/graphos/metrics/field-usage) for all operations that occur after you've enabled the configuration.

Enabling this configuration also automatically enables [enhanced operation checks](#enhanced-operation-checks), though you won't see an immediate change in your operations check behavior. This is because operation checks rely on historical client operation data.
To ensure sufficient data to distinguish between genuinely unused values and those simply not reported in legacy data, enhanced checks require some historical operations with extended reference reporting enabled.

#### Enhanced operation checks

Thanks to extended reference reporting, enhanced operation checks can more accurately flag issues with enum values and input object fields. See the comparison table below for differences between standard operation checks that come with legacy reference reporting and enhanced checks that are automatically enabled with extended reference reporting.
...
##### Default argument changes or removals for input object fields

</td>

<td>In general, changing or removing a default argument is considered a breaking change, but changing or removing default values in input object fields isn't.</td>
<td>Changing or removing default values in input object fields is considered a breaking change.</td>
</tr>
<tr>
<td>

##### Nullable input object field removals
</td>
<td>Removing a nullable input object field is always considered a breaking change.</td>
<td>Removing a nullable input object field is only considered a breaking change if the nullable field is present in historical operations. If the nullable field was always omitted in historical operations, its removal isn't considered a breaking change.</td>
</tr>
<tr>
<td>
