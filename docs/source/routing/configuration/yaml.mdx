...
Query validation errors might reveal information about your schema structure, such as field names, types, and required arguments. Although this is not typically a significant security concern (especially with persisted queries), you might want to redact these error details in production environments.

The router returns detailed validation error messages by default. To redact validation errors and return a generic error message, set the `supergraph.redact_query_validation_errors` flag:
...
When you enable this setting, Apollo Router replaces any query validation errors with a single generic error:
