...
<Note>
  Where caching sits in the request pipeline determines what still runs on a cache hit:

  - **Subgraph responses** are cached at the subgraph service level, right after coprocessor and Rhai plugins, so the
    coprocessor and Rhai subgraph stages still run on a cache hit (both at request and response level).
  - **Connector root fields** are cached at the connector HTTP request level, so the coprocessor's
    `connector_request`/`connector_response` stages still run on a cache hit. A cached response is replayed as an
    ordinary connector response, carrying the status the API returned when the entry was stored and its cache-control
    headers, so your coprocessor does not need to distinguish a cache hit from a live call. Response headers other than
    cache-control are not stored, so they are absent on a hit.
  - **Connector entity fetches** are cached above `make_requests`, where one GraphQL fetch fans out into several HTTP
    requests. A fully cached entity fetch therefore performs no connector requests at all, and runs none of the layers
    attached to them: no coprocessor stages, and no connector-level telemetry, header propagation, traffic shaping or
    request limits. When only some entities are cached, those layers run for the representations that missed.

  Rhai has no connector hook at all: connector requests and responses cannot be customized with Rhai, cached or not.
</Note>
...
For connector sources, configure `private_id` in the `connector` block:
...
response_cache:
  ...
  connector:
    all:
      enabled: true
      cache_key_headers: [] # Required: request headers to include in the cache key ([] = none)
      redis:
        urls: ["redis://..."]
    sources:
      "accounts.rest_api":
...
- An `all` field, which affects all subgraph and connector requests
- A `subgraphs` field, which contains an object with per-subgraph customization
- A `connectors` field, which contains an object with per-connector-source customization, keyed by `subgraph_name.source_name`
...
### Connector cache-key headers

For Apollo Connectors, the set of client request headers that are folded into the cache key is an **explicit, required** configuration: `cache_key_headers`. Because a connector calls your REST API directly, the router cannot know which request headers cause your API to return a different response—so you must declare them. Set it on `connector.all`: the router refuses to start if connector caching is enabled and `all.cache_key_headers` is unset, so you cannot enable connector caching without first deciding which headers matter. A source listed under `connector.sources` inherits `all`'s value, or overrides it (including with an explicit empty list) by setting its own.
...
response_cache:
  ...
  connector:
    all:
      ...
      # REQUIRED: the client request headers whose values distinguish cache entries.
      cache_key_headers:
        - x-user-id
        - x-tenant
    sources:
      products.catalog_api:
        # Per-source override; replaces (does not merge with) the `all` list for this source.
        cache_key_headers:
          - x-region
```
...
- Each listed header's values (matched case-insensitively; all values of a repeated header, in sorted order) become part of the cache key, so two requests that differ only in a listed header get separate cache entries.
- A header that is **not** listed does not affect the cache key—two requests that differ only in an unlisted header share one cache entry.
- An empty list (`cache_key_headers: []`) is a valid, explicit choice meaning "no request header distinguishes cache entries."

<Caution>

Choosing this list is a safety decision, and both directions have consequences:

- **Omitting a header your API varies on is a data-disclosure risk.** If your REST API returns different data based on a header (for example, a per-user identity header) and you do not list it, one caller's cached response is served to another caller who sent a different value. List every request header that changes what your API returns—including any header you forward to the API via `@connect`'s `http.headers`.
- **Listing a header that doesn't affect the response wastes cache.** Each distinct value of a listed header creates a separate entry, so including a high-cardinality header that has no effect on the response (for example, a per-request trace or correlation ID) fragments the cache and drives the hit rate toward zero.

The goal is to list **exactly** the headers whose values change your API's response—no more, no fewer.
...
### Per-source Redis instances (connectors)

Configure a global Redis instance for connectors and override it for specific connector sources:
...
response_cache:
  ...
  connector:
    ...
    sources:
      "products.rest_api":
        redis:
          urls: ["redis://products-cache:6379"]
          pool_size: 15
          namespace: products_connector_cache
      "inventory.stock_api":
        enabled: false # Disable caching for this source
```
...
Nest this `tls` configuration under `response_cache.subgraph.all` for subgraphs or `response_cache.connector.all` for Apollo Connectors — both use the same Redis configuration type, TLS included.
...
For connector sources, configure TTL with `connector.all.ttl` or per source. Connectors respect `Cache-Control` headers from the upstream HTTP response directly—the configured TTL is used as a fallback when no header is present:
