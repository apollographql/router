...
You can enable the sandbox by editing your configuration file. Make sure introspection is enabled as well:
...
sandbox:
  ...
  path: /sandbox
  enabled: true
# make sure you enable introspection as well!
supergraph:
  introspection: true
```

The sandbox default path is `/`, which will conflict with the homepage if both are enabled.
You can work around this by either disabling the homepage...:
...
...or changing the sandbox port or path:
