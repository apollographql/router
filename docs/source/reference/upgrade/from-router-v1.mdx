...
As we renamed several context keys it could impact your coprocessor logic and you might need to update the codebase. In order to smoothly migrate to the new context keys we changed the `context` configuration on coprocessors.
So by default if you didn't specify the `context` configuration nothing will change. But you can now specify either if you have to still use the deprecated names (`deprecated`) or the new ones (`all`).
We also added a new configuration, if you specify an array in `context` you'll have to specify only the context keys you want to send to your coprocessors. It will lower the body size of the request/response and then improve performances.
...
So if you don't want to change anything in your coprocessor you can switch `context: true` to `context: deprecated`. And if it was `context: false` it will be `context: none`.
