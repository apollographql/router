...
These error codes are defined in the [GraphQL spec](https://spec.graphql.org/October2021/) and can appear in client responses under `errors[].extensions.code`, which is an established convention found in [GraphQL error extensions](https://spec.graphql.org/October2021/#sec-Errors.Error-result-format). Because these error codes are part of the core spec, they cannot be disabled directly as they are part of the core Router implementation. Only Apollo Router-specific features or subgraph errors can be disabled.

Changing these errors makes your router non-compliant with the GraphQL spec. To edit the response, you must use an external proxy.
...
These errors are part of Apollo Router and not the core GraphQL spec, so they can be suppressed or disabled with [router customizations](/graphos/routing/customization/overview).
