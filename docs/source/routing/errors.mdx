...
<Property name="QUERY_PLAN_COMPLEXITY_EXCEEDED">

The router stopped planning because the operation exceeded a query-planning complexity limit. The response has a 400 HTTP status code. For the non-local-selections limit, see [Non-local-selections limit](/graphos/routing/security/request-limits#non-local-selections-limit).
...
<Property name="SUBGRAPH_COST_ESTIMATED_TOO_EXPENSIVE">

The estimated cost of the query directed at a specific subgraph exceeds the configured maximum cost.
...
<Property name="VALIDATION_INVALID_TYPE_VARIABLE">

One or more operation variables have runtime values that don't match their defined GraphQL types.
