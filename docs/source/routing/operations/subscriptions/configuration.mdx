...
**For cloud routers**, subscriptions are available on all plans:
...
**For self-hosted routers**, subscriptions require an Enterprise plan (see [Prerequisites](#prerequisites) below).
...
1. **For self-hosted routers only**: Make sure your router is [connected to a GraphOS Enterprise organization](/router/enterprise-features/#enabling-enterprise-features).
    - Subscription support is an Enterprise feature of self-hosted routers.
    - Cloud routers support subscriptions on all plans.
