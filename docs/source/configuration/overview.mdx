...
Apollo's legacy operation signature algorithm removed information about non-standard object fields such as input, enum, union, and interface types. Because usage reporting relies on operation signatures, usage reports did not include these types.

Beginning in v1.49.0, the router supports enhanced operation signature normalization, including aliases and the full normalized form of input objects and enum references.

Configure it in `router.yaml` with the `telemetry.apollo.experimental_apollo_signature_normalization_algorithm` option:
...
Once you enable this configuration, you can ____ in GraphOS Studio.

<Note>

Once you enable `enhanced` signature normalization, there will be a lag in changes to metrics in Studio.
This is because historical operations still use the `legacy` operation signature normalization algorithm.

</Note>

<MinVersion version="1.49.0">

### Enhanced reference reporting

</MinVersion>

Apollo's legacy reporting didn't include input and enum references.
Beginning in v1.49.0, the router supports reporting these reference types.

Configure it in `router.yaml` with the `telemetry.apollo.experimental_apollo_metrics_reference_mode` option:

```yaml title="router.yaml"
telemetry: 
  apollo:
    experimental_apollo_metrics_reference_mode: enhanced # Default is legacy
```
...
Once you enable `enhanced` reference reporting, there will be a lag in changes to metrics in Studio.
This is because historical operations still use the `legacy` reference reporting.
