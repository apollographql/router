...
fn test_empty() -> Result<(), Failed> {
    ...
    let service_name = resource
        .get(&Key::new(opentelemetry_semantic_conventions::resource::SERVICE_NAME))
        .unwrap();
    assert!(
        service_name
            .as_str()
    ...
    assert!(
        resource
            .get(&Key::new(opentelemetry_semantic_conventions::resource::SERVICE_NAMESPACE))
            .is_none()
    );
    assert_eq!(
        resource.get(&Key::new(opentelemetry_semantic_conventions::resource::SERVICE_VERSION)),
        Some(std::env!("CARGO_PKG_VERSION").into())
    );

    assert!(
        resource
            .get(&Key::new(opentelemetry_semantic_conventions::resource::PROCESS_EXECUTABLE_NAME))
...
fn test_config_resources() -> Result<(), Failed> {
    ...
    let resource = test_config.to_resource();
    assert_eq!(
        resource.get(&Key::new(opentelemetry_semantic_conventions::resource::SERVICE_NAME)),
        Some("override-service-name".into())
    );
    assert_eq!(
        resource.get(&Key::new(opentelemetry_semantic_conventions::resource::SERVICE_NAMESPACE)),
