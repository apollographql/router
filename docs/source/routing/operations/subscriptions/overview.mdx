...
The router sends subscription events to clients over multipart HTTP. It can receive the initial event from a subgraph over WebSocket or HTTP callbacks, or it can consume an entity event directly from a supported event provider. With subscription support enabled, you can add `Subscription` fields to your subgraph schemas:
...
- Use [event-stream-backed subscriptions](/graphos/routing/operations/subscriptions/event-streams/overview) to consume entity events directly from NATS, Kafka, or Redis.
