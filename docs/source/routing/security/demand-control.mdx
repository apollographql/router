...
When a field returns an object that contains one or more list fields, use the `sizedFields` argument to specify which nested fields use this directive's list size for cost. Only selections that match those paths are scaled by the size. Other selections are not.
