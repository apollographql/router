...
The TLS handshake timeout is the amount of time the Router will wait to complete the TLS handshake with a client before timing out.

By default, the TLS handshake timeout is set to 10 seconds. A longer timeout can be configured using the `server.http.tls_handshake_timeout` configuration option.
