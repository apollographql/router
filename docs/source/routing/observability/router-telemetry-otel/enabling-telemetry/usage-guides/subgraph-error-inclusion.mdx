...
## Connector mapping errors

A connector response mapping can report an error without failing the field, using the [`->withConnectorError` method](/graphos/connectors/responses/error-handling#report-errors-without-failing-a-field). Those errors are written by the author of the connector's subgraph schema and can interpolate data from the API's response, so they are governed by this configuration too, under the name of the subgraph the connector belongs to.

They are reported in the response's `extensions.connectorErrors` rather than in `errors`, so the rules apply slightly differently:

*   If a subgraph's errors are **fully redacted** (its config is `false`, which is also the default), its connectors' mapping errors are **omitted entirely**. There is no `Subgraph errors redacted` placeholder in `extensions.connectorErrors`, because such a row would tell a client nothing but that something was withheld.
*   Otherwise, `redact_message`, `allow_extensions_keys`, `deny_extensions_keys` and the `service` extension behave exactly as they do for errors in the `errors` array.
