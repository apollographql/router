...
**For self-hosted routers, subscription support is an [Enterprise feature](/router/enterprise-features/).**
...
1. Update your router instances to version `1.22.0` or later. [Download the latest version.](/router/quickstart/)
...
By default, if you've configured your router to [propagate](/router/configuration/header-propagation/) HTTP `Authorization` headers to your subgraph, then the router automatically sets corresponding `connectionParams` when initiating a WebSocket connection to that subgraph.
...
To specify a custom payload for  the`connection_init` message, you can write a [Rhai script](/router/customizations/rhai/) and use the `context` directly:
