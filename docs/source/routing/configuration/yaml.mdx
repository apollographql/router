...
This option is not GraphQL spec-compliant. It changes how errors are associated with paths in the response compared to the behavior described in the [GraphQL specification](https://spec.graphql.org/). Use this feature for your subgraphs only when the reduction in error volume outweighs strict spec compliance.
