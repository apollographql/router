         ...
         jwks: # This key is required.
           - url: https://dev-zzp5enui.us.auth0.com/.well-known/jwks.json
             issuers: # optional list of issuers
               - https://issuer.one
               - https://issuer.two
             allow_missing_exp: true # optional, allow tokens without exp claim
             poll_interval: 60s # optional, how often to refresh the JWKS
...
2. Pass all of the following to the `router` executable on startup:
   ...
   - A [graph API key](/graphos/platform/access-management/api-keys/graph-api-keys) that enables the router to authenticate with GraphOS to fetch its supergraph schema (via the `APOLLO_KEY` environment variable)
...
- `allow_missing_exp`: **optional** boolean that allows tokens without an `exp` (expiration) claim for this JWKS. Tokens that include an `exp` claim are still validated normally and rejected if expired. When enabled, it's strongly recommended to also configure non-empty `issuers` and/or `audiences` to restrict which tokens are accepted. The default value is `false`.
