...
limits:
  ...
  http_max_header_size: 8192 # Default value: unlimited
  http_max_header_list_items: 10 # Default value: unlimited
...
Limits the maximum size of individual HTTP headers (header name + header value combined).
If a header exceeds this size limit, the router responds with `431 Request Header Fields Too Large`.
...
Limits the maximum number of items in comma-separated HTTP header values.
For example, a header like `Accept: text/html, application/json, */*` contains 3 items.
If a header value contains more items than this limit, the router responds with `431 Request Header Fields Too Large`.
...
This limit is useful for preventing abuse of headers that accept multiple values, such as `Accept`, `Accept-Encoding`, or custom headers that use comma-separated lists.
