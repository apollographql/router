...
> ⚠️ Under all circumstances, it is important that subgraphs are only accessible via the router, and not directly. This is especially true if you are relying on JWT authentication in the router. Be sure to [secure your subgraphs](/federation/building-supergraphs/subgraphs-overview/#securing-your-subgraphs).
...
> ⚠️ This example forwards JWT claims via headers. When combined with [header propagation](./header-propagation/) this can introduce a security risk. If you are relying on header-based forwarding, **be sure not to propagate matching headers**.
