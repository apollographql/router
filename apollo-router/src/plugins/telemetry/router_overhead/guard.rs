...
impl SubgraphRequestGuard {
    pub(super) fn new(inner: Arc<Mutex<TrackerInner>>) -> Self {
        ...
        let prev_count = inner_lock.active_count.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
...
impl Drop for SubgraphRequestGuard {
    fn drop(&mut self) {
        ...
        let prev_count = inner.active_count.fetch_sub(1, std::sync::atomic::Ordering::SeqCst);
