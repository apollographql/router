---
title: Configuring the GraphOS Router
subtitle: Configure with environment variables, command-line options, and YAML
...
Learn how to customize the behavior of your GraphOS Router with environment variables, command-line commands and options, and YAML file configuration.

## Environment variables

If you're using the GraphOS Router with [managed federation](/federation/managed-federation/overview/) and GraphOS, set these environment variables in the startup command:
...
The GraphOS Router's [supergraph schema](/federation/federated-types/overview/#supergraph-schema). Specified by absolute or relative path (`-s` / `--supergraph <supergraph_path>`, or `APOLLO_ROUTER_SUPERGRAPH_PATH`), or a comma-separated list of URLs (`APOLLO_ROUTER_SUPERGRAPH_URLS`).
...
If set, the GraphOS Router runs in dev mode to help with local development.
...
The GraphOS Router provides a set of subcommands for interacting with its configuration. You run these subcommands with the following syntax:
...
Takes a config file created for a _previous_ version of the GraphOS Router and outputs the corresponding configuration for the _current_ version.
...
The GraphOS Router takes an optional YAML configuration file as input via the [`--config`](#-c----config) option:
...
By default, the GraphOS Router starts an HTTP server that listens on `127.0.0.1:4000`. You can specify a different address by setting `supergraph.listen`:
...
- To configure logging, see [Logging in the GraphOS Router](./telemetry/exporters/logging/overview).

- To configure the inclusion of subgraph errors, see [Subgraph error inclusion](./subgraph-error-inclusion).

### Landing pages

The GraphOS Router can serve any of the following landing pages to browsers that visit its [endpoint path](#endpoint-path):
...
By default, the GraphOS Router stores the following data in its in-memory cache to improve performance:

- Generated query plans
- Automatic persisted queries (APQ)
- Introspection responses

You can configure certain caching behaviors for generated query plans and APQ (but not introspection responses). For details, see [In-Memory Caching in the GraphOS Router](./in-memory-caching/).
...
To configure the shape of traffic between clients, routers, and subgraphs, see [Traffic shaping in the GraphOS Router](./traffic-shaping).

### Cross-Origin Resource Sharing (CORS)

See [Configuring CORS in the GraphOS Router](./cors).

### Defer support

See [GraphOS Router support for `@defer`](../executing-operations/defer-support/#disabling-defer).

### Query batching support

See [GraphOS Router's _experimental_ support for query batching](../executing-operations/query-batching).
...
To configure CSRF prevention, see [CSRF prevention in the GraphOS Router](./csrf).
...
The GraphOS Router supports standard and custom instrumentation to collect telemetry data from its request and response processing pipeline to produce logs, metrics and traces to export.

See the [GraphOS Router telemetry overview](./telemetry/overview).
...
In versions of the GraphOS Router prior to 1.17, this limit was defined via the config option `experimental_parser_recursion_limit`.

</Note>

### Demand control

See [Demand Control](../executing-operations/demand-control) to learn how to analyze the cost of queries and how to reject requests with queries that exceed customizable cost limits. 
...
Additionally, since v1.43.1, the Apollo Router Core can show a log when it detects that the client canceled the request. This log can be activated with:
...
You can customize the GraphOS Router's behavior with [plugins](../customizations/overview). Each plugin can have its own section in the configuration file with arbitrary values:
...
Currently, the GraphOS Router supports expansion of environment variables and file paths. Corresponding variables are prefixed with `env.` and `file.`, respectively.
...
By default, the GraphOS Router will attempt to reuse fragments from the original query while forming subgraph requests. This behavior can be disabled by setting the option to `false`:
...
The GraphOS Router can generate a JSON schema for config validation in your text editor. This schema helps you format the YAML file correctly and also provides content assist.
...
New releases of the GraphOS Router might introduce breaking changes to the [YAML config file's](#yaml-config-file) expected format, usually to extend existing functionality or improve usability.
