---
title: Health Checks in the GraphOS Router
subtitle: Determining the router's status
description: Learn how to run health checks to determine whether an GraphOS Router is available and ready to start serving traffic.
---
...
The GraphOS Router supports a basic HTTP-level health check. This is enabled by default and is served on port `8088` at the URL path `/health`. This returns a `200` status code if the HTTP server is successfully serving.
...
The following example demonstrates using the `curl` command to send a basic health check query to an GraphOS Router instance running at `127.0.0.1:4000`:
