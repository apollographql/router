...
If your organization uses a corporate proxy that performs TLS inspection (also known as SSL inspection or HTTPS interception), add the proxy's root certificate to your router container. Without this certificate, the router cannot establish secure connections to GraphOS or your subgraphs.
...
Alternatively, build a custom Docker image with the certificate included as described in [Option 2](#option-2-build-a-custom-image-with-the-certificate) and reference it in your Helm values:
...
To verify the certificate is correctly installed, check the container's CA store:
