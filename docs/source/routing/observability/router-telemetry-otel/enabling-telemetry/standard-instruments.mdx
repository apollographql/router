...
- `apollo.router.query_planning.warmup.operations` - Counter of operations processed during query planner warm-up. Together with `apollo.router.query_planning.warmup.operations.expected`, this lets you compute warm-up coverage (planned / expected). Attributes:
  - `outcome`: The result of warming up the operation. Shares the `apollo.router.query_planning.plan.duration` values (`success`, `timeout`, `cancelled`, `error`), plus `memory_limit` for memory-limit cancellations and `reused` for operations whose plan was carried over from the previous cache (or already present) rather than planned fresh.
  - `source`: Where the operation came from (`persisted_query` for the persisted-query manifest, `cache` for operations carried over from the previous in-memory cache)
- `apollo.router.query_planning.warmup.operations.expected` - Counter of operations that warm-up intended to plan. Attributes:
  - `source`: Where the operation came from (`persisted_query`, `cache`)
- `apollo.router.query_planning.warmup.backpressure` - Counter of the number of times warm-up retried after a temporary compute-backpressure error. Attributes:
  - `source`: Where the operation came from (`persisted_query`, `cache`)
  - `phase`: The warm-up phase where backpressure occurred (`parse` for document parsing, `plan` for the planner call)
- `apollo.router.query_planning.plan.duration` - Histogram of plan durations isolated to query planning time only. Attributes:
  - `planner`: The query planner implementation that produced the plan (for example, `rust`)
  - `outcome`: The result of planning the operation (`success`, `timeout`, `cancelled`, `error`)
  - `job.type`: The compute-job type that produced the plan (`query_planning` for a regular request, `query_planning_warmup` for query planner warm-up), matching the `job.type` attribute on `apollo.router.compute_jobs.duration`
      ...
      router:
        apollo.router.operations.subscriptions.terminated.client:
          attributes:
            ...
            client.name: true
```
- `apollo.router.operations.subscriptions.terminated.subgraph` - Number of subgraph WebSocket subscriptions ended by the subgraph closing the connection. Incremented once per subscription that terminates because of a subgraph-side closure. When `max_reconnect_attempts > 0`, intermediate closures that are recovered by a successful reconnect are not counted — only the final closure that ends the subscription (either a normal server-sent `complete` or the closure after all reconnect attempts are exhausted) increments this counter. Each deduplicated client sharing that subscription will also emit a corresponding `terminated.client` event with `reason=server_close`. Attributes:
...
### Conventional GraphQL subscription metrics (opt-in, draft)

The router can additionally emit two metrics under OpenTelemetry's own [GraphQL semantic convention](https://github.com/open-telemetry/semantic-conventions/pull/3515). That convention is an open pull request at `development` stability. Its names, and therefore these metrics, may still change before it's ratified.

Set the `OTEL_SEMCONV_STABILITY_OPT_IN` environment variable to `graphql` or `graphql/dup` to turn the metrics on. The router treats both tokens the same way: it has no existing metric that either replaces or duplicates, so there is nothing for `/dup` to do differently. The variable is a comma-separated list, so it composes with the other domains OpenTelemetry defines for it, for example `OTEL_SEMCONV_STABILITY_OPT_IN=http,graphql`. The router recognizes only the `graphql` domain; other domains are ignored. Leaving the variable unset turns both metrics off; the router's existing subscription metrics are unaffected either way.

- `graphql.server.subscription.event_count` - A counter of subscription events processed by the router, including events whose processing ended with an error. Attributes:
  - `error.type`: The error code of the event's first GraphQL error. Absent when the event carried no error, and absent for the router's own termination notices (`SUBSCRIPTION_SCHEMA_RELOAD`, `SUBSCRIPTION_CONFIG_RELOAD`, `SUBSCRIPTION_MAX_LIFETIME_EXCEEDED`, `SUBSCRIPTION_JWT_EXPIRED`): those reach the client as GraphQL errors, but the router ended the subscription on purpose rather than failing to process an event. Use `apollo.router.operations.subscriptions.terminated.client` and its `reason` attribute to observe them.
- `graphql.server.subscription.event.duration` - A histogram, in seconds, of the time from when the router received a subscription event from its source to when it wrote the corresponding response to the subscriber's stream. Recorded for events received from a subgraph or callback endpoint, including transport errors and the notice sent after reconnection attempts are exhausted. The router's own termination notices (JWT expiry, max subscription lifetime, schema reload, config reload) count toward `event_count` but carry no duration, because the router generates them itself rather than receiving them from a source. Attributes:
  - `error.type`: The error code of the event's first GraphQL error. Absent when the event carried no error, and absent for the router's own termination notices (`SUBSCRIPTION_SCHEMA_RELOAD`, `SUBSCRIPTION_CONFIG_RELOAD`, `SUBSCRIPTION_MAX_LIFETIME_EXCEEDED`, `SUBSCRIPTION_JWT_EXPIRED`): those reach the client as GraphQL errors, but the router ended the subscription on purpose rather than failing to process an event. Use `apollo.router.operations.subscriptions.terminated.client` and its `reason` attribute to observe them.

Both metrics omit `graphql.document.*` and `graphql.operation.name`, which the convention marks opt-in for cardinality reasons the router has no configuration to bound yet.
...
- `apollo.router.limits.subgraph_response_size.exceeded`: A counter of subgraph responses aborted because their body exceeded the configured `http_max_response_size` limit under `limits.subgraph`.
  - `subgraph.name`: The name of the subgraph whose response was aborted.

- `apollo.router.limits.connector_response_size.exceeded` - A counter of connector responses aborted because their body exceeded the configured `http_max_response_size` limit under `limits.connector`.
