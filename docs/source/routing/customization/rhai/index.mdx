...
### Propagating extensions from subgraph responses

By default, the router doesn't propagate the root-level `extensions` field from subgraph responses to the final client response. This is because merging extensions from multiple subgraphs can lead to conflicts when different subgraphs use the same extension keys.

However, you can use Rhai to propagate extensions from subgraph responses to the client response. This example shows how to safely collect extensions from subgraph responses and merge them into the final response.

<Note>

**Important considerations:**

- **Race conditions**: Subgraph requests execute in parallel, so their responses may arrive in any order. Use defensive coding to handle concurrent updates.
- **Key conflicts**: Different subgraphs may use the same extension keys. The last subgraph response to complete will overwrite earlier values for conflicting keys.
- **Data types**: Extension values can be any valid JSON type (objects, arrays, strings, numbers, booleans).
...
fn supergraph_service(service) {
  let f = |response| {
    // Check if we collected any extensions from subgraphs
    // The safe navigation operator (?.) returns () if the key doesn't exist
    if response.context["extensions"]?.len > 0 {
      // Merge all collected extensions into the response body
...
fn subgraph_service(service, subgraph) {
    let f = |response| {
      // Check if this subgraph response includes extensions
      if response.body.extensions != () {
        // Initialize the extensions array in context if it doesn't exist yet
        if response.context["extensions"] == () {
          response.context.extensions = [];
        }

        // Store this subgraph's extensions for later merging
        // Note: Due to parallel execution, the order of these additions
        // is non-deterministic. If multiple subgraphs use the same
        // extension keys, the last one to complete will win.
...
1. The `subgraph_service` hook intercepts each subgraph response and checks for the presence of `extensions`.
2. If extensions are found, they're stored in the request context (which is shared across all services).
3. The `supergraph_service` hook runs after all subgraph responses are collected and merged.
4. It retrieves all stored extensions from the context and adds them to the final response's `extensions` field.

**Customization options:**

If you need more control over extension merging (for example, to handle key conflicts differently or to filter specific extensions), you can modify the script:
...
fn supergraph_service(service) {
  let f = |response| {
    if response.context["extensions"]?.len > 0 {
      ...
      // Merge extensions with custom logic
      for extension_map in response.context["extensions"] {
        for key in extension_map.keys() {
          // Example: Prefix keys with subgraph name to avoid conflicts
          // Or: Implement your own conflict resolution logic here
          response.body.extensions[key] = extension_map[key];
        }
      }
    }
  };
...
fn subgraph_service(service, subgraph) {
    let f = |response| {
      if response.body.extensions != () {
        ...
        // Store the subgraph name with the extensions if needed
        // for conflict resolution or debugging
        response.context.extensions += response.body.extensions;
      }
    };
