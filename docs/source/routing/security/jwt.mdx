         ...
         jwks: # This key is required.
           - url: https://dev-zzp5enui.us.auth0.com/.well-known/jwks.json
             ...
             headers: # optional list of static headers added to the HTTP request to the JWKS URL
               - name: User-Agent
                 value: router
             retry: # optional retry configuration for JWKS URL fetching
               attempts: 3
               backoff:
                 initial: 100ms
                 max: 5s
...
- `retry`: **optional** retry configuration for JWKS URL fetching. If not specified, failed requests will only retry at the next polling interval.
  - `attempts`: Total number of attempts including the initial request (default: `3`)
  - `backoff`: Exponential backoff configuration for delays between retries
    - `initial`: Initial delay before first retry (default: `100ms`)
    - `max`: Maximum delay between retries (default: `5s`)
