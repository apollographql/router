...
### When to use private data caching
...
To set up private information caching, configure the `private_id` option. `private_id` is a string pointing at a field in the request context that contains data used to recognize users (for example, user id, or `sub` claim in JWT).
...
The router provides several Redis options to ensure you can scale properly and achieve the best performance.
...
preview_response_cache:
  ...
  subgraph:
    all:
      ...
      redis:
        urls: ["redis://..."]
        fetch_timeout: 750ms # Optional, by default: 500ms
