...
Best practice: Follow the [OpenTelemetry semantic conventions](https://opentelemetry.io/docs/specs/semconv/general/metrics/#instrument-units) and use **seconds (`s`)** as the unit for duration measurements whenever possible.

Only use non-second units (`ms`, `us`, `ns`) when integrating with an observability platform that **requires** a specific unit and does not properly translate time units automatically.
