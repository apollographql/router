...
- Configure a TTL either globally under `subgraph.all.ttl` or for each individual subgraph. The router requires a configured TTL and fails to start without one. This TTL acts as a fallback. When a subgraph response includes a `Cache-Control` header with `max-age`, that value takes precedence.
