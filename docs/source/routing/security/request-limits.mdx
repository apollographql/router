...
Before setting a limit, consider the largest legitimate response your subgraphs can return (for example, paginated list queries with large page sizes). Set the limit above that ceiling with enough headroom for normal variation.
