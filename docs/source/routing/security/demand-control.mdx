...
The `actual_cost_mode` option enables you to choose how the actual cost of an operation is calculated.
...
- `by_response_shape` uses only the final shape of the response. It doesn't consider the intermediate work required to
  ...
  response. This behavior was the only option available in Router versions v2.11.0 and earlier.
