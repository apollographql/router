...
impl<'schema> GraphQLString<'schema> {
    ...
    pub fn new(value: &'schema Node<Value>, sources: &'schema SourceMap) -> Option<Self> {
        ...
        let first_line_of_value = value_without_quotes.lines().next()?;
        let start_of_value = value_with_quotes.find(first_line_of_value)?;
        let last_line_of_value = value_without_quotes.lines().last()?;
        let end_of_value = value_with_quotes.rfind(last_line_of_value)? + last_line_of_value.len();
        let raw_string = value_with_quotes.get(start_of_value..end_of_value)?;
    ...
    pub fn line_col_for_subslice(
        ...
        let (line, column) = lookup.get(end_offset);
