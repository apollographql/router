...
The `supergraph.query_planning` namespace also exposes configuration options that limit the planner's work itself, separate from caching: `plans_limit` and `paths_limit`. Adjust them as described in [Tuning query planner limits](/graphos/routing/query-planning/query-planning-best-practices#tuning-query-planner-limits).
...
With router v1.61.0+ and v2.x+, if you have enabled exposing query plans via `--dev` mode or `expose_query_plan: true`, you can pass the `Apollo-Expose-Query-Plan` header to return query plans in the GraphQL response extensions. You must set the header to one of the following values:
...
In your router's YAML config file, always specify Redis URLs via environment variables and [variable expansion](/graphos/routing/configuration/yaml#variable-expansion). This prevents Redis URLs from being committed to version control, which is especially dangerous if they include authentication information like a username or password.
...
supergraph:
  query_planning:
    cache:
      redis:
        urls: ["redis://..."]
        username: "${env.REDIS_USERNAME}" # Recommended: adopt an environment variable or secret. Takes precedence over username in the URL
        password: "${env.REDIS_PASSWORD}" # Recommended: adopt an environment variable or secret. Takes precedence over password in the URL
...
<Caution>
Avoid hardcoding `username` and `password` directly in your YAML config file.
Instead, use [variable expansion](/graphos/routing/configuration/yaml#variable-expansion) to inject credentials via environment variables or a secrets manager (for example, `${env.REDIS_PASSWORD}`).
