...
A common timeout offender is the idle timeout, but any equivalent read, response, or connection-drain timeout on each hop has the same effect. Defaults vary widely by provider (often a minute or less, but some platforms default higher), so check each hop's documentation rather than relying on a global rule. Set every relevant timeout to be longer than the longest subscription you support. If you've set [`max_lifetime`](#limit-subscription-lifetime), use that value as the floor.
