...
The Apollo Router supports the `@defer` directive as it's documented in [these edits to the RFC](https://github.com/graphql/graphql-spec/pull/742), according to the state of those edits on 2022-09-09.

We will continue to update the router's support for `@defer` as the directive's definition advances through remaining spec contribution stages.
