...
flowchart LR
    ...
    prometheus_exporter(Prometheus<br/>Exporter)
    otlp_exporter("OTLP<br/>Exporter")
    datadog_exporter("Datadog<br/>Exporter")
    jaeger_exporter("Jaeger<br/>Exporter")
    zipkin_exporter("Zipkin<br/>Exporter")
    terminal(stdout)
    
    lifecycle--"metrics"--->prometheus_exporter
    lifecycle--"metrics,<br/>traces"--->otlp_exporter
    lifecycle--"traces"--->zipkin_exporter
    lifecycle--"traces"--->jaeger_exporter
    lifecycle--"traces"--->datadog_exporter
    lifecycle--"logs"--->terminal
    end

    otlp_apm["OTLP enabled APM (e.g. New Relic)"]
    zipkin[Zipkin]
    datadog[Datadog agent]
    apm1[APM]
    collector("OpenTelemetry<br/>Collector")
    jaeger("Jaeger (agent or collector)")

    
    otlp_exporter--"OTLP"-->otlp_apm
    otlp_exporter--"OTLP"-->collector
    otlp_exporter--"OTLP"-->datadog
    otlp_exporter--"OTLP"-->jaeger
    datadog_exporter--"native"-->datadog
    jaeger_exporter--"native"-->jaeger
    zipkin_exporter--"native"-->zipkin
    prometheus_exporter<--"scrapes"-->apm1
