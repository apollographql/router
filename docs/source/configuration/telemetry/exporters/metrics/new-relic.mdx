...
description: Enable and configure the New Relic exporter for metrics in the Apollo Router
---

Enable and configure the [OTLP exporter](./opentelemetry) for metrics in the Apollo Router for use with [New Relic](https://newrelic.com/).

For general tracing configuration, refer to [Router Metrics Configuration](./overview).

## New Relic configuration

To configure the router, enable the [OTLP exporter](./opentelemetry#configuration) and set both `temporality: delta`, `endpoint: <new-relic-endpoint>` and `api-key: <new-relic-api-key>`. For example: 
