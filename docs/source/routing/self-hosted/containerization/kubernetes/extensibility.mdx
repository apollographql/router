---
title: Using router extensibility features in Kubernetes
...
The `router` chart would be the router's Helm chart, which you can deploy as described in the [Kubernetes deployment guide](/graphos/routing/kubernetes/quickstart).
