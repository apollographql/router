...
When a response's full label set would exceed `max_bytes`, the router packs the header coarsest-first and drops whatever doesn't fit, finest-grained first — so an oversized response still gets a usable header rather than none at all. Setting `experimental_drop_on_overflow: true` instead omits the header entirely whenever truncation would occur, for pairing with a CDN-side rule that forces cache bypass on a missing header.
