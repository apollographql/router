...
By default, the Router errors when an unknown prefix is provided in the `Authorization` header. You must explicitly define prefixes in `header_value_prefix` or `sources`.

If you set `ignore_other_prefixes` to `true`, the Router allows unknown prefixes in the `Authorization` header and does not respond with an error when encountering one.

If you set `ignore_other_prefixes` to `false` or leave it unspecified, the Router assumes the default behavior of responding with an error when it finds an unknown prefix in the `Authorization` header.

If you set a header prefix to an empty string, the Router ignores this option.
...
Explicitly listing claims and _always_ setting headers for them is strongly recommended to avoid possible security issues when [forwarding headers](./header-propagation/) to subgraphs.
...
If your router enables [tracing](./telemetry/exporters/tracing/overview), the JWT authentication plugin has its own tracing span: `authentication_plugin`

If your router enables [metrics collection via Prometheus](./telemetry/exporters/metrics/prometheus), the JWT authentication plugin provides and exports the following metrics:
