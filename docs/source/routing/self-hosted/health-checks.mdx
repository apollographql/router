...
GraphOS Router and Apollo Router Core support a basic HTTP-level health check. This is enabled by default and is served on port `8088` at the URL path `/health`. This returns a `200` status code if the HTTP server is successfully serving. The health check is a **separate route** and does **not** go through the [RouterHttp pipeline](/graphos/routing/request-lifecycle#request-path), so [Rhai](/graphos/routing/customization/rhai/) scripts, native plugins, and license enforcement do not run on health requests.
