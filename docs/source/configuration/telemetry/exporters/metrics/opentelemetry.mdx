...
Enable and configure the OpenTelemetry Protocol (OTLP) exporter in the Apollo Router.

## Enable OpenTelemetry exporter

You can export metrics to [OpenTelemetry Collector](https://opentelemetry.io/docs/collector/) for processing and reporting metrics.

To export metrics to OpenTelemetry Collector, enable the OTLP exporter and optionally customize its configuration in [`router.yaml`](./configuration/overview#yaml-config-file). For example: 
