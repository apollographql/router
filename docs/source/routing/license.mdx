...
A router using an offline license relies on [local manifests](/graphos/routing/security/persisted-queries#local_manifests) for [safelisting persisted queries](/graphos/routing/security/persisted-queries) as they provide the metadata needed for safelisting to work in disconnected environments, also these manifests also need to be distributed through your own CI/CD or deployment process since they’re the supported way to supply Persisted Query Lists offline.
