...
supergraph:
  query_planning:
    cache:
      redis:
        urls: ["redis://..."]
        username: "${env.REDIS_USERNAME}" # Recommended: use an environment variable or secret. Takes precedence over username in the URL
        password: "${env.REDIS_PASSWORD}" # Recommended: use an environment variable or secret. Takes precedence over password in the URL
...
Instead, use [variable expansion](https://www.apollographql.com/docs/graphos/routing/configuration/yaml#variable-expansion)
to inject credentials via environment variables or a secrets manager (for example, `${env.REDIS_PASSWORD}`).
