...
When the list size is controlled by an argument that is nested inside an input object, use a dot-separated path in `slicingArguments`. The router resolves the path against the query arguments (and variable values) to obtain the size.
...
For a query such as `search(input: { pagination: { first: 10 }, query: "fiction" })`, the router uses 10 as the list size. Paths can be deeper (for example, `input.level1.level2.count`).

#### The `sizedFields` argument

<MinVersionBadge version="Router v2.12.0" />

When a field returns an **object** that contains one or more list fields, use the `sizedFields` argument to specify which nested fields should use this directive's list size for cost. Only selections that match those paths are scaled by the size; other selections are not.

```graphql
type Query {
  container(first: Int): ResultContainer
    @listSize(slicingArguments: ["first"], sizedFields: ["page"], requireOneSlicingArgument: false)
}

type ResultContainer {
  page: [Book]      # uses the list size from "first"
  metadata: String  # not in sizedFields; cost is 1
}
```

For nested lists, use a path in `sizedFields` such as `"results { page }"` so the list size applies to the inner list when the query selects `deepContainer { results { page { ... } } }`:
