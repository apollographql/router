...
* Set a subgraph-specific list size in your router configuration file (see [Configuring demand control](#configuring-demand-control); requires Router >=v2.12.0)
...
Requires Router v2.12.0 or later.
...
| `static_estimated.actual_cost_mode`               | `by_subgraph`, `response_shape` | `by_subgraph` | - `by_subgraph` calculates the cost of an operation as the sum of the cost of each subgraph response.<br/>- `response_shape` calculates the cost based on only the final shape of the response |
...
* `by_subgraph` (default for Router versions &ge;v2.12.0) sums the cost of each subgraph response. This method reflects
  the total work done per operation and more closely mirrors the cost estimation strategy.
* `response_shape` uses only the final shape of the response. It does not consider the intermediate work required to
  attain that shape, i.e. fields that were fetched to support federated lookups that were not included in the client
  response. This behavior was the only option available in Router versions &le;v2.11.0.
