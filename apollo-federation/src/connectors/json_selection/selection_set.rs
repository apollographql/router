                    ...
                    if let Some(single_key_for_selection) = selection.get_single_key() {
                        if let Some(fields) = field_map.get_vec(single_key_for_selection.as_str()) {
                            for field in fields {
                                ...
                                let applied_path =
                                    path.apply_selection_set(document, &field.selection_set);

                                if let Some(single_key_for_path_only) =
                                    applied_path.get_single_key()
                                {
                                ...
                                new_selections.push(NamedSelection {
                                    alias: Some(Alias::new(response_key)),
                                    path: applied_path,
                                });
                            }
                        }
                        ...
                        new_selections.push(NamedSelection {
                            alias: None,
                            path: path.apply_selection_set(document, selection_set),
                        });
                    }
                }
            }
