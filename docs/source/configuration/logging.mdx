...
Note that `request_tracing` must be set to at least `INFO` for tracing to subgraphs to work. This is because request tracing propagation happens at `INFO` level.
