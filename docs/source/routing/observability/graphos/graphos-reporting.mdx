...
be visible in Studio. To reduce cardinality warnings in your logs, adjust the Apollo metrics OTLP batch processor configuration to send reports more frequently. See the example below lowering the `scheduled_delay` to reduce cardinality stored in the router before the metric is flushed: 
