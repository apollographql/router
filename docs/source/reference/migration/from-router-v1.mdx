...
GraphOS Router 2.x is an [Active](https://www.apollographql.com/docs/graphos/reference/router-release-lifecycle#active) and [Generally Available](https://www.apollographql.com/docs/graphos/reference/feature-launch-stages#general-availability) release. Its latest version is **{products.router.version("lts-2x-latest").version}**. 
...
### Deprecated `OneShotAsyncCheckpointLayer` and `.oneshot_checkpoint_async()`
...
Make sure that you are referencing the correct router release: **v{products.router.version("lts-2x-latest").version}**
