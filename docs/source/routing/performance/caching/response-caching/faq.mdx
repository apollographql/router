...
Yes. Cache tags work like CDN surrogate key systems (used by Fastly, Cloudflare, and other CDN providers). Both approaches let you tag cached content and invalidate it by tag name. In the router, cache tags are set via the `@cacheTag` directive, and surrogate keys are typically set via response headers like `Surrogate-Key` or `Cache-Tag`.

You don't need to wire this up yourself: enable [CDN invalidation](/router/performance/caching/response-caching/cdn-invalidation) to have the router emit your existing cache tags as a response header your CDN can purge by directly.
...
### Why is my Redis CPU high under `response_cache`, and can I reduce it?

By default, each subgraph that uses active invalidation maintains all three Redis invalidation indexes—`subgraph`, `type`, and `cache_tag`—writing to each on every cache insert. At high cache volumes, maintaining indexes you never invalidate by is avoidable Redis work.

If your workload only invalidates by a subset of methods (for example, only by cache tag), you can opt out of the unused indexes per subgraph with the [`indexes`](/router/performance/caching/response-caching/invalidation#tune-which-invalidation-indexes-are-maintained) block under `subgraph.<all|name>.invalidation`. Each field set to `false` removes one ZSET write per cache insert for that subgraph. See [Tune which invalidation indexes are maintained](/router/performance/caching/response-caching/invalidation#tune-which-invalidation-indexes-are-maintained) for the full configuration and the additive-only caveat.

### How does response caching work with Apollo Connectors?

Connectors use the same response caching infrastructure as subgraphs, but with a separate `response_cache.connector` configuration block. The router caches connector responses at both the entity level and the root field (HTTP request) level. Configuration uses `connector.all` for global defaults and `connector.sources` for per-source overrides, where sources are identified by `subgraph_name.source_name`.

### How do I identify a connector source in the configuration?

Connector sources use the `subgraph_name.source_name` format. For example, if your subgraph is named `products` and the connector source is `my_rest_api`, the source identifier is `products.my_rest_api`. This format appears in the `connector.sources` configuration map, invalidation requests, and telemetry labels.

### Can I cache subgraphs and connectors simultaneously?

Yes. The `response_cache.subgraph` and `response_cache.connector` blocks are independent. You can enable caching for subgraphs, connectors, or both. Each has its own Redis configuration, TTLs, and invalidation settings.

### Do connectors support `@cacheControl` directives?

Connectors don't use `@cacheControl` directives because they call HTTP APIs directly. Instead, the router reads the `Cache-Control` HTTP headers returned by the upstream REST API. If the API returns `Cache-Control: max-age=300`, the router caches the response for 300 seconds. The configured `ttl` serves as a fallback when no `Cache-Control` header is present.
