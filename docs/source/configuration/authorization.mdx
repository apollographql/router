...
directive @policy(scopes: [[federation__Policy!]!]!) on OBJECT | FIELD_DEFINITION | INTERFACE | SCALAR | ENUM
```
...
A request must include _all_ elements in the inner-level `policies` array to resolve the associated field or type. In other words, the authorization validation uses **AND** logic between the elements in the inner-level `policies` array.

```graphql
@requiresScopes(policies: [["policy1", "policy2", "policy3"]])
```

For the preceding example, a request would need `policy1` **AND** `policy2` **AND** `policy3` to be authorized.
...
For the preceding example, a request would need `policy1` **OR** `policy2` **OR** `policy3` to be authorized.
...
This syntax requires requests to have either (`policy1` **AND** `policy2`) **OR** just `policy3` to be authorized.
...
By default, when part of a query is filtered by authorization, the list of filtered paths is added to the response and logged by the router. This is behavior can be customized for your needs.

#### log

By enabling the option, you can choose if query filtering will result in a log event being output.
...
If filtering parts of a query according to the client's rights may be seen as normal operation, and not warrant investigation by platform operators, logging should be disabled.

#### response

By default, filtered paths are placed in the errors part of the graphql response.

You can configure response to carry them in:
