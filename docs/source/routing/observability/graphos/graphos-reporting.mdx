...
be aware that they can override or interfere with the Router’s built-in telemetry configuration.
...
**Starting in Apollo Router v2.12.0 and later, the Router will _refuse to start_ if any of these variables are set.**  
They must be removed from the environment before launching the Router.
