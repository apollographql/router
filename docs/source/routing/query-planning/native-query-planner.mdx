...
As of v1.59.0, the native query planner is the default planner in the router. As part of this, Deno, and by extension v8, are no longer intialised at router startup. The legacy implementation is deprecated, but it is still possible to opt-in to legacy query planner.
