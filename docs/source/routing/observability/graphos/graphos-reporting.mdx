...
:::note
`send_headers` applies only to incoming client request headers. Subgraph response headers are not captured by this option, even if they are forwarded to the client response.
:::
...
telemetry:
  apollo:
    # The percentage of requests will include HTTP request headers in traces sent to GraphOS Studio.
    # This is expensive and should be left at a low value.
    # This cannot be higher than tracing->common->sampler
    field_level_instrumentation_sampler: 0.01 # (default)

    # Include HTTP request headers in traces sent to GraphOS Studio
