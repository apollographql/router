...
A connector response mapping can report an error without failing the field, using the [`->withGraphQLError` method](/graphos/connectors/responses/error-handling#report-errors-without-failing-a-field). Those errors are written by the author of the connector's subgraph schema and can interpolate data from the API's response, so they are governed by this configuration too, under the name of the subgraph the connector belongs to.
