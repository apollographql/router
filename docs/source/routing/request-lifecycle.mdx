...
To learn how to hook in to the various lifecycle stages, including example customizations, start with the [router customization overview](/graphos/routing/customization/overview), then refer to the [WebAssembly plugins](/graphos/routing/customization/wasm/), [Rhai scripts](/graphos/routing/customization/rhai/), and [external coprocessing](/router/customizations/coprocessor/) docs.
