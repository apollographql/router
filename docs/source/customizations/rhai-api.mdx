...
Note: Be particularly careful when interacting with headers in a response context. For SupergraphService and ExecutionService, headers only exist for the first response in a deferred response stream. Since you have no way of knowing if a response is deferred or not, the best way to handle this is to wrap your `response.headers` interactions in a `try...catch` block and handle exceptions accordingly.

Here's an example of how to handle this specific error:
...
```rhai
    ...
    {
        // ignore deferred header access errors
