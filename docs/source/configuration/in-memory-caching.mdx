...
* histograms:
  * `apollo.router.query_planner.plan.duration`: time spent planning queries
...
then look at `apollo_router_schema_loading_time` and `apollo.router.query_planner.plan.duration` to decide how much time we want to spend warming up queries.
