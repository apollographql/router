...
telemetry:
  logging:
    ...
    router:
      request:
        enabled: false # Display router's request payload with a debug log (default: false)
      response:
        mode: on_error # Display router's response payload on the router with a debug log only when there is an error (default: always_off). Could be always_on|always_off|on_error.
    subgraph:
      all: # For all subgraphs
        request:
          enabled: true # Display subgraph's request payload with a debug log (default: false)
        response:
          mode: always_off # Display subgraph's response payload on the router with a debug log only when there is an error (default: always_off). Could be always_on|always_off|on_error.
      subgraphs:
        accounts: # Only for subgraph named "accounts"
          request:
            enabled: false # Display "accounts" subgraph's request payload with a debug log (default: false)
          response:
            mode: always_on
```
