...
The JavaScript query planner also supports an experimental algorithm that attempts to
reuse fragments from the original operation while forming subgraph requests. This
experimental feature used to be enabled by default, but is still available to support
subgraphs that rely on the specific shape of fragments in an operation:
