...
When you apply `@authenticated`, `@requiresScopes`, or `@policy` to a root operation type itself in a subgraph:
...
To scope authorization on root operation types reliably, apply the directive at the **field** level rather than the type level:
