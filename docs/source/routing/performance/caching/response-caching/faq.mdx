...
If you're currently using the `preview_entity_caching` plugin, you should migrate to response caching as soon as possible. The new implementation provides better performance and more flexible caching controls.
...
2. **Remove the `scan_count` setting:** Delete the `redis.scan_count` configuration option if you have it set. Response caching uses approach that does not require Redis `SCAN` operations, eliminating a key performance bottleneck that existed in the previous entity caching version.
...
3. **Update timeout configuration:** Replace the single `redis.timeout` setting with more granular timeout options as appropriate:
   ...
   All timeout settings remain optional. Determine which timeouts you need based on your cache operations.
