...
- `apollo.router.operations.subscriptions.terminated` - Number of subscription client streams that have ended. Incremented once per client connection. When deduplication is enabled, a single subgraph WebSocket closure produces one event per deduplicated client (each with `reason=server_close`). Default attributes:
      ...
      router:
        apollo.router.operations.subscriptions.terminated:
          attributes:
            ...
            client.name: true
```
- `apollo.router.operations.subscriptions.ended.subgraph` - Number of subgraph WebSocket connections closed by the subgraph. Incremented once per subgraph connection closure. Each deduplicated client sharing that connection will also emit a corresponding `terminated` event with `reason=server_close`. Attributes:
