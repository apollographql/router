...
```yaml title="router.yaml" novalidate
telemetry:
  exporters:
    tracing:
      propagation:
...
In v2.x, this flag is renamed to `otlp_tracing_sampler`, and it's enabled by default. In v3.x, the legacy trace transport is removed entirely: traces are always reported via OTLP and the `otlp_tracing_sampler` option no longer exists.

**Upgrade step**: in your router config, remove any use of `experimental_otlp_tracing_sampler` or `otlp_tracing_sampler`.
