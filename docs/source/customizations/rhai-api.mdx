...
There are multiple ways to encode base64, and the default one might not correspond to what the Router is receiving. The `encode` and `decode` functions support an additional argument to specify the alphabet, with the following possible values:
