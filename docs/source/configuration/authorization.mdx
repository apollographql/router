...
At the `SupergraphService` level, you need to provide a Rhai script or a coprocessor that evaluates the map. If the policy is validated, set its value to `true`, otherwise set it to `false`. Afterwards, the router filters the requests' types and fields to only those where the policy is `true`.

If no field of a subgraph query passes its authorization policies, the router stops further processing of the query and precludes unauthorized subgraph requests. This efficiency gain is a key benefit of the `@policy` directive and other authorization directives.
