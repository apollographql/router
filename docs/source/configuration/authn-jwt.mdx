...
Whenever the router receives a JWT with a key ID (`kid`) that it doesn't recognize, it fetches an up-to-date JWK Set from its [`jwks_url`](#jwks_url), _as long as_ it's been _at least_ the amount of time specified by `cooldown`.
...
The Apollo Router obtains each JSON Web Key (JWK) that it uses from a URL that you specify via the [`jwks_url`](#jwks_url) configuration option. This URL must provide the full list of valid JWKs in a single JSON object called a **JWK Set** (or JWKS).
