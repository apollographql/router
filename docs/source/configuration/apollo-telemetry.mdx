...
telemetry:
  ...
  apollo:
    ...
    # Enable apollo FTV1 (federated traces) on subgraphs
    field_level_instrumentation: true
  tracing:
    trace_config:
      sampler: 0.5 # Traces are sampled then don't forget to configure a sampler for traces (could be a rate or `always_on` or `always_off`)
```
