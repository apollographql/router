...
By default, the router validates the fields of input object variables against their type definitions. If a variable contains an unknown field—one not defined on the corresponding input type—the router rejects the request with an HTTP 400 error.

For example, given this schema and query:
...
query SendMessage($msg: MessageInput) {
  send(message: $msg) {
    id
  }
}
...
Sending `$msg` with an extra `unknownField` key:
...
Returns a `VALIDATION_INVALID_TYPE_VARIABLE` error:

```json
{
  "errors": [
    {
      "message": "unknown field input value at `$msg.unknownField` found for GraphQL type `MessageInput`",
      "extensions": {
...
This is the default `enforce` mode:
...
**When to use `measure` mode:** If you have existing clients sending extra fields that you can't update immediately, set `strict_variable_validation: measure` to allow those requests through while you migrate. In `measure` mode, the router logs a warning for each unknown field at `warn` level instead of rejecting the request.
...
`measure` mode is intended as a temporary migration aid. Unknown fields in input variables are silently ignored by subgraphs, which can mask bugs or data loss. Switch to `enforce` as soon as your clients are updated.
