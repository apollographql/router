...
Response caching enables the router to cache origin responses and reuse them across queries. (An _origin_ is a data source.) The router caches two kinds of data:
...
Response caching helps you:
- Reduce the load on your underlying services
- Improve overall latencies for client requests

The router provides a GraphQL-aware cache with advantages over a generic CDN. It caches at the entity and root field level—GraphQL-native and reusable across users and requests.
...
Response caching addresses these challenges by caching origin responses with flexible cache control. The router determines cache TTLs from the `Cache-Control` HTTP headers returned by your origins—these headers are derived from `@cacheControl` directives in your schema. When an origin response contains multiple entity representations, the router uses the minimum TTL value. Different responses can have different TTLs and visibility settings, and you can actively invalidate cached data when you know it has changed. When the router runs behind a CDN, it can also emit the cache tags involved in a request as a response header, so the router's response cache and the CDN edge cache can be invalidated by the same tag in one workflow—see [Propagate cache tags to a CDN](/router/performance/caching/response-caching/customization#propagate-cache-tags-to-a-cdn).
