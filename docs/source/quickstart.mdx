...
That's it! Running the Router with the `--dev` flag enables a development mode that exposes [Apollo Sandbox](/studio/explorer/sandbox/) so you can run queries against the Apollo Router.  This flag also relaxes some defaults to provide a better development experiences (like exposing subgraph error messages), so you'll exclude this flag when not doing development.

Visit `http://127.0.0.1:4000` to open Apollo Sandbox, inspect the entire federated graph and run your first queries!
