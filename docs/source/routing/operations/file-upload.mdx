...
##### `protocols.multipart.limits.operation_body_timeout`

The maximum time to allow the client to deliver the GraphQL operation (query and variables) within the multipart body. This timeout does not apply to reading the file contents themselves. If the operation is too slow to arrive, the router rejects the request with a `504 Gateway Timeout` error.
