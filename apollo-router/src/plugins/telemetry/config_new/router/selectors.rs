...
pub(crate) enum RouterSelector {
    ...
    /// String "T" or "F" if the response body contains graphql error (for v1 plugin compatibility)
    OnGraphQLErrorString { on_graphql_error_string: bool },
...
impl Selector for RouterSelector {
    ...
    fn on_response(&self, response: &router::Response) -> Option<opentelemetry::Value> {
        match self {
                        ...
                        .and_then(|body_json| {
                            ...
                            if extracted.is_empty() {
                                ...
                                Some(opentelemetry::Value::Array(
                                    extracted.into_iter().map(opentelemetry::StringValue::from).collect::<Vec<_>>().into()
                                ))
                            }
                        })
                }),
                ...
                .map(|tracker| {
                    ...
                    match request_duration {
                        ...
                        DurationUnit::Seconds => {
                            opentelemetry::Value::F64(duration.as_secs_f64())
                        }
            ...
            RouterSelector::OnGraphQLErrorString { on_graphql_error_string }
                if *on_graphql_error_string =>
            {
                let contains_error = response
                    .context
                    .get_json_value(CONTAINS_GRAPHQL_ERROR)
                ...
                Some(opentelemetry::Value::String(
                    if contains_error { "T" } else { "F" }.into(),
                ))
            }
            RouterSelector::Static(val) => Some(val.clone().into()),
            RouterSelector::StaticField { r#static } => Some(r#static.clone().into()),
...
mod test {
    ...
    fn router_response_errors_count() {
        ...
        let res = &crate::services::RouterResponse::fake_builder()
            ...
            .build()
            .unwrap();
        assert_eq!(selector.on_response(res), Some(opentelemetry::Value::I64(3)));
