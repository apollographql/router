...
- `apollo.router.oci.manifest.count` - Number of requests made to get the manifest for a Graph Artifact, attributes:
  - `registry`: The hostname of the OCI registry
  - `kind`: (`get_manifest`, `head_manifest`)
  - `status`: (`success`, `failure`)
- `apollo.router.oci.manifest.duration.seconds` - Duration of request to get the manifest for a Graph Artifact, attributes:
  - `registry`: The hostname of the OCI registry
  - `kind`: (`get_manifest`, `head_manifest`)
  - `status`: (`success`, `failure`)
- `apollo.router.oci.blob.count` - Number of fetches made to get the blob containing the supergraph from a Graph Artifact, attributes:
  - `registry`: The hostname of the OCI registry
  - `kind`: (`get_blob`)
  - `status`: (`success`, `failure`)
- `apollo.router.oci.blob.duration.seconds` - Duration of request to get the blob containing the supergraph from a Graph Artifact, attributes:
  - `registry`: The hostname of the OCI registry
  - `kind`: (`get_blob`)
  - `status`: (`success`, `failure`)
...
As with the initial call to Uplink, calls to the GraphArtifacts endpoint during router startup are not reflected in metrics.
