...
- **Emitter** fails to send **Router** a [`check`](#check) message for an active subscription every five seconds, causing **Router** to terminate that subscription.
...
2. _The Apollo Router_ generates an unique ID for the following subscription
...
7. _Event source_ spawns a background task to listen on `subscription` events
    - Every 5 seconds the _Event source_ must call the callback endpoint with a [`check` payload](#check) to heartbeat and confirm it's still listening to the subscription
