...
When processing batched operations, the Router will count each entry in the batch as a distinct billable operation that counts towards your graph usage. Even if you send a batch with `N` requests of the same exact operation, that is counted the same as sending `N` requests of the same non-batched operation. Note that GraphOS plans only track Router operations, so configuring subgraph batching does not impact your graph usage count.
