...
Most environment variables are also documented with their [Command-line options](#command-line-options) counterpart. However, there are some options that are only available to configure as environment variables.

To set environment variables, make sure you include them when starting the router binary or that they are setup with your infrastructure configuration:
```bash
APOLLO_KEY="..." APOLLO_GRAPH_REF="..." MY_CUSTOM_VAR="..." ./router
```
...
<table class="field-table api-ref">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
      `EXPERIMENTAL`: The starting size of query planner heap to use in MB. Setting `APOLLO_ROUTER_BRIDGE_EXPERIMENTAL_V8_INITIAL_HEAP_SIZE=4096` is ~4GB.

      Apollo Router v1.x uses a JavaScript runtime internally to run the query planner. While this is mostly an internal implementation detail, large supergraphs tend to increase the amount of memory required in the Router. The query planner heap will dynamically expand as needed however, increasing the starting size of the memory allocated to the heap can improve the first planning times for large operations against a large supergraphs.
      </td>
    </tr>
