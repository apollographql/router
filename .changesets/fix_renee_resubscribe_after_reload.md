...
When a hot reload was triggered by a schema change, the router closed subscriptions with a `SUBSCRIPTION_RELOAD` error.  This happened *before* the new schema was fully active and warmed up, so clients could reconnect to the _old_ schema, which should not happen.
