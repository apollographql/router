...
async fn insert() {
    ...
    .into_iter()
    .collect();
    let entity_cache = SubgraphCache::with_mocks(redis_cache.clone(), map, valid_schema.clone())
        .await
        .unwrap();
...
async fn invalidate() {
    ...
    let redis_cache = RedisCacheStorage::from_mocks(Arc::new(MockStore::new()))
        .await
        .unwrap();
    let entity_cache = SubgraphCache::with_mocks(redis_cache.clone(), HashMap::new())
        .await
        .unwrap();
