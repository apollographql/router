...
When the subgraph WebSocket connection drops and reconnection is configured, the router re-establishes the connection and re-sends the original subscription request transparently, client subscriptions remain open during the reconnect window and resume receiving events once the connection is restored. Each reconnect attempt increments the `apollo.router.operations.subscriptions.reconnect` counter (labeled with `subgraph.name`). If all retry attempts are exhausted, the subscription terminates and the router increments the `apollo.router.operations.subscriptions.terminated.subgraph` metric.
