...
- **Configure a TTL** either globally under `subgraph.all.ttl` or for each individual subgraph—the router requires a TTL to be configured and will fail to start without one. This TTL acts as a fallback; when a subgraph response includes a `Cache-Control` header with `max-age`, that value takes precedence.
