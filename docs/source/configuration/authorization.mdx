...
You may need to restrict sensitive information to authorized users or enforce conditional access rules for particular fields.
The Apollo Router allows you to control access to specific fields and types through the `@authenticated` and `@requiresScopes` directives:
...
You define and use the directives on a subgraph level, and the router [composes](#composition-and-federation) them to the supergraph.
...
The JWT plugin validates the token, extract its claims, and sets them in the request context at the key `apollo_authentication::JWT::claims`.
...
The router woudl filter the query as follows:
