...
**Prometheus name changes in router 3.x.** Several duration and byte-count instruments are gaining a UCUM unit (`s` for durations, `By` for bytes). The Prometheus exporter appends a UCUM-derived suffix to the exported name when a unit is set — for example, `apollo_router_cache_hit_time` becomes `apollo_router_cache_hit_time_seconds`.

To give you time to migrate dashboards, alerts, and recording rules, router 3.x **dual-emits** each affected metric: the legacy unsuffixed series continues to appear on `otel_scope_name="apollo/router"`, and a new suffixed series appears in parallel on `otel_scope_name="apollo/router/ucum"`. The legacy series will be removed in a future major version.
...
- `apollo.router.telemetry.studio.reports` - The number of reports submitted to GraphOS Studio by the router.
  ...
  - `report.protocol`: "apollo" for usage report metrics, "otlp" for traces.

## Telemetry

- `apollo.router.telemetry.batch_processor.errors` - The number of errors encountered by exporter batch processors.
  - `name`: One of `apollo-tracing`, `datadog-tracing`, `otlp-tracing`.
