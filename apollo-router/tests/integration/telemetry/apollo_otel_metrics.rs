...
/// The metrics an operator actually sees for an error a connector mapping
/// declared with `->withError`, end to end through a real router.
///
/// Declared errors are reported in `extensions.connectorErrors` rather than in
/// `errors`, so "is this counted like any other error?" is a fair question to
/// ask of the running router rather than of a doc comment. The answer this
/// pins: counted by `apollo.router.operations.error`, and deliberately **not**
/// by `apollo.router.graphql_error`, because the field each one describes
/// resolved and so the response carries no GraphQL error.
///
/// The author's `code` is what identifies it — `POST_BODY_MISSING` here, not
/// the `CONNECTORS_MAPPING_ERROR` default — and `apollo.router.error.service`
/// is the connector's subgraph name, the same name `include_subgraph_errors`
/// is keyed by.
///
/// One row, so one error and one datapoint. `graphql.error.path` differs per
/// row, so several rows would split the count across datapoints and assert
/// nothing clearly; that a `->withError` inside a mapping over N rows produces
/// N errors is pinned by `every_declared_error_is_reported` instead.
///
/// `include_subgraph_errors: all: true` is not decoration: the default redacts
/// subgraph errors, and a connector's declared errors are omitted along with
/// them, so without it this test would assert the absence of a metric.
///
/// `connectors: sources: {}` is the stanza that makes the harness point the
/// connector at the local wiremock instead of the schema's real
/// `jsonplaceholder.typicode.com` baseURL — see the comment on
/// `test_connector_request_emits_histogram`, which paid for that lesson with a
/// CircleCI flake.
#[tokio::test(flavor = "multi_thread")]
async fn test_declared_connector_error_emits_metrics() {
    if !graph_os_enabled() {
        return;
    }
    let expected_operation_name = "ExampleQuery";
    ...
    let expected_service = "connectors";
    let expected_operation_type = "query";
    let expected_error_code = "POST_BODY_MISSING";
    ...
    let mut router = IntegrationTest::builder()
        ...
        // its error and the field still resolves — which is the whole point of
        // the method, and the reason this is not an entry in `errors`.
        .responder(ResponseTemplate::new(200).set_body_json(json!([{
            "id": 1,
            "title": "Awesome post",
            "userId": 1
    ...
    assert!(
        response.contains(expected_error_code),
        "expected the author's error code, got: {response}"
    );

    let expected_operations_error = Metric::builder()
        .name("apollo.router.operations.error".to_string())
        .attribute("graphql.operation.name", expected_operation_name)
        .attribute("graphql.operation.type", expected_operation_type)
        .attribute("apollo.client.name", expected_client_name)
        ...
        .value(1)
        .build();
    let metrics = wait_for_metric_match(
        &mut router,
        &expected_operations_error,
        Duration::from_secs(5),
    )
    .await;
    assert!(!metrics.is_empty());
    assert_metrics_contain(&metrics, expected_operations_error);

    // And *not* in the legacy counter. Asserted rather than assumed, because
    // "do these land in `graphql_error`?" was the review question and the
    // answer is a deliberate no — a regression here would be silent, since
    // nothing else would fail.
    let unwanted_graphql_error = Metric::builder()
        .name("apollo.router.graphql_error".to_string())
        .attribute("code", expected_error_code)
        .build();
    let (found, observed) = metrics_contain_and_observed(&metrics, &unwanted_graphql_error);
    assert!(
        !found,
        "a declared error must not reach apollo.router.graphql_error, but found:\n{}",
        observed
            .iter()
            .map(|m| m.to_string())
            .collect::<Vec<_>>()
...
/// The metric survives redaction; the client-facing error does not.
///
/// This is the claim that `count_connector_errors` is called at the connector
/// rather than read back off the reported extension, and it is the one worth
/// testing end to end, because the two halves are decided in different places:
/// `include_subgraph_errors` governs what a client receives, and
/// `telemetry.apollo.errors.subgraph` governs what leaves as telemetry. Without
/// `include_subgraph_errors` — the default, asserted here by its absence from
/// the config — a connector's declared errors are omitted from the response
/// along with every other subgraph error, and an operator would otherwise have
/// no way to know they happened at all.
///
/// This is also the first thing a mapping author hits: write `->withError`,
/// query, see nothing, and conclude the method is broken.
#[tokio::test(flavor = "multi_thread")]
async fn test_declared_connector_error_is_counted_even_when_redacted() {
    ...
    let expected = Metric::builder()
        .name("apollo.router.operations.error".to_string())
        .attribute("graphql.error.extensions.code", expected_error_code)
        .attribute("apollo.router.error.service", "connectors")
        .value(1)
        .build();
    let metrics = wait_for_metric_match(&mut router, &expected, Duration::from_secs(5)).await;
    assert!(!metrics.is_empty());
...
/// `->withProblem` is the other half of the partition, and it reaches neither
/// the client nor the error counters.
///
/// The two methods differ only in who reads the result, which makes the
/// negative case the one that can regress invisibly: a `->withProblem` that
/// started counting as an error, or started appearing in
/// `extensions.connectorErrors`, would break the promise the method is for
/// without breaking any assertion about `->withError`.
///
/// The responder omits `title` and supplies `body`, so the fixture's
/// `->withProblem` fires and its `->withError` does not.
#[tokio::test(flavor = "multi_thread")]
async fn test_connector_mapping_problem_is_not_reported_or_counted() {
    if !graph_os_enabled() {
        return;
    }
    ...
    assert!(
        !response.contains("connectorErrors"),
        "a problem must not be reported to the client, got: {response}"
    );
    assert!(
        !response.contains("Post title was missing"),
        "a problem's message must not reach the client, got: {response}"
    );

    // Give the batch processor a window comparable to the positive tests', so
    // this is "the metric did not arrive in the time one normally takes to
    // arrive" rather than "nothing had arrived yet".
    let unwanted = Metric::builder()
        .name("apollo.router.operations.error".to_string())
        .attribute("apollo.router.error.service", "connectors")
        .build();
    let metrics = wait_for_metric_match(&mut router, &unwanted, Duration::from_secs(5)).await;
    let (found, observed) = metrics_contain_and_observed(&metrics, &unwanted);
    assert!(
        !found,
        "a mapping problem must not be counted as an error, but found:\n{}",
        observed
            .iter()
            .map(|m| m.to_string())
            .collect::<Vec<_>>()
