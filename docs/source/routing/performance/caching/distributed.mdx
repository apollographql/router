...
When enabling distributed caching, consider how frequently you publish new schemas and configure the TTL accordingly. When new schemas are published, the router will [pre-warm](/router/configuration/in-memory-caching#cache-warm-up) the in-memory and distributed caches but **will not** invalidate existing cached query plans in the distributed cache,
creating an additive effect on cache utilization. Consider decreasing the TTL e.g. to 24 hours or twice the median publish interval when publishing frequently to prevent cache overflow and monitor cache utilization in your environment especially during schema publish events.
...
of router instances multiplied by the number of warmed-up queries per instance, which may non-trivially increase the total cache utilization.
