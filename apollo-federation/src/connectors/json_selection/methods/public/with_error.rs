...
mod tests {
    ...
    fn every_with_error_diagnostic() {
        let cases = [
            ...
            ("a bare number", r#"$->withError($(42))"#),
            ("a bare array", r#"$->withError($([1, 2]))"#),
            ("an empty object", r#"$->withError({})"#),
            (
                "a misspelled `message`",
                r#"$->withError({ messge: "typo" })"#,
            ),
            (
                "several keys, none of them `message`",
                r#"$->withError({ messge: "typo", code: "OOPS" })"#,
            ),
            ("a non-string `message`", r#"$->withError({ message: 42 })"#),
            (
                "non-object `extensions`",
                r#"$->withError({ message: "m", extensions: "nope" })"#,
            ),
            (
                "an author field outside `extensions`",
                r#"$->withError({ message: "m", code: "OOPS" })"#,
            ),
            (
    ...
    fn a_string_argument_is_the_message() {
        let (value, errors) =
            selection!(r#"$->withError("Balance unavailable")"#).apply_to(&json!(null));
    ...
    fn a_misspelled_message_key_is_named_in_the_diagnostic() {
        let (value, errors) =
            selection!(r#"$->withError({ messge: "Balance unavailable" })"#).apply_to(&json!("v"));

        assert_eq!(value, Some(json!("v")));
        assert_eq!(
            errors.iter().map(ApplyToError::message).collect::<Vec<_>>(),
            vec![concat!(
                r#"Method ->withError( { messge: "Balance unavailable" } ) declared no "#,
    ...
    fn an_empty_object_argument_says_it_is_empty() {
        let (value, errors) = selection!("$->withError({})").apply_to(&json!("v"));
    ...
    fn a_non_string_message_is_rejected() {
        ...
        assert_eq!(
            errors.iter().map(ApplyToError::message).collect::<Vec<_>>(),
            vec![concat!(
                "Method ->withError( { message: 42 } ) declared no error ",
    ...
    fn non_object_extensions_are_rejected() {
        ...
        assert_eq!(
            errors.iter().map(ApplyToError::message).collect::<Vec<_>>(),
            vec![concat!(
                r#"Method ->withError( { message: "m", extensions: "nope" } ) declared no error "#,
    ...
    fn a_bare_scalar_is_rejected() {
        let (value, errors) = selection!(r#"$->withError(42)"#).apply_to(&json!("v"));
    ...
    fn a_failed_argument_costs_the_error_and_not_the_value() {
        let (value, errors) = selection!(r#"balance: $.amount ?? $(null)->withError(@.nope)"#)
    ...
    fn several_errors_about_one_value_are_several_calls() {
        let (value, errors) =
            selection!(r#"$->withError("Code is unrecognized")->withError("Amount is negative")"#)
                .apply_to(&json!("v"));
    ...
    /// `?? $(null)->withError(...)` silently does nothing, which is the
    ...
    fn a_call_with_the_wrong_number_of_arguments_is_reported() {
        let (value, errors) = selection!("$->withError").apply_to(&json!("value"));
