...
use super::{
    core_json_request_to_supergraph_request, supergraph_request_to_core_json_request,
    core_json_response_to_supergraph_response, supergraph_response_to_core_json_response,
...
mod tests {
    use super::*;
    use http::StatusCode;
    use apollo_router_core::services::json_server::{Request as CoreJsonRequest, Response as CoreJsonResponse};
    ...
    async fn test_core_json_request_to_supergraph_request() {
        ...
        let supergraph_request = core_json_request_to_supergraph_request(core_request)
            .await
            .unwrap();

        assert_eq!(supergraph_request.supergraph_request.method(), "POST");
        assert_eq!(supergraph_request.supergraph_request.uri(), "http://example.com/graphql");
        assert_eq!(
            supergraph_request.supergraph_request.headers().get("content-type").unwrap(),
            "application/json"
        );
        assert_eq!(supergraph_request.supergraph_request.body(), &graphql_request);
        
        // Compare specific values in context instead of direct Context comparison
        let stored_value = supergraph_request.context.get::<_, String>("test_key").unwrap().unwrap();
    ...
    async fn test_supergraph_request_to_core_json_request() {
        ...
        let core_request = supergraph_request_to_core_json_request(supergraph_request)
            .await
            .unwrap();
        ...
        let metadata_arc = core_request.extensions.get::<Arc<RequestMetadata>>().unwrap();
        assert_eq!(metadata_arc.http_parts.method, "POST");
        assert_eq!(metadata_arc.http_parts.uri, "http://example.com/graphql");
        let stored_value = metadata_arc.context.get::<_, String>("test_key").unwrap().unwrap();
        assert_eq!(stored_value, "test_value");
    }
    ...
    async fn test_core_json_response_to_supergraph_response() {
        let context = Context::new();
        context.insert("response_key", "response_value".to_string()).unwrap();
        ...
        let supergraph_response = core_json_response_to_supergraph_response(core_response)
            .await
            .unwrap();

        assert_eq!(supergraph_response.response.status(), StatusCode::OK);
        assert_eq!(
            supergraph_response.response.headers().get("content-type").unwrap(),
            "application/json"
        );
        
        // Compare specific values in context instead of direct Context comparison
        let stored_value = supergraph_response.context.get::<_, String>("response_key").unwrap().unwrap();
    ...
    async fn test_supergraph_response_to_core_json_response() {
        ...
        let core_response = supergraph_response_to_core_json_response(supergraph_response)
            .await
            .unwrap();

        // Verify metadata is stored in extensions
        let metadata_arc = core_response.extensions.get::<Arc<ResponseMetadata>>().unwrap();
        assert_eq!(metadata_arc.http_parts.status, StatusCode::OK);
        let stored_value = metadata_arc.context.get::<_, String>("response_key").unwrap().unwrap();
        assert_eq!(stored_value, "response_value");
    ...
    async fn test_roundtrip_request_conversion() {
        let context = Context::new();
        context.insert("roundtrip_key", "roundtrip_value".to_string()).unwrap();
        ...
        let original_value = original_request.context.get::<_, String>("roundtrip_key").unwrap().unwrap();

        // Convert to core and back
        let core_request = supergraph_request_to_core_json_request(original_request)
            .await
            .unwrap();
        let converted_request = core_json_request_to_supergraph_request(core_request)
            .await
            .unwrap();

        assert_eq!(converted_request.supergraph_request.method(), "POST");
        assert_eq!(converted_request.supergraph_request.uri(), "http://example.com/graphql");
        assert_eq!(
            converted_request.supergraph_request.headers().get("content-type").unwrap(),
            "application/json"
        );
        assert_eq!(converted_request.supergraph_request.body(), &graphql_request);
        
        // Compare context values
        let final_value = converted_request.context.get::<_, String>("roundtrip_key").unwrap().unwrap();
    ...
    async fn test_roundtrip_response_conversion() {
        let context = Context::new();
        context.insert("roundtrip_response_key", "roundtrip_response_value".to_string()).unwrap();
        ...
        // Store original context value for comparison
        let original_value = original_response.context.get::<_, String>("roundtrip_response_key").unwrap().unwrap();

        // Convert to core and back
        let core_response = supergraph_response_to_core_json_response(original_response)
            .await
            .unwrap();
        let converted_response = core_json_response_to_supergraph_response(core_response)
            .await
            .unwrap();

        assert_eq!(converted_response.response.status(), StatusCode::OK);
        assert_eq!(
            converted_response.response.headers().get("content-type").unwrap(),
            "application/json"
        );
        
        // Compare context values
        let final_value = converted_response.context.get::<_, String>("roundtrip_response_key").unwrap().unwrap();
        assert_eq!(original_value, final_value);
    ...
    async fn test_complex_graphql_request_conversion() {
        let context = Context::new();
        context.insert("complex_key", "complex_value".to_string()).unwrap();
        ...
        assert_eq!(converted_request.supergraph_request.method(), "POST");
        assert_eq!(converted_request.supergraph_request.uri(), "http://api.example.com/graphql");
        assert_eq!(
            converted_request.supergraph_request.headers().get("authorization").unwrap(),
            "Bearer token123"
        );
        assert_eq!(
            converted_request.supergraph_request.headers().get("x-client-name").unwrap(),
            "test-client"
        );
        assert_eq!(converted_request.supergraph_request.body(), &graphql_request);
        
        let stored_value = converted_request.context.get::<_, String>("complex_key").unwrap().unwrap();
