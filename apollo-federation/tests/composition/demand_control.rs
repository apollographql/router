...
#[test]
fn composes_fractional_cost_weights() {
    let subgraph = Subgraph::parse(
        ...
        type Query {
            halfCost: Int @cost(weight: "0.5")
        }
    "#,
    )
    .unwrap();
    let result = compose(vec![subgraph]).unwrap();
    let half_cost = coord!(Query.halfCost)
        .lookup_field(result.schema().schema())
        .unwrap();
    let cost_directive = half_cost
        .directives
        .iter()
        .find(|d| d.name == "cost")
        .expect("Expected @cost directive to be present on Query.halfCost");

    assert_eq!(cost_directive.to_string(), r#"@cost(weight: "0.5")"#);
}

#[test]
fn composes_legacy_integer_cost_weights() {
    let subgraph = Subgraph::parse(
        "subgraph",
        "",
        r#"
        ...
        type Query {
            legacyCost: Int @cost(weight: 5)
        }
    "#,
    )
    .unwrap();
    let result = compose(vec![subgraph]).unwrap();
    let legacy_cost = coord!(Query.legacyCost)
        .lookup_field(result.schema().schema())
        .unwrap();
    let cost_directive = legacy_cost
        .directives
        .iter()
        .find(|d| d.name == "cost")
        .expect("Expected @cost directive to be present on Query.legacyCost");

    assert_eq!(cost_directive.to_string(), r#"@cost(weight: "5")"#);
}

#[test]
fn errors_on_non_numeric_cost_weight_strings() {
    let subgraph = Subgraph::parse(
        "subgraph",
        "",
        r#"
        ...
        type Query {
            badCost: Int @cost(weight: "abc")
        }
    "#,
    )
    .unwrap();
    let errors = compose(vec![subgraph]).unwrap_err().errors;

    assert!(
        errors
            .iter()
            .any(|error| error.code() == ErrorCode::InvalidGraphQL
                && error.to_string().contains("serialized finite Float")),
        "Expected invalid cost weight error, got: {errors:?}"
    );
}
