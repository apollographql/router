...
>    * `graph-ws` (with a dash) is the name of the [library](https://github.com/enisdenjo/graphql-ws) that uses the recommended `graphql_transport_ws` WebSocket subprotocol. 
>     * `graph_ws` (with an underscore) is the router's configuration value to use the legacy subprotocol of the `subscriptions-transport-ws` library.
