...
- **[OpenTelemetry collector](./otel-collector)** - Vendor-neutral telemetry pipeline with advanced processing (this guide)
- **[Datadog agent](./datadog-agent/datadog-agent-traces)** - Direct connection with native Datadog features ([guide available](./datadog-agent/datadog-agent-traces))
...
- Apollo Router installed and running - see [deployment options](/graphos/routing/self-hosted)
- OpenTelemetry collector - see [installation options](https://opentelemetry.io/docs/collector/installation/)
- Datadog account with an API key - see [how to generate an API key](https://docs.datadoghq.com/account_management/api-app-keys/)
- OpenTelemetry integration installed in Datadog - [install from the integrations page](https://app.datadoghq.com/integrations?search=otel&integrationId=otel)

### Alternative: Apollo Runtime container

For a simpler deployment, consider using the [Apollo Runtime container](https://github.com/apollographql/apollo-runtime) which packages both the router and OpenTelemetry Collector together. See the [Datadog example configuration](https://github.com/apollographql/apollo-runtime/tree/main/examples/datadog/otel-collector) for ready-to-use configuration.
