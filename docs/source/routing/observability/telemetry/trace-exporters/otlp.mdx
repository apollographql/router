...
### `span_metrics`
Configuration for which spans are eligible for span stats collection in observability and monitoring tools. When enabled, spans are marked with the `_dd.measured` attribute and include a `span.kind` attribute.

By default, all built-in span names are enabled for span metrics:
* `request`
* `router`
* `supergraph`
* `subgraph`
...
You can override the defaults by explicitly configuring specific spans:

```yaml
span_metrics:
  router: true          # Enable metrics for router spans
  supergraph: false     # Disable metrics for supergraph spans
  custom_span: true     # Enable metrics for custom spans
```

To disable all span metrics, use an empty configuration:
...
| `span_metrics`    |                | All built-in span names enabled                                      | Configure which spans are eligible for span stats collection in observability tools. |
