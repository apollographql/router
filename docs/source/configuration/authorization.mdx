...
By default, when part of a query is filtered by authorization, the list of filtered paths is added to the response as top-level errors and logged by the router. Filtering parts of a query according to the client's rights may be seen as normal operation, and not warrant investigation by platform operators, so those logs can be disabled.
