...
For descriptions of these services, see [How router plugins work](./overview/#how-router-plugins-work).
...
impl Plugin for HelloWorld {
    ...
    async fn startup(&mut self) -> Result<(), BoxError> {
        ...
        // This is called before any requests are served.
        Ok(())
    }
    ...
    async fn shutdown(&mut self) -> Result<(), BoxError> {
        ...
        // No new requests are served after this is called.
        Ok(())
    }
    ...
    // being invoked. That's because this service might invoked *multiple*
...
Whenever the Apollo Router receives a request, it creates a corresponding `context` object and passes it along to each service. This object can store anything that is Serde-compatible (e.g., all simple types or a custom type).
...
When the router is ready to start serving requests, it calls each plugin's `startup` method. Plugins are started in the same order they're declared in your [YAML configuration file](../configuration/overview/#configuration-file).
...
When the router is ready to stop serving requests, it calls each plugin's `shutdown` method. Plugins are stopped in the opposite of the order in which they were started. During shutdown, plugin errors are logged and do not affect router execution.
