...
Proxy routing for GraphOS OTLP exporters also requires the HTTP transport. Set `otlp_tracing_protocol` and `otlp_metrics_protocol` to `http` in your router config. See [GraphOS reporting](/graphos/routing/observability/graphos/graphos-reporting#configuring-the-transport-protocol) for details. <MinVersionBadge version="Router v2.14.0" />
