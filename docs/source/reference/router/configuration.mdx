...
Starting with v1.59.0, the native query planner is GA and is run by default in the router with an ability to switch back to the now deprecated legacy implementation.

```yaml title="router.yaml"
experimental_query_planner_mode: legacy
```

The legacy implementation will be removed in the next release.

</Tip>

<Tip>

Previously, to run the most performant and resource-efficient native query planner and to disable the V8 JavaScript runtime in the router, set the following options in your `router.yaml`:
...
The legacy query planner still supports an experimental algorithm that attempts to
...
specific shape of fragments in an operation:
