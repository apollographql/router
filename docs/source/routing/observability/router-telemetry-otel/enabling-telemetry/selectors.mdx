...
The `http_client` service only supports the `request_header` and `response_header` selectors. Unlike other services, conditions and the `static` selector are **not** supported. Attempting to configure them causes a router startup failure.
