...
Trailing slashes are normalized automatically. For example, if the configured path is `/graphql`, the router accepts requests to both `/graphql` and `/graphql/`. This applies regardless of whether the configured path itself includes a trailing slash.
