...
use crate::plugins::telemetry::error_counter::{count_execution_errors, count_operation_errors, count_router_errors, count_subgraph_errors, count_supergraph_errors, unwrap_from_context};
...
use crate::services::{execution, router, subgraph, supergraph, RouterResponse};
...
async fn test_operation_errors_emitted_when_config_is_enabled() {
    async {
        ...
        let router_service = test_harness.supergraph_service(|req| async {
            let example_response = graphql::Response::builder()
                .data(json!({"data": null}))
