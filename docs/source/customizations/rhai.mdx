...
The router supports simple customization (via YAML configuration) and more complex customization (via a Rust Plugin Extension), but what if you want to do something slightly more complex than configuration, but don't want to write a Rust plugin. That's where our Rhai plugin steps in.

Rhai is a simple scripting language, [`Rhai`](http://rhai.rs), that provides basic capabilities. With Rhai it's easy to write simple scripts which manipulate strings, process headers and perform a variety of small ad-hoc programming tasks.

The Rhai scripting uses the same mechanisms as native Rust plugins and has access to the same life-cycle events.

## Rhai support is experimental

We are actively developing Rhai capability, and there is limited functionality right now.

What you can do:
* Simple manipulation of headers.

What you cannot do:
* Calls to external services.
* Modify the body or status codes of requests/responses.
* `checkpoint` style short circuiting of requests.

We will likely make breaking changes to the Rhai script functionality in the future. However, updating your scripts should not be a large task. 
