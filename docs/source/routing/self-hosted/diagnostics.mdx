---
title: Diagnostics (experimental)
...
The Router includes an experimental diagnostics plugin that collects system and memory information useful when diagnosing issues such as memory leaks or configuration problems. It has minimal overhead and can be enabled in production, but it exposes potentially sensitive information. Enable it only when needed and control access—for example, do not expose the diagnostics port on pods; use port-forwarding to access it instead.
...
Memory profiling helps diagnose **memory leaks** (e.g. growth over time after schema reload). It is **not** intended for diagnosing sudden OOMs or peak memory usage from normal allocations.

**Platform support:** Memory profiling (start, stop, dump, flamegraph, callgraph) works only on **Linux** with the jemalloc allocator. On other platforms (including macOS), the memory endpoints return "not supported."
