...
For Apollo Connectors, the set of client request headers that are folded into the cache key is an **explicit, required** configuration: `cache_key_headers`. Because a connector calls your REST API directly, the router cannot know which request headers cause your API to return a different response—so you must declare them. This field has no default: a connector cache configuration that omits it is rejected, so you cannot enable connector caching without first deciding which headers matter. Set it on `connector.all`, and on each source you list under `connector.sources`.
