...
You can pass the `--dev` flag to the `router` executable to enable plugins that assist with development. Currently, this flag enables [subgraph error inclusion](/subgraph-error-inclusion), and it might enable other plugins or modify other behavior in the future.

**Do not pass this flag when running the Apollo Router in production.**
