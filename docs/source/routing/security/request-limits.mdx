...
When a subgraph response is aborted because it exceeds the configured limit, the router increments the `apollo.router.limits.subgraph_response_size.exceeded` counter with a `subgraph.name` attribute identifying the affected subgraph. Monitor this counter to detect misconfigured limits or misbehaving subgraphs.
