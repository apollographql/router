...
rhai:
  ...
  # Set to false to disable string interning and eliminate lock contention
  # under concurrent load. See String interning and concurrent performance
  # under Limitations for details.
...
Router compiles Rhai with the [`sync` feature](https://rhai.rs/book/start/features.html) enabled, which makes the engine safe to share across threads. A side effect of this is that Rhai's string interner — which caches frequently-used strings to reduce allocations — is protected by a lock (namely a Rust `RwLock`). Under high concurrency, threads that encounter new strings must acquire a write lock, which serializes those operations across all concurrent requests.

String interning is enabled by default. You can disable interning with the `intern_strings` configuration option, which removes the write lock:

```yaml title="config.yaml"
rhai:
  scripts: "./rhai"
  main: "main.rhai"
  intern_strings: false  # disable interning
```

<Note>

Considerations for disabling string interning:
- Eliminating `RwLock` contention on the string interner can meaningfully improve throughput for deployments processing many concurrent requests through Rhai scripts.
- Without interning, repeated identical strings are each allocated separately rather than sharing a single allocation. This increases memory pressure and allocation frequency, which might be noticeable in scripts that produce a high volume of repeated short strings.
- Without interning, string equality comparisons lose the interning fast path (pointer equality) and must compare bytes. This effect is unlikely to be noticeable in most scripts.
