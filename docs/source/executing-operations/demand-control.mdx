...
Application clients may overload your supergraph infrastructure by sending overly costly operations. These operations may be costly due to their complexity, or they may be simple operations run against a particularly expensive resolver. In either case, demand control can help you protect your infrastructure from these expensive operations. When your router receives a request, it calculates a cost for that operation. If the cost is greater than your configured maximum, the operation is rejected.

## Calculating cost

When calculating the cost of an operation, your router will sum the costs of the sub-requests that it plans to send to your subgraphs. For each operation, the cost is the sum of its base cost plus the costs of its fields. For each field, the cost is defined recursively as its own base cost plus that of its selections. In the IBM specification, this is called [field cost](https://ibm.github.io/graphql-specs/cost-spec.html#sec-Field-Cost).
...
The Apollo Federation [@cost directive](/federation/federated-schemas/federated-directives/#cost) differs the IBM specification in that the `weight` argument is of type `Int!` instead of `String!`.

</Note>

Annotating your schema with this directive alters how the router scores operations. Say the `Address` resolver for our example query is particularly expensive. We can annotate the schema with the following directive.
...
During the static analysis phase, the router does not know the size of the list fields in a given query. The closer the estimated list size is to the actual list size for a field, the closer the estimated cost will be to the actual cost.
...
There are two ways to indicate the expected list sizes to the router. The first is to set the global maximum in your router configuration file (see [Configuring demand control](#configuring-demand-control)).

The second is to use the Apollo Federation [@listSize directive](/federation/federated-schemas/federated-directives/#listsize). This directive supports field-level granularity in setting list size. By using the `assumedSize` argument, you can set a statically-defined list size for a field. If you are using paging parameters which control the size of the list, use the `slicingArguments` argument.
...
Record the estimated cost when `cost.result` is `COST_ESTIMATED_TOO_EXPENSIVE`.
