---
title: Export traces to Datadog
description: Collect and export tracing information from the Apollo Router
---

Enable and configure the [Datadog](https://www.datadoghq.com/) exporter endpoint in the Apollo Router.
For general tracing configuration, refer to [Router Tracing Configuration](./overview).

## OTLP configuration

To export traces to Datadog via OTLP, you must both 

- Configure the Datadog agent to accept OTLP traces, and
...
To configure the router, enable OTLP metrics and set both `temporality: delta` and `endpoint: <datadog-agent-endpoint>`. For example: 
...
1. Use the Open Telemetry span name to set the Datadog span operation name.
