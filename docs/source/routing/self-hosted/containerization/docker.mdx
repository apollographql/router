...
=======
The exact image version to use depends on which release you wish to use. In the following examples, replace `<image version>` with your chosen version, additional details on version information is available [here](https://github.com/apollographql/apollo-runtime?tab=readme-ov-file#container-tags).
>>>>>>> 22ddee3fa (Fill out the rest of the docs areas)
...
=======
You can provide your own configuration from the host environment to the router by mounting the path which containers your configuration files to `/config` as follows:
>>>>>>> 22ddee3fa (Fill out the rest of the docs areas)
