...
Adding `experimental_local_manifests` to your `persisted_queries` configuration lets you use local persisted query manifests instead of the hosted Uplink version. This is helpful when you're using an offline Enterprise license and can't use Uplink. With the `experimental_local_manifests` option, the router doesn't reload the manifest from the file system, so you need to restart the router to apply changes.
