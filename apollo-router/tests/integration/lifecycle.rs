...
async fn test_cli_router_info() {
    let output = Command::new(IntegrationTest::router_location())
        ...
        .unwrap();
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(output.status.success(), "router info should succeed: stderr={}", String::from_utf8_lossy(&output.stderr));
    assert!(stdout.contains("Router version:"), "output should contain version: {}", stdout);
    assert!(stdout.contains("OS / architecture:"), "output should contain OS/arch: {}", stdout);
    assert!(stdout.contains("Environment variables set:"), "output should contain env section: {}", stdout);
