...
Configure every intermediate hop between the client and the router to enable long-lived streaming responses. Subscription replies aren't one-time HTTP responses. Any proxy, CDN, gateway, or firewall that buffers responses or enforces a short idle timeout closes the connection before the client receives subscription events.

A subscription request from a client takes a single HTTP POST to the router, but the router holds the connection open and pushes events back as a multipart response (or over the established WebSocket, depending on the subgraph protocol). Every hop in that path needs three things:

- Streaming, unbuffered responses
- Idle timeouts long enough to cover the subscription's lifetime
- Egress allowed back to the client domain

A reverse proxy that buffers the full response before forwarding it (the default for many proxies) holds subscription events until the connection is forcibly closed. The client then sees a 504 with no payload. Configure each proxy in the path to forward chunked responses without buffering.

Idle timeouts shorter than the subscription's lifetime close the client connection before any keepalive or heartbeat arrives. The default on most CDNs and load balancers is 30 to 60 seconds. For subscriptions intended to run for minutes or hours, extend the idle timeout on every hop to at least the [`max_lifetime` configuration value](#limit-subscription-lifetime).

When a corporate firewall or VPC security group restricts egress, the router still accepts the subscription request but can't push events back to the client. Clients see a 504 timeout with no diagnostic information at the router level. Verify that the path from the router back to the client domain is open for streaming responses.

If clients receive 504 errors on subscriptions and the [`apollo.router.operations.subscriptions.terminated.client` metric](#tracking-subgraph-connection-failures-and-closures) doesn't show a corresponding event, an intermediary is dropping the connection, not the router. Check the proxy, gateway, and firewall logs first. The router can't record a client disconnect it never observed.

When all hops are configured to enable streaming responses and the idle timeouts cover the expected subscription lifetime, subscription events flow end-to-end without 504s.
