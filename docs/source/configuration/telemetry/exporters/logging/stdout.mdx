...
Log messages ca become too frequent, especially when reporting errors when processing a request. To prevent the router from filling the logs with similar messages, it is possible to limit their rate. The rate is set per log location, which means that two different log messages are limited independently, but log lines with the same message with different sets of attributes fal l under the same rate. This rate limiting only applies to logging to stdout, and does not affect event sent to trace exporters like OTLP, which have their own sampling configuration.
    ...
    logging:
      stdout:
        ...
        rate_limit: #highlight-line
          count: 1     # number of allowed messages during the rate limiting interval
          interval: 3s
```
