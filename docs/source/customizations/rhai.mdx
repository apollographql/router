...
Similar to native Rust plugins, Rhai plugins can hook into the Apollo Router's [four services](./overview/#how-router-plugins-work) that handle requests. _Unlike_ native Rust plugins, Rhai plugins use two different hooks for each service, one for handling request data and one for handling response data:
