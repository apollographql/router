...
If omitted, Apollo Studio receives all spans that pass the common sampler.
...
Enable richer error reporting via `extended_error_metrics` and `redaction_policy` router configurations. Extended error metrics are enabled by default; set `extended_error_metrics: disabled` to opt out.
