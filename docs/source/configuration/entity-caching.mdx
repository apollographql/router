...
Entity caching provides multiple ways to invalidate data:
- an invalidation endpoint exposed by the router, that can receive invalidation requests from any authorized service out of band. This can be used by the subgraph to asynchronously trigger invalidation events, or by the platform operator to maintain the caching levels
- invalidation requests can be sent through subgraph response extensions, allowing the subgraph to invalidate cached data right after a mutation

Invalidation requests can be used to invalidate a large number of cached entries at once, matching either:
...
An invalidation request is processed first by sending a `SCAN` command to Redis to find all the keys that match the invalidation request. Upon iterating over the scan cursor, the router will send a `DEL` command to Redis to remove the matching keys.

#### Configuration

Invalidation can be configured both globally, and overriden per subgraph.
...
Invalidation requests are sent as JSON objects with the following format:
...
This is the same key format used by the router in the `representations` field for `_entities` queries.

#### Invalidation HTTP endpoint

The invalidation endpoint exposed by the router can receive multiple invalidation requests at once and will process them in sequence. A shared key can be provided in the request as authorization. As an example, with the precedent configuration, we could send the following request:
...
- `apollo.router.operations.entity.invalidation.entry`, counter measuring how many entries are removed per `DEL` call. It has a label `origin` that can be either `endpoint` or `extensions`, and a label `subgraph.name` with the name of the subgraph that the request is for
...
- `apollo.router.cache.invalidation.duration`, histogram measuring the time spent handling one invalidation requestt

It is also reported under the following spans:
...
Entity caching will greatly reduce traffic to subgraphs, resulting in smaller infrastructure needed to serve more client traffic. In the rare cases where Redis is unavailable, the router should be configured with rate limiting for subgraph requests, to avoid overwhelming the subgraphs. It can also be paired with subgraph query deduplication to further reduce traffic.
