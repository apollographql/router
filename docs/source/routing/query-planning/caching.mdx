...
GraphOS Router enables query plan caching by default using an in-memory cache backed by a W-TinyLFU (Window TinyLFU) eviction policy. In your router's [YAML config file](/graphos/routing/configuration/overview#yaml-config-file), you can configure the maximum number of query plan entries in the cache:
