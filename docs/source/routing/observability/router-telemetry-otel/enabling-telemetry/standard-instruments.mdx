...
- `apollo.router.limits.connector_mapping_errors.exceeded` - A counter of connector responses whose declared mapping errors were truncated because they exceeded the configured `max_mapping_errors` limit under `limits.connector`. The errors a mapping declares with `->withError` are themselves counted by `apollo.router.operations.errors`, like any other error.
  - `connector.source`: The source key (`subgraph_name.source_name`) whose errors were truncated.
