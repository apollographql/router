...
subtitle: Customize the Redis cache key for subgraph request
...
This entry contains an object with the <code>all</code> field to affect all subgraph requests under one client request, and fields named after subgraph operation names to affect individual subgraph queries. The field's value can be any valid JSON value, such as an object or a string.
