...
fn validate_tag_propagation(supergraph: &Supergraph<Satisfiable>) {
    ...
    if let Some(query_type_name) = &schema.schema_definition.query {
        if let Some(ExtendedType::Object(query_obj)) = schema.types.get(query_type_name.as_str()) {
            if let Some(users_field) = query_obj.fields.get("users") {
                let tag_directives: Vec<_> = users_field.directives.iter()
                    .filter(|d| d.name == "tag")
                    .collect();
                assert!(!tag_directives.is_empty(), "Expected @tag directive on Query.users field");
                
                // Check for the specific tag name "aTaggedOperation"
                let has_operation_tag = tag_directives.iter().any(|d| {
                    d.arguments.iter().any(|arg| {
                        arg.name == "name" && arg.value.to_string().contains("aTaggedOperation")
                    })
                });
                assert!(has_operation_tag, "Expected @tag(name: \"aTaggedOperation\") on Query.users");
            }
        }
    }
    ...
    if let Some(ExtendedType::Object(user_type)) = schema.types.get("User") {
        let tag_directives: Vec<_> = user_type.directives.iter()
            .filter(|d| d.name == "tag")
            .collect();
        assert!(!tag_directives.is_empty(), "Expected @tag directive on User type");
        
        // Check for the specific tag name "aTaggedType"
        let has_type_tag = tag_directives.iter().any(|d| {
            d.arguments.iter().any(|arg| {
                arg.name == "name" && arg.value.to_string().contains("aTaggedType")
            })
        });
        assert!(has_type_tag, "Expected @tag(name: \"aTaggedType\") on User type");
        
        // Check for @tag directive on User.name field
        if let Some(name_field) = user_type.fields.get("name") {
            let field_tag_directives: Vec<_> = name_field.directives.iter()
                .filter(|d| d.name == "tag")
                .collect();
            assert!(!field_tag_directives.is_empty(), "Expected @tag directive on User.name field");
            
            // Check for the specific tag name "aTaggedField"
            let has_field_tag = field_tag_directives.iter().any(|d| {
                d.arguments.iter().any(|arg| {
                    arg.name == "name" && arg.value.to_string().contains("aTaggedField")
                })
            });
            assert!(has_field_tag, "Expected @tag(name: \"aTaggedField\") on User.name field");
        }
    }
}
...
fn validate_tag_merging(supergraph: &Supergraph<Satisfiable>) {
    ...
    if let Some(ExtendedType::Object(user_type)) = schema.types.get("User") {
        ...
        assert!(tag_directives.len() >= 2, "Expected multiple @tag directives on User type, got {}", tag_directives.len());
        
        // Extract tag names for validation
        let tag_names: Vec<String> = tag_directives.iter()
            .filter_map(|d| {
        ...
        assert!(tag_names.contains(&"aTagOnTypeFromSubgraphA".to_string()), "Missing aTagOnTypeFromSubgraphA");
        assert!(tag_names.contains(&"aMergedTagOnType".to_string()), "Missing aMergedTagOnType"); 
        assert!(tag_names.contains(&"aTagOnTypeFromSubgraphB".to_string()), "Missing aTagOnTypeFromSubgraphB");
    }
    
    // Check merged tags on Name.firstName field
    if let Some(ExtendedType::Object(name_type)) = schema.types.get("Name") {
        if let Some(first_name_field) = name_type.fields.get("firstName") {
            let field_tag_directives: Vec<_> = first_name_field.directives.iter()
                .filter(|d| d.name == "tag")
                .collect();
            
            assert!(field_tag_directives.len() >= 2, "Expected multiple @tag directives on Name.firstName field");
        }
    }
}
...
fn tag_rejects_tag_and_external_together_fed2_subgraphs() {
    ...
    assert_composition_errors(&result, &[
        ("MERGED_DIRECTIVE_APPLICATION_ON_EXTERNAL", r#"[subgraphA] Cannot apply merged directive @tag(name: "myTag") to external field "User.birthdate""#),
    ]);
}
...
fn tag_errors_if_imported_under_mismatched_names() {
    ...
    let result = compose_as_fed2_subgraphs(&[subgraph_a, subgraph_b]);
    assert_composition_errors(&result, &[
        ("LINK_IMPORT_NAME_MISMATCH", r#"The "@tag" directive (from https://specs.apollo.dev/federation/v2.0) is imported with mismatched name between subgraphs: it is imported as "@tag" in subgraph "subgraphB" but "@apolloTag" in subgraph "subgraphA""#),
    ]);
}
...
fn tag_succeeds_if_imported_under_same_non_default_name() {
    ...
    let result = compose_as_fed2_subgraphs(&[subgraph_a, subgraph_b]);
    let _supergraph = result.expect("Expected composition to succeed with consistent @tag import names");
}