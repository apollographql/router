...
#[test]
fn one_of_input_object_merges_when_consistent() {
    let subgraph_a = ServiceDefinition {
        name: "subgraphA",
        type_defs: r#"
        type Query {
...
#[test]
fn one_of_input_object_errors_when_inconsistent() {
    let subgraph_a = ServiceDefinition {
        name: "subgraphA",
        type_defs: r#"
        type Query {
    ...
    assert_composition_errors(
        &result,
        &[(
            "INPUT_OBJECT_ONE_OF_MISMATCH",
            r#"Input object type "FindInput" is marked with @oneOf in subgraph "subgraphA" but not in subgraph "subgraphB""#,
        )],
    );
}
