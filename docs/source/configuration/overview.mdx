...
Most environment varialbes are also documented with their [Command-line options](#command-line-options) counterpart. However there are some options that are only available to configure as environment variables.

To set environment variables, make sure you include them when starting the router binary or they are setup with your infrastructure configuration
```bash
...
<table class="field-table api-ref">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
      The starting size of query planner heap to use in MB. Setting `APOLLO_ROUTER_BRIDGE_EXPERIMENTAL_V8_INITIAL_HEAP_SIZE=4096` is ~4GB.
