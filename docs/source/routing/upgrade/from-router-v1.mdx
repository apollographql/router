...
In v2.x, this flag is renamed to `otlp_tracing_sampler`, and it's enabled by default.

**Upgrade step**: if your router config sets `experimental_otlp_tracing_sampler`, rename it to `otlp_tracing_sampler`. The `router config upgrade` command does this for you.
