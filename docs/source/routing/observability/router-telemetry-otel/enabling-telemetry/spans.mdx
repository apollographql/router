...
`http_client` span attributes support the `request_header` and `response_header` selectors. Use these selectors to configure your attributes. Attempting to use conditions or the `static` selector causes a router startup failure with an error similar to:
