...
## Customizing along the request lifecycle
...
Understand the entire request lifecycle by following flowcharts of its [request path](/graphos/routing/request-lifecycle#request-path) and [response path](/graphos/routing/request-lifecycle#response-path), starting from a client request all the way to your subgraphs, and all the way back from subgraph responses to a client response.
