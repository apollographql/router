...
| -------------- | ------------ | ------------ | -------------- | ---------------- | ---------------- |
| Active    | July 2024  | v{products["apollo-router"].latest().version.withPatch(0).toString()}  |  February 2025 | -     | -     |
...
To upgrade to the latest router v{products["apollo-router"].latest().version.toString()}, including resolving breaking changes and using new capabilities, follow the [router 1.x to 2.x upgrade guide](/graphos/reference/upgrade/from-router-v1).
