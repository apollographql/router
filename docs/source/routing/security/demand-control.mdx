...
| `static_estimated.actual_cost_mode`               | `by_subgraph`, `legacy` | `by_subgraph` | `by_subgraph` calculates the cost of an operation as the sum of the cost of each subgraph response; `legacy` calculates the cost based on only the final shape of the response |
