...
This includes the errors a connector's response mapping declares with `->withError`, which clients read in `extensions.connectorErrors` rather than in `errors`. Their `service` is the connector's subgraph, and their `code` is the one the mapping author supplied. They are reported here and not by `apollo.router.graphql_error`, because the field each one describes resolved: the response carries data at that position, not a GraphQL error. They are counted at the connector, so a subgraph's `send: false` withholds them from Studio, but `include_subgraph_errors` withholding one from a *client* does not suppress its metric.
