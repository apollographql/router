...
If your organization uses a transparent or egress proxy (such as Zscaler or Netskope), the recommended approach is to [add the proxy's root CA certificate to your container image](/graphos/routing/self-hosted/containerization/proxy-certificates). Adding this certificate enables Apollo Router to establish TLS connections to Apollo Uplink and helps prevent certificate validation errors or `invalid license` failures.
