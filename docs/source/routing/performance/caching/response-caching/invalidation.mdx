...
For cache tags on _entities_ we set `apolloEntityCacheTags` in `extensions`. Example of response payload if you want to set cache tags for different entities returned by subgraph:
...
For cache tags on _root fields_ we set `apolloCacheTags` in `extensions`. Example of response payload if you want to set cache tags for root fields returned by subgraph:
