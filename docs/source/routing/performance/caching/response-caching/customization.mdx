...
<Note>
    Response caching happens at the subgraph service level, after Rhai scripts, WebAssembly plugins, and coprocessors. The router therefore invokes those customizations even when it serves the subgraph response from the cache.
</Note>
