...
Because `.sock` isn't required as a file extension, the router can't tell if a path has been appended to the base URL. You can specify a custom HTTP path using the `path` query parameter:
...
This also works for subgraph URLs defined directly in the supergraph schema (via the `url` argument of the `@join__graph` directive).
