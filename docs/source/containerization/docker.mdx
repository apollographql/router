...
Note: The exact image version to use is your choice depending on which release you wish to use. In the following examples, replace `<image version>` with your chosen version. e.g.: `v0.9.3`
...
Usage: build_docker_image.sh [-b] [<release>]
	...
	Example 2: Building tag from the repo
		build_docker_image.sh -b v0.9.3
	Example 3: Building commit hash from the repo
		build_docker_image.sh -b 7f7d223f42af34fad35b898d976bc07d0f5440c5
	Example 4: Building tag v0.9.3 from the released tarball
		build_docker_image.sh v0.9.3
```
