...
If your router requires requests to [a device or service on a private network](https://wicg.github.io/private-network-access/), you need to enable this behavior in your CORS configuration. Without this change, the browser will not allow any requests to be sent to a device on the private network.

To enable private network access with CORS, your router must return the [`Access-Control-Allow-Private-Network`](http://wicg.github.io/private-network-access/#http-headerdef-access-control-allow-private-network) HTTP header with a value of `true`.

You can enable private network access by including `private_network_access` in the policies of your `cors` config. Although optional, specifying the `access_id` and `access_name` subfields clarifies what in the private network is being accessed when the browser prompts the user for permission.
