...
Selectors are used to extract data from the Apollo Router's request lifecycle (pipeline) services and attach them to telemetry, specifically [spans](spans.mdx), [instruments](instruments.mdx), [events](events.mdx), and [conditions](conditions.mdx).
