...
fn flatten_inputs_graphql_name(inputs: &[Selection]) -> HashSet<String> {
    inputs
        .iter()
        .flat_map(|s| match s {
            Selection::Field(f) => {
            ...
            Selection::InlineFragment(f) => flatten_inputs(&f.selection_set),
...
fn flatten_inputs(inputs: &[Selection]) -> HashSet<String> {
    inputs
        .iter()
        .flat_map(|s| match s {
            Selection::Field(f) => {
