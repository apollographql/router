...
By default, the `traffic_shaping` plugin is enabled with [preset values](#link-to-default-values). To override presets, add `traffic_shaping` to your [YAML config file](./overview/#yaml-config-file) like so:
