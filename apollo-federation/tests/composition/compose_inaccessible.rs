...
fn validate_inaccessible_propagation(supergraph: &Supergraph<Satisfiable>) {
    ...
    if let Some(query_type_name) = &schema.schema_definition.query {
        if let Some(ExtendedType::Object(query_obj)) = schema.types.get(query_type_name.as_str()) {
            if let Some(me_field) = query_obj.fields.get("me") {
                let inaccessible_directives: Vec<_> = me_field.directives.iter()
                    .filter(|d| d.name == "inaccessible")
                    .collect();
                assert!(!inaccessible_directives.is_empty(), "Expected @inaccessible directive on Query.me field");
            }
        }
    }
    ...
    if let Some(ExtendedType::Object(user_type)) = schema.types.get("User") {
        if let Some(age_field) = user_type.fields.get("age") {
            let inaccessible_directives: Vec<_> = age_field.directives.iter()
                .filter(|d| d.name == "inaccessible")
                .collect();
            assert!(!inaccessible_directives.is_empty(), "Expected @inaccessible directive on User.age field");
        }
    }
}
...
fn validate_inaccessible_merging(supergraph: &Supergraph<Satisfiable>) {
    ...
    if let Some(ExtendedType::Object(user_type)) = schema.types.get("User") {
        if let Some(name_field) = user_type.fields.get("name") {
            let inaccessible_directives: Vec<_> = name_field.directives.iter()
                .filter(|d| d.name == "inaccessible")
                .collect();
            assert!(!inaccessible_directives.is_empty(), "Expected @inaccessible directive on User.name field");
        }
    }
}
...
fn inaccessible_rejects_inaccessible_and_external_together() {
    ...
    assert_composition_errors(&result, &[
        ("MERGED_DIRECTIVE_APPLICATION_ON_EXTERNAL", r#"[subgraphA] Cannot apply merged directive @inaccessible to external field "User.birthdate""#),
    ]);
}
...
fn inaccessible_errors_if_imported_under_mismatched_names() {
    ...
    let result = compose_as_fed2_subgraphs(&[subgraph_a, subgraph_b]);
    assert_composition_errors(&result, &[
        ("LINK_IMPORT_NAME_MISMATCH", r#"The "@inaccessible" directive (from https://specs.apollo.dev/federation/v2.0) is imported with mismatched name between subgraphs: it is imported as "@inaccessible" in subgraph "subgraphB" but "@private" in subgraph "subgraphA""#),
    ]);
}
...
fn inaccessible_succeeds_if_imported_under_same_non_default_name() {
    ...
    let result = compose_as_fed2_subgraphs(&[subgraph_a, subgraph_b]);
    let _supergraph = result.expect("Expected composition to succeed with consistent @inaccessible import names");
}
...
fn inaccessible_errors_if_subgraph_misuses_inaccessible() {
    ...
    let result = compose_as_fed2_subgraphs(&[subgraph_a, subgraph_b]);
    assert_composition_errors(&result, &[
        ("INVALID_INACCESSIBLE_USAGE", r#"Field "User.name" is marked @inaccessible in subgraph "subgraphA" but not in subgraph "subgraphB""#),
    ]);
}
...
fn inaccessible_uses_security_core_purpose_in_supergraph() {
    ...
    let schema = supergraph.schema().schema();
    let has_security_purpose_link = schema
        .schema_definition
        .directives
        .iter()
        .any(|d| {
