...
Using other features of [Kubernetes lifecycle hooks](https://kubernetes.io/docs/concepts/containers/container-lifecycle-hooks/), you can configure the [health checks](/graphos/routing/self-hosted/health-checks) to make sure all pods are shutdown safely. It may also be wise to consider setting the shut down grace period to be slightly longer than any  configured [router timeouts](/graphos/routing/performance/traffic-shaping).
