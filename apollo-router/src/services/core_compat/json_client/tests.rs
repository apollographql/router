...
async fn test_core_json_request_to_subgraph_request_conversion() {
    ...
    // Convert to subgraph request using From trait
    let subgraph_request: SubgraphRequest = core_request.into();
    ...
    let context_value = subgraph_request.context.get::<_, String>("test_key").unwrap().unwrap();
    assert_eq!(context_value, "test_value");
    
    // Verify the GraphQL request was deserialized correctly
    assert_eq!(subgraph_request.subgraph_request.body().query.as_ref().unwrap(), "{ hello }");
    assert_eq!(subgraph_request.subgraph_request.body().operation_name.as_ref().unwrap(), "TestOperation");
}
...
async fn test_subgraph_request_to_core_json_request_conversion() {
    ...
    // Convert to core JSON request using Into trait
    let core_request: CoreJsonRequest = subgraph_request.into();
    
    // Verify the JSON body contains the GraphQL request
    let query = core_request.body.get("query").and_then(|v| v.as_str()).unwrap();
    assert_eq!(query, "{ user(id: \"123\") { name } }");
    
    let operation_name = core_request.body.get("operationName").and_then(|v| v.as_str()).unwrap();
    assert_eq!(operation_name, "GetUser");
    
    // Verify context is preserved in extensions
    let context = core_request.extensions.get::<Context>().unwrap();
...
async fn test_subgraph_response_to_core_json_response_conversion() {
    ...
    // Convert to core JSON response using Into trait
    let core_response: CoreJsonResponse = subgraph_response.into();
    
    // Verify context is preserved in extensions
    let context = core_response.extensions.get::<Context>().unwrap();
    ...
    let user_name = data.get("user").unwrap().get("name").unwrap().as_str().unwrap();
...
async fn test_round_trip_request_conversion() {
    ...
    context.insert("user_roles", vec!["admin".to_string(), "user".to_string()]).unwrap();
    extensions.insert(context);
    ...
    let original_query = original_request.body.get("query").unwrap().as_str().unwrap().to_string();
    let original_amount = original_request.body.get("variables").unwrap().get("amount").unwrap().as_f64().unwrap();
    
    // Round trip: Core -> Subgraph -> Core
    let subgraph_request: SubgraphRequest = original_request.into();
    let final_request: CoreJsonRequest = subgraph_request.into();
    ...
    let final_amount = final_request.body.get("variables").unwrap().get("amount").unwrap().as_f64().unwrap();
    assert_eq!(final_amount, original_amount);
    
    let operation_name = final_request.body.get("operationName").unwrap().as_str().unwrap();
    assert_eq!(operation_name, "CreatePayment");
    
    // Verify context data is preserved
    let context = final_request.extensions.get::<Context>().unwrap();
    let trace_id = context.get::<_, String>("trace_id").unwrap().unwrap();
...
async fn test_round_trip_response_conversion() {
    ...
    // Store reference values for comparison
    let _original_user_name = original_response.response.body().data.as_ref().unwrap()
        .get("user").unwrap().get("name").unwrap().as_str().unwrap().to_string();
    let original_execution_time = original_response.context.get::<_, u64>("execution_time").unwrap().unwrap();
    
    // Round trip: Subgraph -> Core -> Subgraph
    let core_response: CoreJsonResponse = original_response.into();
    let final_response: SubgraphResponse = core_response.into();
    
    // Verify HTTP properties are preserved (though some defaults are applied)
    assert_eq!(final_response.response.status(), 200);
    
    // Verify context data is preserved
    let execution_time = final_response.context.get::<_, u64>("execution_time").unwrap().unwrap();
    assert_eq!(execution_time, original_execution_time);
    
    let cache_hit = final_response.context.get::<_, bool>("cache_hit").unwrap().unwrap();
