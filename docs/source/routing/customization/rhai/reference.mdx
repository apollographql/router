...
The `router_http` hook runs at the raw HTTP layer before the Router pipeline; it receives the same `service` object and supports `map_request` and `map_response` like the other hooks.
