...
Histogram metrics (such as `http.server.request.duration`) use bucket boundaries to group measurement values. Configure these buckets to accurately monitor request durations and timeout behavior.
...
Histogram metrics cap values at the highest bucket boundary. With the default buckets, any request taking longer than ten seconds is counted in the `10.0+` bucket. This can make it appear that timeouts aren't being respected in your metrics, especially if you have configured timeouts greater than ten seconds.
