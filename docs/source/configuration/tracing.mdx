...
* [OpenTelemetry Protocol (OTLP)](https://github.com/open-telemetry/opentelemetry-proto/blob/main/docs/specification.md) over HTTP or gRPC
...
You can configure the sampling rate of traces to match the rate of your application performance monitors (APM). To enable sampling configuration, in [`router.yaml`](./configuration/overview#yaml-config-file) set `telemetry.tracing.common.sampler` and `telemetry.tracing.common.parent_based_sampler`:
