...
You can control the coprocessor client configuration as needed.  At the moment only experimental_http2 is supported.

For example to enable h2c (http2 cleartext) communication:

```yaml title="router.yaml"
coprocessor:
  url: http://127.0.0.1:8081 # When using an http (not https) url and http2only will result in connections that use h2c
  client:
