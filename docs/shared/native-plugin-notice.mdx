...
The recommended approach for router customizations is to use a [Rhai script](https://www.apollographql.com/docs/graphos/routing/customization/rhai/) or a [WebAssembly plugin](https://www.apollographql.com/docs/graphos/routing/customization/wasm/). For customizations that need network or filesystem access, use an [external coprocessor](https://www.apollographql.com/docs/router/customizations/coprocessor). These options provide separation of concerns without requiring a custom router binary.
