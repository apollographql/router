...
- If heartbeat is enabled (`heartbeatIntervalMs` > 0). Every `heartbeatIntervalMs` milliseconds (value coming from the initial payload sent in `extensions` from the **Router**), **Emitter** must send a [`check` message](#check) to **Router** to confirm that **Router** is still listening.
