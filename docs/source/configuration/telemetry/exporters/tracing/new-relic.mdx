...
Enable and configure the [OpenTelemetry](https://www.opentelemetry.io/) exporter for tracing in the Apollo Router for use with [New Relic](https://newrelic.com/).

For general tracing configuration, refer to [Router Tracing Configuration](./overview).

## New Relic configuration

To configure the router, enable [OpenTelemetry tracing exporter](./opentelemetry#configuration) and set both `endpoint: <new-relic-endpoint>` and `api-key: <new-relic-api-key>` for example: 
