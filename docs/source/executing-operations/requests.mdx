...
If you are using the Automatic Persisted Queries or Persisted Queries/Safelisting features of Apollo Router they use a separate protocol to send the operation document infomation in the `extensions`. This protocal can also use HTTP POST or GET. See the Apollo Client docs on the [APQ protocol](https://www.apollographql.com/docs/react/api/link/persisted-queries/#protocol) for details.
