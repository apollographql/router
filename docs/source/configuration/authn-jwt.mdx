...
By enabling JWT authentication, you can block malicious traffic at the _edge_ of your graph instead of relying on [header forwarding](/router/configuration/header-propagation/) to propagate tokens to your subgraphs.
...
Explicitly listing claims and _always_ setting headers for them is strongly recommended to avoid possible security issues when [forwarding headers](/router/configuration/header-propagation/) to subgraphs.
...
If you _do_ need to pass entire JWTs to subgraphs, you can do so via the GraphOS Router's general-purpose [HTTP header propagation settings](/router/configuration/header-propagation).
