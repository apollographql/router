...
Limit the total size of the header list of incoming HTTP/2 requests.
The default value is `16KiB`.

If router receives a request whose headers exceed the allowed size, it responds to the client with `431 Request Header Fields Too Large`.
