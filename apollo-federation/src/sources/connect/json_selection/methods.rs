...
// JSONSelection strings in connector schemas, but are implemented and tested.
// Before any of these methods graduates to public status, it should be reviewed
// carefully and then moved to the top level of the methods.rs file (out of the
// future:: namespace).
mod future {
    use apollo_compiler::collections::IndexSet;
    ...
    pub(super) fn eq_method(
        ...
        if let Some(MethodArgs(args)) = method_args {
            if args.len() == 1 {
                let matches =
                    if let Some(value) = args[0].apply_to_path(data, vars, input_path, errors) {
                        data == &value
                    } else {
                        false
                    };
