...
- `algorithms`: **optional** list of accepted algorithms. Possible values are HS2356, HS384, HS512, ES256, ES384, RS256, RS384, RS512, PS256, PS384, PS512, EdDSA
...
Below is an example [Rhai script](../customizations/rhai/) that rejects requests that don't present a valid JWT.

> This script hooks into the router's `SupergraphService`, which executes before the router begins generating the query plan for an operation. [Learn more about router services.](../customizations/rhai#router-request-lifecycle)
...
> This script hooks into the router's `SubgraphService`, which executes before the router sends a subquery to an individual subgraph. [Learn more about router services.](../customizations/rhai#router-request-lifecycle)
...
Below is an example [Rhai script](../customizations/rhai/) that throws distinct errors for different invalid JWT claim details.
