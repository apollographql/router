...
macro_rules! assert_plan {
    ($planner: expr, $operation: expr, $options: expr, @$expected: literal) => {{
    ...
    ($api_schema_and_planner: expr, $operation: expr, @$expected: literal) => {{
        assert_plan!($api_schema_and_planner, $operation, Default::default(), @$expected)
    }};
}
...
pub(crate) fn test_planner(
    ...
    QueryPlanner::new(&supergraph, config.clone()).expect("can create query planner")
}
