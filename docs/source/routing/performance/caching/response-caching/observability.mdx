...
| `apollo.router.operations.response_cache.cdn_tag_header.untruncated_size_bytes` | Size the header would have been without `max_bytes` truncation, recorded whenever there's at least one label to report. Use this to see how much headroom you have before truncation starts happening. | `By` |
...
- **Outcome**: whether the `Cache-Tag` header was empty (nothing to report), sent complete, truncated to fit `max_bytes`, or dropped entirely due to `experimental_drop_on_overflow`
