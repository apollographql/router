...
As part of the effort to ensure correctness and stability of the new query planner, starting in v1.53.0 the router can enable both the new and legacy planners and run them in parallel to compare their results. After their comparison, the router discards the native query planner's results and uses only the legacy planner to execute requests.
...
## Configure native query planning

To maximize performance and minimize resource utilization of query planning, you should run your router with the following options to enable completely native query planning:
