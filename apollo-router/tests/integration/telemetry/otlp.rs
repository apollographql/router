...
async fn validate_telemetry(
    ...
    Err(anyhow!("Could not get traces and metrics").into())
}
...
async fn find_valid_trace(
    ...
    if let Some(trace) = requests.iter().find(|r| r.url.path().ends_with("/traces")) {
        let trace =
            opentelemetry_proto::tonic::collector::trace::v1::ExportTraceServiceRequest::decode(
                bytes::Bytes::copy_from_slice(&trace.body),
            )?;
        let json_trace = serde_json::to_value(trace)?;
        //println!("{}", serde_yaml::to_string(&json_trace)?);
