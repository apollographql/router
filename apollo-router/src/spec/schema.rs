...
use apollo_federation::sources::connect::expand_connectors;
use apollo_federation::sources::connect::Connector;
use apollo_federation::sources::connect::ExpansionResult;
use http::Uri;
...
impl Schema {
    ...
    pub(crate) fn parse(sdl: &str, config: &Configuration) -> Result<Self, SchemaError> {
        ...
        let expansion =
            expand_connectors(sdl).map_err(|_| SchemaError::Connector("TODO".to_string()))?;
        ...
        if let Some(join_enum) = definitions.get_enum("join__Graph") {
            for (name, url) in join_enum.values.iter().filter_map(|(_name, value)| {
                ...
                let url = if is_connector {
                    Uri::from_static("http://placeholder")
                } else {
                    if url.is_empty() {
