// NOTE: this is copy-pasted from src/query_plan/display.rs and modified for th
// different fetch node structure.
    ...
    fn write_indented(&self, state: &mut State<'_, '_>) -> fmt::Result {
        ...
        match source_data {
            source::query_plan::FetchNode::Graphql(fetch) => {
                let subgraph_name = fetch.source_id.subgraph_name.clone();
                state.write(format_args!("Fetch(service: {subgraph_name:?}"))?
            }
            source::query_plan::FetchNode::Connect(fetch) => {
                let label = fetch.source_id.label.clone();
                state.write(format_args!("Fetch(service: {label:?}"))?
            }
        };
        // state.write(format_args!("Fetch(service: {subgraph_name:?}"))?;
        // if let Some(id) = id {
