    ...
    RS: Service<TransportRequest, Response = TransportResponse, Error = BoxError> + Send,
{
    ...
    let req: TransportRequest = Request::from_parts(head, body).into();

    // TODO: Put apq in the right spot
    ...
    match service.ready_oneshot().await {
        Ok(mut service) => {
            let accepts_multipart = accepts_multipart(req.http_request.headers());
            let accepts_json = accepts_json(req.http_request.headers());
            let accepts_wildcard = accepts_wildcard(req.http_request.headers());
            ...
            match service.call(req).await {
                ...
                Ok(response) => {
                    ...
                    // // TODO: Maybe this belongs to http_service
