...
- `HttpRequest`: The router has just received the client request at the HTTP layer (before `RouterService`). Raw HTTP only; same payload shape as `RouterRequest`.
- `HttpResponse`: The router is about to send the HTTP response to the client (after the pipeline, at the HTTP layer). Same payload shape as `RouterResponse`.
...
- For `HttpRequest` and `HttpResponse` stages, `body` is a JSON _string_ (same as `RouterRequest` / `RouterResponse`).
...
The HTTP layer request stage. Same payload shape as `RouterRequest`; the router has just received the raw HTTP request.
...
```json title="Example coprocessor request body"
{
  ...
  "body": "{\"query\": \"query GetActiveUser { me { name } }\"}",
  "context": { "entries": {} },
  "sdl": "...",
  "path": "/",
  "method": "POST"
...
The HTTP layer response stage. Same payload shape as `RouterResponse`; the router is about to send the raw HTTP response to the client.
...
```json title="Example coprocessor request body"
{
  ...
  "body": "{\"data\": {\"me\": {\"name\": \"Ada Lovelace\"}}}",
  "context": { "entries": {} },
  "statusCode": 200,
  "sdl": "..."
}
