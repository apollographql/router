...
impl<'a> SchemaUpgrader<'a> {
    ...
    fn fix_federation_directives_arguments(&mut self) -> Result<(), FederationError> {
        ...
        for directive_name in ["requires", "provides"] {
            ...
            for field in &referencers.object_fields.clone() {
                ...
                for directive in field_type.directives.get_all_mut(directive_name) {
                    ...
                    {
                        if let Some(new_fields_string) = Self::make_fields_string_if_not(fields)? {
                            *fields.make_mut() = Value::String(new_fields_string);
                        }
                        break;
                    }
                }
        ...
        for field in &referencers.object_types.clone() {
            ...
            for directive in field_type
                .directives
                .iter_mut()
                .filter(|d| d.name.as_str() == "key")
            {
                if let Some(fields) = directive
                    .make_mut()
                    .specified_argument_by_name_mut("fields")
