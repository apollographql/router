...
traffic_shaping:
  ...
  subgraphs: # Rules applied to requests from the router to individual subgraphs
    products:
      ...
      http2: enable # Configures HTTP/2 usage. Can be 'enable' (default), 'disable' or 'http2only'
```
...
Keep-alive pings require HTTP/2 to be active. Setting `http2: disable` will prevent pings from being sent regardless of these settings.
