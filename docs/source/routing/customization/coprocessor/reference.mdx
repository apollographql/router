...
Present and `true` when `stage` is `ConnectorResponse` and the response was served from the router's [response cache](/router/performance/caching/response-caching/) instead of an HTTP call to the API.

Because no HTTP call was made, `statusCode` and `headers` are absent on such a response, and a status or headers your coprocessor returns for it are ignored. Use this property rather than the absence of `statusCode` to tell a cache hit from a failed request.
...
Absent on a `ConnectorResponse` that carries `cacheHit`, and on one whose request failed before a response was received.
...
A response served from the router's response cache made no HTTP call, so it carries `cacheHit: true` and no `statusCode` or `headers`:
