...
async fn subscription_callback_schema_reload() {
    ...
    let mut configuration: Configuration = serde_json::from_value(serde_json::json!({"include_subgraph_errors": { "all": true }, "headers": {"all": {"request": [{"propagate": {"named": "x-test"}}]}}, "subscription": { "enabled": true, "mode": {"callback": {"public_url": "http://localhost:4545"}}}})).unwrap();
    configuration.notify = notify.clone();
...
async fn subscription_with_callback_with_limit() {
    ...
    let mut configuration: Configuration = serde_json::from_value(serde_json::json!({"include_subgraph_errors": { "all": true }, "subscription": { "enabled": true, "max_opened_subscriptions": 1, "mode": {"callback": {"public_url": "http://localhost:4545"}}}})).unwrap();
    configuration.notify = notify.clone();
