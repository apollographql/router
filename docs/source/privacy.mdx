...
subtitle: Learn what data the GraphOS Router collects and how to opt out
description: Learn what data the GraphOS Router collects and how to opt out. By default, the GraphOS Router collects anonymous usage data to help improve the product.
---

By default, the GraphOS Router collects anonymous usage data to help us improve the product.

The GraphOS Router doesn't collect any personally identifiable information such as API keys, graph names, or file paths. 

<Tip>

**To opt out of data collection,** set the `APOLLO_TELEMETRY_DISABLED` environment variable to `true` in each environment where you run the GraphOS Router.
...
Unless you opt out, the GraphOS Router reports the following data:
