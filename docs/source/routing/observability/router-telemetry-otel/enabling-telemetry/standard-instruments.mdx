...
- `apollo.router.graphql_error` - counts GraphQL errors in responses. Also counts errors which occur during the response validation and connectors' response mapping phases, which are represented in client responses as `extensions.valueCompletion` and `extensions.connectorErrors` respectively instead of actual GraphQL errors. Attributes:
