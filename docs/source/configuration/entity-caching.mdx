...
<Note>
There is an issue when using `entity.type` on new relic because it's a reserved attribute name and so it won’t work properly. cf [Learn about New Relic entities](https://docs.newrelic.com/docs/new-relic-solutions/new-relic-one/core-concepts/what-entity-new-relic/#reserved-attributes).
To avoid this problem, please use an [alias on standard attribute](./telemetry/instrumentation/standard-instruments#alias) by using
```yaml
entity.type:
  alias: entity_type
```
