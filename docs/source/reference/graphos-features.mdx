...
A router connected to GraphOS, whether cloud-hosted or self-hosted, is called a **GraphOS Router**. It has access to specific GraphOS features depending on the connected GraphOS organization's licensed plan. Refer to the [pricing page](https://www.apollographql.com/pricing#graphos-router) to compare GraphOS Router features across plan types.
...
## Licensed plan features

<Tip>

Try the licensed features of GraphOS Router with a free [GraphOS trial](https://www.apollographql.com/pricing).

</Tip>

To enable support for licensed features in GraphOS Router:
...
After enabling support, you can begin using [licensed features](#graphos-router-features).

### License management

Whenever your router instance starts up and connects to GraphOS, it fetches a **license**, which is the credential that authorizes its use of the additional licensed features:
...
A router using an offline license requires [the use of local manifests](/graphos/routing/security/persisted-queries#experimental_local_manifests) when using [safelisting with persisted queries](/graphos/routing/security/persisted-queries), otherwise it will not work as designed when the router is disconnected from Uplink.
...
**You _can_ use licensed router features with a locally composed supergraph schema!** To do so, your router must still connect to GraphOS to obtain its [license](#license-management).

#### Set up local development

These steps work both for running the router executable directly (`./router`) and for running it via `rover dev`:

1. [Create a new variant](/graphos/graphs/federated-graphs/#adding-a-variant-via-the-rover-cli) for your supergraph that you'll use _only_ to fetch licenses.
...
4. Your router will fetch a license while using its locally composed supergraph schema.

### Common errors

**If your router doesn't successfully connect to GraphOS,** it logs an error that begins with one of the following strings if any licensed features are enabled:
...
| `License not found.`        | The router connected to GraphOS with credentials that are not associated with a licensed GraphOS plan. |
...
## Turning off licensed features

To turn off a licensed feature, remove all of its associated configuration keys from your router's [YAML config file](/graphos/reference/router/configuration#yaml-config-file).

## Grace period for expired plans

If your organization terminates its GraphOS subscription, your router's license is considered expired at the end of your final paid subscription period. GraphOS provides a grace period for expired licenses so that you can turn off licensed features before they produce breaking errors in your router.

If your router has an expired license, its behavior degrades according to the following schedule, _if_ any licensed features are still enabled:
...
Your router resumes normal functioning whenever you renew your GraphOS subscription or turn off all [licensed features](#licensed-plan-features).
