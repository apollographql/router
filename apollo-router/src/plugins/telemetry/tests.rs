...
use crate::plugins::telemetry::{filter_headers, CustomTraceIdPropagator, EnableSubgraphFtv1, EnabledFeatures, Telemetry, CLIENT_NAME, CLIENT_VERSION};
...
fn it_test_send_headers_to_studio() {
    ...
    let filtered_headers = filter_headers(&headers, &fw_headers);
    assert_eq!(
        filtered_headers.as_str(),
        r#"{"apollo-x-name":["polaris"],"test":["content"]}"#
    );
    let filtered_headers = filter_headers(&headers, &ForwardHeaders::None);
