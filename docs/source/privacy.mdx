...
- The configuration properties which have been referenced, but limited to types which could not contain personally identifyable information (e.g., certain scalar types; a property which is set to `true` not the value if it is a `string`)
