...
The key field is the same argument as defined in the subgraph's <code>@key</code> directive. If a subgraph has multiple keys defined and the entity is being invalidated, you need to send a request for each key definition.
