---
title: Configuring and applying @defer
...
- Configuring Apollo Router and clients to support the `@defer` directive.
- Writing subgraph schemas and queries that use `@defer`.
...
- Your supergraph must be one of:
  ...
  - A [self-hosted supergraph](/graphos/routing/self-hosted#self-hosted-supergraphs) running the [GraphOS Router](/graphos/routing/about-router)
