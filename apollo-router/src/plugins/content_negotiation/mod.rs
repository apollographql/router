        ...
        ServiceBuilder::new()
            .checkpoint(|request: router::Request| {
                ...
                if valid_content_type_header {
                    ...
                    let response = Self::invalid_content_type_header_response();
            ...
            .checkpoint(|request: router::Request| {
                ...
                if valid_accept_header {
                    ...
                    let response = Self::invalid_accept_header_response();
                    Ok(ControlFlow::Break(response.into()))
                }
            })
            .service(service)
...
fn is_multipart_defer(mime: &MediaType) -> bool {
    use mediatype::names::{MIXED, MULTIPART};
...
/// Returns true if the `CONTENT_TYPE` header contains `application/json` or `application/graphql-response+json`.
fn content_type_includes_json(headers: &HeaderMap) -> bool {
    headers
        .get_all(CONTENT_TYPE)
        .iter()
        .filter_map(|header| header.to_str().ok())
        .flat_map(|header_str| MediaTypeList::new(header_str))
        .any(|mime_result| mime_result.as_ref().map_or(false, is_json_type))
}
