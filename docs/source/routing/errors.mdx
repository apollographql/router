...
<Property name="SUBGRAPH_COST_TOO_EXPENSIVE">

<Since version="Router v1.53.0" />

The estimated cost of a subgraph query exceeded the configured per-subgraph cost limit. The error includes details about which subgraph exceeded its limit, the estimated cost, and the maximum allowed cost for that subgraph.
