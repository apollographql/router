...
**In Apollo Router versions earlier than v2.13.0**, these variables take precedence over Router telemetry settings and may cause traces or metrics to be sent to an unintended destination.

**Starting in Apollo Router v2.13.0 and later, Router will _refuse to start_ if any of these variables are set.**
