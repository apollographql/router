...
Measuring latency in the router is complex because it executes tasks in parallel. If the router makes two subgraph requests in parallel, it could appear idle on one request and busy on the other. One way to investigate the performance of the router is by comparing the behavior of the <code>http.client.request.duration</code> and <code>http.server.request.duration</code> metrics:
