...
// FIXME: potentially it could be useless, we might just choose the cargo package version in order to make sure we're cleaning the cache from every new router version to avoid breaking anything
...
impl CacheService {
    // FIXME: Potential improvement to get rid of the optional entity type is to directly get the parsed subgraph query from the query plan as the query plan is in rust now
    async fn call_inner(
        mut self,
...
pub(crate) fn hash_query(query_hash: &QueryHash, body: &graphql::Request) -> String {
    // FIXME: we should add rename QUeryHash with SupergraphQUeryHash or SubgraphQueryHash to have better understanding
    // QUestion: Do we ALWAYS generate operation names in subgraph query in QUery planner ? Is it possible to have several queries in an operation generated by the QP ?!
    let mut digest = Sha256::new();
...
// FIXME: global thought here, it doesn't make sense to serialize data in json and then hash it, I think we need a custom serde serializer to directly hash it without having to serialize anything first
...
fn extract_cache_key_root(
    ...
    // FIXME: find a better name for additional_data
...
fn extract_cache_keys(
    ...
    for representation in representations {
        let typename_value = representation
            .as_object_mut()
        ...
        // FIXME: I think we should just return an error instead of assigning "-" if the typename is not a string
        let typename = typename_value.as_str().unwrap_or("-");

        let hashed_entity_key = hash_entity_key(representation);

        // FIXME: to fix the @requires issue, we could try to separate stuffs from requires and entity keys and add another level in query hash after `entity` and add `requires_keys:{requires_keys}`
        // Requires data have to be stored no matter what because it could change the response value
        // // We can get the entity keys from the subgraph schema
        ...
        representation
            .as_object_mut()
            .map(|o| o.insert(TYPENAME, typename_value));
        res.push(key);
    }
    Ok(res)
}
