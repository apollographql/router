...
Only the GraphOS Router supports authorization directives&mdash;[`@apollo/gateway`](/federation/v1/gateway/) does _not_. Check out the [migration guide](../migrating-from-gateway/) if you'd like to use them.
...
#### Multiple authorization directives on a shared field use `AND` logic

If shared subgraph fields include multiple directives, composition merges them. For example, suppose the `me` query requires `@authentication` in on subgraph and the `read:user` scope in another subgraph:
...
#### Multiple `@requiresScopes` on a shared field using `AND` logic

<Tip>

Refer to the section on [Combining policies with AND/OR logic](#combining-policies-with-andor-logic) for a refresher of `@requiresScopes` boolean logic in one subgraph.

</Tip>

If multiple shared subgraph fields include `@requiresScopes`, the supergraph schema merges them with the same logic used to [combine scopes for a single use of `@requiresScopes`](#combining-required-scopes-with-andor-logic). For example, if one subgraph requires the `read:others` scope on the `users` query and another subgraph requires the `read:profiles` scope on `users` query:
...
#### Multiple `@requiresScopes` on a shared field using `OR` logic
...
If multiple shared subgraph fields include `@requiresScopes` with **OR** logic, the supergraph schema merges them so that satisfying any one of the required scopes grants access. This is similar to the approach for [combining required scopes with OR logic](#combining-required-scopes-with-andor-logic) within a single subgraph.

For example, if one subgraph requires the `read:others` scope on the `users` query and another subgraph requires the `read:profiles` scope on the same `users` query:
...
Then the supergraph schema would require **either** scope for access:
...
#### Multiple `@requiresScopes` on a shared field using `AND` and `OR` logic
