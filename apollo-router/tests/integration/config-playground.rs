...
//
// IMPORTANT: `response: info` is required to enable body stashing for response_errors selectors.
...
telemetry:
    ...
    instrumentation:
        events:
            router:
                # CRITICAL: response event triggers body stashing
                # Without this, response_errors selectors won't work
                response: info
                ...
                request.lifecycle:
                    ...
                    attributes:
                        ...
                        # -------------------------------------------------
                        # ERROR DETECTION
                        # hasErrors as "T"/"F" string (v1 plugin parity)
                        # -------------------------------------------------
                        hasErrors:
                            on_graphql_error_string: true
...
/// Test if early 400 errors are visible to response_errors with response: info.
#[tokio::test(flavor = "multi_thread")]
async fn test_early_400_extension_code_visibility() -> Result<(), BoxError> {
    ...
    // response_errors should also see the extension code
    let can_see_extension_code = metrics.contains("early_400_missing_query_total");
    eprintln!("\n=== RESULT ===");
    eprintln!(
        "Can response_errors see early 400 extension codes? {}",
        can_see_extension_code
    );

    if can_see_extension_code {
        eprintln!("✅ response: info DOES enable extension code classification for early 400s!");
    }
...
/// Test logging with response timing (txTime).
#[tokio::test(flavor = "multi_thread")]
async fn test_log_response_timing() -> Result<(), BoxError> {
    let mut router = IntegrationTest::builder()
        .config(SHARED_CONFIG)
        .build()
        .await;
    ...
    let logs = router.logs();
    let has_response_log = logs.iter().any(|l| l.contains("router.response"));
    assert!(has_response_log, "Expected router.response event in logs");
...
async fn test_logging_plugin_parity() -> Result<(), BoxError> {
    ...
    // hasErrors as "T"/"F"
    assert!(
        log_entry.contains("\"hasErrors\":\"T\""),
        "hasErrors should be 'T'"
    );
    eprintln!("✅ hasErrors (\"T\"/\"F\" string): PRESENT");
...
async fn test_logging_has_errors_true() -> Result<(), BoxError> {
    ...
    eprintln!("✅ Response with errors: hasErrors = \"T\"");
...
async fn test_logging_has_errors_false() -> Result<(), BoxError> {
    ...
    assert!(
        log_entry.contains("\"hasErrors\":\"F\""),
        "hasErrors should be 'F' when response has no errors"
    );

    eprintln!("✅ Response without errors: hasErrors = \"F\"");
