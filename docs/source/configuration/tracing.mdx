...
Set a service name for your router traces so they can be easily searched and found in your metrics dashboards. 
...
3. `telemetry.tracing.common.service_name` in `router.yaml`

      <ExpansionPanel title="Example service_name">

      Example setting service name in `telemetry.tracing.common.service_name`:
...
4. `telemetry.tracing.common.resource` in `router.yaml`

      <ExpansionPanel title="Example resource">

      Example setting service name in `telemetry.tracing.common.resource`:
...
In `router.yaml`, resource attributes are set in `telemetry.tracing.common.resource`. For example:
...
You can configure the sampling rate of traces to match the rate of your application performance monitors (APM). To enable sampling configuration, in `router.yaml` set `telemetry.tracing.common.sampler` and `telemetry.tracing.common.parent_based_sampler`:
...
The `telemetry.tracing.propagation` section allows you to configure which propagators are active in addition to those automatically activated by using an exporter.
...
[Jaeger (native) tracing is deprecated](https://opentelemetry.io/blog/2022/jaeger-native-otlp/) and will be removed in a future release of the Apollo Router. [Jaeger via OTLP](#jaeger) should be used instead.
