...
The router's configuration provides several Redis options to ensure you can scale properly and achieve the best performance.
...
preview_response_cache:
  ...
  subgraph:
    all:
      ...
      redis:
        urls: ["redis://..."]
        fetch_timeout: 2s # Optional, by default: 500ms
        insert_timeout: 2s # Optional, by default: 500ms
        invalidate_timeout: 2s # Optional, by default: 1s
...
The <code>pool_size</code> option defines the number of Redis connections the router opens. By default, the router opens five connections. If you have high traffic between the router and Redis, or if you observe latency in those requests, increase the pool size to reduce that latency. You can measure whether you need to increase this number by monitoring the <code>apollo.router.operations.response_cache.insert</code> and <code>apollo.router.operations.response_cache.fetch</code> metrics (see [Metrics](#metrics)). If the time to insert and fetch data from the cache is long, it might be because of delays in acquiring a connection from the pool.
