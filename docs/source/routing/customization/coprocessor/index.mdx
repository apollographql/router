...
As shown in the diagram, the `RouterService`, `SupergraphService`, `ExecutionService`, `SubgraphService`, and `ConnectorRequestService` steps of the [request-handling lifecycle](/graphos/routing/customization/rhai/#router-request-lifecycle) can send these POST requests (also called "coprocessor requests").
...
**Breaking changes in 2.16:** The `client` configuration block has been updated. If you use any of the following fields, you must migrate before upgrading:

| Removed field | Replacement |
|---|---|
| `client.experimental_http2: enable` | `client.protocol: alpn` |
| `client.experimental_http2: http2only` | `client.protocol: http2` |
| `client.experimental_http2: disable` | `client.protocol: http1` |
| `client.dns_resolution_strategy` | No replacement — remove this field from your config. |
| `client.pool_idle_timeout` | `client.http1.pool_idle_timeout` |
...
For example, to enable h2c (HTTP/2 cleartext) communication with a coprocessor you can use this configuration:

```yaml title="router.yaml"
coprocessor:
  url: http://127.0.0.1:8081
  # Using an HTTP (not HTTPS) URL and protocol: http2 results in connections that use h2c (HTTP/2 cleartext)
  client:
    protocol: http2
```
...
The router maintains a pool of HTTP/1.1 connections to your coprocessor. Use `client.http1.pool_idle_timeout` to set how long an idle connection stays in the pool before it's eligible for eviction. The default is 90 seconds.
...
coprocessor:
  ...
  client:
    http1:
      pool_idle_timeout: 30s  # default: 90s
```

#### TLS configuration

To connect to a coprocessor over HTTPS with a custom certificate authority, client certificate (mTLS), or both, configure the `client.tls` section:

```yaml title="router.yaml"
coprocessor:
  url: https://my-coprocessor.internal
  client:
    tls:
      # PEM-encoded CA certificate bundle. Loaded from disk via file expansion.
      certificate_authorities: "${file:./certs/ca.pem}"
      # Set to false to trust only the supplied bundle (disables OS native store).
      use_native_certificate_store: true
      # Optional: present a client certificate for mutual TLS (mTLS).
