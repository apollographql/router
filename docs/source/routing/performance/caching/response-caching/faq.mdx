...
All changes are backward-compatible. If you're using the `preview_entity_caching` plugin, you can migrate to response caching by:
- Renaming `preview_entity_caching` to `preview_response_caching`.
- Get rid of `redis.scan_count` configuration, it's now useless because response caching doesn't scan Redis anymore.
- Rename `redis.timeout`, response caching introduces more granularity in timeouts so instead of just having `redis.timeout` it will now be replaced with `redis.fetch_timeout`, `redis.insert_timeout`, `redis.invalidate_timeout`, these timeouts are still all optionals.
