...
impl RouterService {
    async fn call_inner(&self, req: RouterRequest) -> Result<RouterResponse, BoxError> {
        ...
        let graphql_request: Result<graphql::Request, (StatusCode, &str, String)> = if parts.method
            == Method::GET
        {
            parts
                .uri
        ...
        match graphql_request {
            Ok(graphql_request) => {
                ...
                let request_res = self.apq_layer.supergraph_request(request).await;
                let SupergraphResponse { response, context } = match request_res {
                    Err(response) => response,
                    Ok(request) => {
                        match self.query_analysis_layer.supergraph_request(request).await {
                            Err(response) => response,
                            Ok(request) => {
                                self.supergraph_creator.create().oneshot(request).await?
                            }
                        }
                    }
                };
