  ...
  subgraph:
    all:
      ...
      redis:
        ...
        ttl: 24h # Optional, default: no expiration. Default TTL for data saved in Redis
...
When active, the `required_to_start` option prevents the router from starting if it cannot connect to Redis. By default, the router starts without a connection to Redis, which means it sends requests to subgraphs directly, bypassing the cache:
