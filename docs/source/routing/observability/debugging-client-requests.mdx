...
Once you enable [router telemetry](/graphos/routing/observability/telemetry), traces can have spans labeled `router`. You can add attributes to include helpful information about the request and response. The following sections show some common patterns.

<Note>

Spans don't include an attribute for the HTTP body text. If you want to see the GraphQL operation, first check your [GraphOS Insights](/graphos/platform/insights) or use the [request logging configuration](#request-body-logging).
</Note>

#### Add OpenTelemetry attributes

You can add [OpenTelemetry (OTel) standard attributes](/graphos/reference/router/telemetry/instrumentation/standard-attributes#router) such as `error.type` and `http.response.status_code` to spans with this configuration:
...
You can map GraphQL error data to OTel errors with the following configuration:
...
You can flag rate limits and timeouts on router spans with the following configuration:
