...
A path to a file containing the [graph API key](/graphos/platform/access-management/api-keys/graph-api-keys) that the router should use to authenticate with GraphOS when fetching its supergraph schema.
