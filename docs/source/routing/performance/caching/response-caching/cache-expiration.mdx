...
subtitle: Control cache freshness with TTL and invalidation
description: Configure time-to-live (TTL) for automatic expiration and use active invalidation to remove stale cache entries on demand.
minVersion: Router v2.8.0
releaseStage: preview
---

Cache expiration ensures that cached data stays fresh. The router supports two approaches: passive expiration based on time to live (TTL) and active invalidation when you know data has changed.
...
Time to live (TTL) determines how long cached data remains valid before the router must fetch fresh data from your subgraphs.

### How TTLs work

The GraphOS Router honors the [`Cache-Control` header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cache-Control) returned with subgraph responses. The router uses the `max-age` value from this header to determine how long to cache each piece of data.

When responding to client queries, the router:
1. Calculates the overall response TTL by taking the **minimum TTL** from all cached data included in the response
...
For subgraphs that support the `@cacheControl` directive (such as Apollo Server), you can set field-level and type-level TTLs directly in your schema. The subgraph translates these directives into `Cache-Control` headers that the router respects.
...
In this example:
- The `votes` field caches for 30 seconds
- The `readByCurrentUser` field caches for 10 seconds and is marked as private (user-specific)
...
This schema example uses both `@cacheTag` (for invalidation) and `@cacheControl` (for TTL):
...
In this example, the `User` type has a TTL of 10 seconds and a cache tag of `user-42` when the user ID is `42`.
    ...
    subgraphs:
      products:
        ...
        invalidation:
          ...
          shared_key: ${env.PRODUCTS_INVALIDATION_SHARED_KEY}
```
