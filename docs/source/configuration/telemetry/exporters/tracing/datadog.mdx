...
To configure the router, enable the [OTLP exporter](./otlp) and set `endpoint: <datadog-agent-endpoint>` and enable `datadog_agent_sampling`. For example:
...
telemetry:
  exporters:
    tracing:
      common:
        # Only 10 percent of spans will be sent to the Datadog agent with `psr` or `sampling.priority` 1. Experiment to see what value works best for you.
        sampler: 0.1
        # Important! APM metrics will not display correctly without this setting.
...
```
Note that if `datadog_agent_sampling: true` is not set in `router.yaml` APM metrics will not display correctly.
...
> [!IMPORTANT]
> Users using OTLP exporter must enable `datadog_agent_sampling` option to get accurate APM metrics.
