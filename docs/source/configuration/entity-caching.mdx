...
In router versions 1.51 and earlier, Redis and per-subgraph caching configurations were set directly on `preview_entity_cache`, for example `preview_entity_cache.redis`.
While in [preview](/resources/product-launch-stages/#product-launch-stages), configuration may change.
