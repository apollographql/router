...
When a corporate firewall, VPC security group, or asymmetric routing policy breaks the path between Router and the client, Router accepts the subscription request, but the response stream might not return to the client. The diagnostic looks identical to a buffering or timeout failure (504 at the client with no router-side metric event). Include path-asymmetry checks in your investigation when you can't identify more common causes.
...
If 504s persist after you have configured every hop for streaming and made sure the timeouts cover the expected lifetime, the metric tells you which side of the router boundary to investigate next.
