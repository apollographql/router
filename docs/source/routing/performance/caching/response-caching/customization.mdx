...
When the router runs behind a CDN that supports tag-based purging (such as Cloudflare or Fastly), it can emit the set of cache tags involved in a request as a response header. Your CDN reads that header and keys its edge entry on those tags, so a single purge by tag can invalidate the router's response cache and the CDN edge cache simultaneously.

This is the most common reason to pair response caching with a CDN: you can cache aggressively at the edge with long TTLs, and still invalidate precisely by tag (for example `product-42`) when the underlying data changes—without flushing the whole edge cache.

### How it works

When `propagate_cache_tags.enabled` is `true`, then for each supergraph response the router:

- Aggregates the union of cache tags contributed by every subgraph call in the request—resolved `@cacheTag` directive values, plus `apolloCacheTags` (root fields) and `apolloEntityCacheTags` (entities) returned in subgraph response extensions.
- Sorts and deduplicates that set for deterministic output.
- Emits the joined value as a configurable response header (default `Cache-Tag`).
...
Emits the configured header on the supergraph response. Defaults to `false`, so existing deployments see no change.
...
Maximum size, in bytes, of the joined header value. Defaults to `16384`. CDN edge servers typically reject oversized header values; when the joined value would exceed this limit, the router applies `on_overflow` instead of emitting a header the CDN would reject.

##### `on_overflow`

What to do when the joined value exceeds `max_bytes`:
...
`truncate` and `drop` log a structured `warn` with `max_bytes`, `actual_bytes`, and `dropped_count` so you can monitor size.
...
This is a **read** surface. The router emits the default `Cache-Tag` header itself. To change what reaches your CDN—prefix the tags, filter them, or emit them on a different header—read this set and write your preferred header. A script's or coprocessor's response stage runs after the router has emitted the default header, so the value you set takes precedence.
...
- Cache entries written before this feature shipped carry no aggregated tag set and contribute no tags until they expire via TTL. To populate tags across the full cache set immediately, flush Redis before adopting propagation.
