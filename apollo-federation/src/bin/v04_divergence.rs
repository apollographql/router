...
use apollo_federation::connectors::DiffKind;
...
fn main() {
    ...
    for (line_no, line) in reader.lines().enumerate() {
        ...
        let obj = record
            .as_object_mut()
            .expect("input record must be an object");
        obj.insert(
            "v03_parse".to_string(),
            Value::String(verdict.v03_parse.clone()),
        );
        obj.insert(
            "v04_parse".to_string(),
            Value::String(verdict.v04_parse.clone()),
        );
        obj.insert(
