...
The router always exports traces to GraphOS via OTLP. Two samplers control how many traces are sent:

- `telemetry.exporters.tracing.common.sampler` sets the fraction of all requests that are traced at all.
- `telemetry.apollo.sampler` sets the fraction of all requests whose traces reach GraphOS. This is an absolute fraction of all traffic, so it must not exceed the common sampler.
...
telemetry:
  exporters:
    tracing:
      common:
        # Trace 50% of all requests globally
        sampler: 0.5

  apollo:
    # Send 10% of all requests to GraphOS. This is an absolute fraction of all traffic and must not exceed the common sampler
    sampler: 0.1
```

Whatever these samplers let through is then subject to a safeguard that further reduces GraphOS trace volume — see the [Apollo trace throttling section](#apollo-trace-throttling).
...
By default, the router sends GraphOS OTLP data over gRPC. To switch to HTTP protobuf, set `otlp_tracing_protocol` and `otlp_metrics_protocol`:

```yaml title="router.yaml"
telemetry:
  apollo:
    otlp_tracing_protocol: http   # grpc (default) | http
    otlp_metrics_protocol: http   # grpc (default) | http
```
...
To keep trace volume manageable for high-traffic graphs, the router applies a throttle that further reduces the number of traces it sends to GraphOS. This runs after the `telemetry.apollo.sampler` and `telemetry.exporters.tracing.common.sampler` head samplers, so it only ever affects the traces those samplers already selected. This reduces the processing and transfer of traces, and it affects only the GraphOS pipeline — traces sent to your own OTLP collector or to Datadog are unaffected.

Configure the strategy of the throttle with `telemetry.apollo.tracing.throttle`:
...
By default, the router reports richer error information to Studio via the `extended_error_metrics` and `redaction_policy` configurations. To opt out, set `extended_error_metrics: disabled`.

```yaml title="router.yaml"
telemetry:
  apollo:
    errors:
      extended_error_metrics: enabled # (default: enabled)
      subgraph:
        all:
          # By default, subgraphs should report errors to GraphOS
          send: true # (default: true)
...
HTTP proxy routing applies to the HTTP transport. Set `otlp_tracing_protocol` and `otlp_metrics_protocol` to `http` to route GraphOS OTLP traffic through a proxy. See [Configuring the transport protocol](#configuring-the-transport-protocol) for details.
