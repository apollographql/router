...
impl Plugin for EchoCoProcessor {
    ...
    fn web_endpoints(&self) -> MultiMap<ListenAddr, Endpoint> {
        let router = Router::new().route("/", any(handle_coprocessor));
...
async fn handle_coprocessor(req: axum::extract::Request) -> axum::response::Response {
    ...
    if json_body.get("serviceName") == Some(&serde_json::Value::String("reviews".to_string())) {
        json_body
            .as_object_mut()
