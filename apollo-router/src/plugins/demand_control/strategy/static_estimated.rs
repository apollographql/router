...
pub(crate) struct StaticEstimated {
    ...
    pub(crate) actual_cost_computation_mode: ActualCostComputationMode,
...
impl StrategyImpl for StaticEstimated {
    ...
    /// Determine actual cost for this specific subgraph request
    fn on_subgraph_response(
        ...
        if !matches!(
            self.actual_cost_computation_mode,
            ActualCostComputationMode::BySubgraph
        ) {
            return Ok(());
        }
    ...
    /// Sum up each subgraph response cost to determine the total cost
    fn on_execution_response(
        ...
        let cost = match self.actual_cost_computation_mode {
            ActualCostComputationMode::BySubgraph => context
                .get_actual_cost_by_subgraph()?
                .map_or(0.0, |cost| cost.total()),
            #[allow(deprecated_in_future)]
            ActualCostComputationMode::Legacy => self.cost_calculator.actual(
                request,
                response,
                &context
                    .extensions()
