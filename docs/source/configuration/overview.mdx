...
If running the router with an [offline license](#--license), setting both `APOLLO_GRAPH_REF` and `APOLLO_KEY` configures the router to send usage metrics to GraphOS on a best-effort basis; not setting both disables sending usage metrics. Otherwise, setting this is **required** when using managed federation.
...
If starting the router with an [offline license](#--license), setting both `APOLLO_GRAPH_REF` and `APOLLO_KEY` configures the router to send usage metrics to GraphOS on a best-effort basis; not setting both disables sending usage metrics. Otherwise, setting this is **required** when using managed federation.
...
The Apollo Router supports standard and custom instrumentation to collect telemetry data from its request and response processing pipeline.

#### Metrics

To configure the metrics collected and exported by the Apollo Router, see [Router Metrics](./metrics).

Supported metrics exporters:

- [Prometheus](./exporters/prometheus)
- [OpenTelemetry Protocol (OTLP)](./exporters/otlp)
- [Datadog (OTLP)](./exporters/datadog-otlp)
...
To configure logging in the Apollo Router, see [Logging](./logging).

#### Instrumentation

To configure standard or custom instrumentation in the Apollo Router, see the following configuration guides:
