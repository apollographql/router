...
The `Config` associated type of the `Plugin` and `PluginUnstable` traits now requires `Clone + Send + Sync + 'static`. This only affects [native Rust plugins](/graphos/routing/customization/native-plugins); router configuration files are unaffected.
