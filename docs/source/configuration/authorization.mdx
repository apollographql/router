...
By default, when part of a query is filtered by authorization, the list of filtered paths is added to the response and logged by the router. We may need more flexibility in reporting errors, for various reasons:
- filtering parts of a query according to the client's rights may be seen as normal operation, and not warrant investigation by platform operators, so logs should be disabled
- when testing authorization, we may not want the clients to raise exceptions if authorization errors are present in the response's `errors` field, so we can put them in `extensions` instead
- we may not want to give the clients any information about authorization errors, so they can be disabled entirely