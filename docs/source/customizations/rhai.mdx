...
As with native plugins, there are 4 services, each with a request and a response, which can be used to inject extra behaviour into a router. The router Rhai plugin implements eight functions (two for each service) as follows:
...
Each of these functions are optional, just implement the ones where you wish to inject behaviour. Every listed function takes a `context` as parameter with `extensions` and `headers` inside and must return an updated `context`.
