...
As shown in the diagram above, the `RouterService`, `SupergraphService`, `ExecutionService`, `SubgraphService`, and `ConnectorRequestService` steps of the [request-handling lifecycle](/graphos/routing/customization/rhai/#router-request-lifecycle) can send these POST requests (also called **coprocessor requests**).
