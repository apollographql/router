...
> ⚠️ This example forwards JWT claims via headers. When combined with [header propagation](./header-propagation/) this can introduce a security risk. If you're relying on header-based forwarding, **make sure not to propagate matching headers**. For example, if you had the claim "admin" in your JWT which was forwarded as a header, and a matching header propagation rule (e.g., all headers via `.*`) an attacker could send the "admin" header in their requests to impersonate an admin.
