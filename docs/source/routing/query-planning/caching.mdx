...
In your router's YAML config file, always specify Redis URLs via environment variables and [variable expansion](https://www.apollographql.com/docs/graphos/routing/configuration/yaml#variable-expansion). This prevents Redis URLs from being committed to version control, which is especially dangerous if they include authentication information like a username or password.
...
Instead, use [variable expansion](https://www.apollographql.com/docs/graphos/routing/configuration/yaml#variable-expansion) to inject credentials via environment variables or a secrets manager (for example, `${env.REDIS_PASSWORD}`).
