...
Two experimental configuration options limit the work the query planner does before emitting a plan: `plans_limit` and `paths_limit`. Both live under `supergraph.query_planning` in the YAML config, alongside the cache configuration.

```yaml title="router.yaml"
supergraph:
  query_planning:
    plans_limit: 10000 # default
    paths_limit: null  # default — no limit
```
...
| --- | --- | --- |
| Planner takes too long but produces a plan | High `evaluated_paths`, high `evaluated_plans` | Reduce `plans_limit` to bound exploration |
| Planner runs out of memory or memory usage spikes during planning | `evaluated_plans` near the limit, high `apollo.router.query_planner.memory` peaks | Reduce `plans_limit` (e.g., `1000`) |
| Specific queries fail with `QueryPlanComplexityExceeded` | `evaluated_paths` hitting `paths_limit` | Review the offending query, or raise `paths_limit` if intentional |

To track the limits each router instance applies, the router emits `apollo.router.config.plans_limit` and `apollo.router.config.paths_limit` gauges (added in v2.14.1).
