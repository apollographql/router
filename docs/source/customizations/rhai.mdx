...
All request/response callback functions are provided with a typed Request/Response which can be interacted with to manipulate headers and context. In addition, partial body details (query, operation name) are available to be read for all requests. The examples illustrate how to interact with request/response objects.
