...
The `apollo.router.overhead` histogram provides a more direct measurement of router processing overhead. This metric tracks the time the router spends on tasks other than waiting for subgraph requests—including GraphQL parsing, validation, query planning, response composition, and plugin execution.
...
You can attach custom attributes using [selectors](/router/configuration/telemetry/instrumentation/selectors#router), documented in the [selectors reference](/router/configuration/telemetry/instrumentation/selectors#router).
