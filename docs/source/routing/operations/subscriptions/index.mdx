...
You can ignore specific headers when doing the subscription deduplication. If you propagate `user-agent` header to subgraphs and you still want to deduplicate subscriptions and so ignore this header
in the deduplication algorithm you can do it by adding the following to your router's YAML config file:
