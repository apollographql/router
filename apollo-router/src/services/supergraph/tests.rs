...
const SEQUENCED_DEFER_SCHEMA: &str = r#"
schema
  @link(url: "https://specs.apollo.dev/link/v1.0")
  @link(url: "https://specs.apollo.dev/join/v0.3", for: EXECUTION)
{
...
enum join__Graph {
  S @join__graph(name: "s", url: "http://s")
}
...
type Mutation @join__type(graph: S) {
  update1: T
  update2: T
}
...
/// The incremental planner plans each top-level mutation field on its own,
/// so a deferred mutation becomes a Sequence of top-level Defer nodes rather
/// than a single Defer. Fetch ids restart per field, hence the parameter.
fn sequenced_mutation_defer_plan(second_fetch_id: &str) -> crate::query_planner::PlanNode {
    // Labels are the ones the router's labeler assigns before planning.
    let defer_for = |field: &str,
                     primary_field: &str,
                     deferred_field: &str,
                     id: &str,
                     label: &str| {
        serde_json::json!({
            "kind": "Defer",
            "primary": {
                "subselection": format!("{{ {field} {{ {primary_field} }} }}"),
                "node": {
                    "kind": "Fetch",
                    "serviceName": "s",
                    "variableUsages": [],
                    "operation": format!("mutation{{{field}{{__typename {primary_field} id}}}}"),
            ...
            "deferred": [{
                ...
                "label": label,
                "queryPath": [field],
                "subselection": format!("{{ {deferred_field} }}"),
                "node": {
                    "kind": "Flatten",
                    "path": [field],
                    "node": {
                        ...
                        "variableUsages": [],
                        "operation": format!(
                            "query($representations:[_Any!]!){{_entities(representations:$representations){{...on T{{{deferred_field}}}}}}}"
                        ),
                        "operationKind": "query",
                    },
                },
    ...
    serde_json::from_value(serde_json::json!({
        "kind": "Sequence",
        "nodes": [
            defer_for("update1", "v0", "v1", "0", "0"),
            defer_for("update2", "v1", "v0", second_fetch_id, "1"),
        ],
    }))
    .expect("valid plan node")
}

async fn execute_sequenced_mutation_defer(second_fetch_id: &'static str) -> Vec<graphql::Response> {
    let entities =
        "query($representations:[_Any!]!){_entities(representations:$representations){...on T{%}}}";
    let subgraphs = MockedSubgraphs(
        [(
            "s",
            MockSubgraph::builder()
                .with_json(
                    serde_json::json! {{"query": "mutation{update1{__typename v0 id}}"}},
                    serde_json::json! {{"data": {"update1": {"__typename": "T", "v0": "a", "id": "1"}}}},
                )
                .with_json(
                    serde_json::json! {{"query": "mutation{update2{__typename v1 id}}"}},
                    serde_json::json! {{"data": {"update2": {"__typename": "T", "v1": "c", "id": "2"}}}},
                )
                .with_json(
                    serde_json::json! {{
                        "query": entities.replace('%', "v1"),
                        "variables": {"representations": [{"__typename": "T", "id": "1"}]},
                    }},
                    serde_json::json! {{"data": {"_entities": [{"v1": "b"}]}}},
                )
                .with_json(
                    serde_json::json! {{
                        "query": entities.replace('%', "v0"),
                        "variables": {"representations": [{"__typename": "T", "id": "2"}]},
                    }},
                    serde_json::json! {{"data": {"_entities": [{"v0": "d"}]}}},
                )
                .build(),
        )]
        .into_iter()
    ...
    let service = TestHarness::builder()
        .configuration_json(serde_json::json!({"include_subgraph_errors": { "all": true } }))
        .unwrap()
        .schema(SEQUENCED_DEFER_SCHEMA)
        .execution_hook(move |service| {
            service
                .map_request(move |mut req: crate::services::execution::Request| {
                    let plan = &req.query_plan;
                    req.query_plan = Arc::new(crate::query_planner::QueryPlan {
                        usage_reporting: plan.usage_reporting.clone(),
                        root: Some(Arc::new(sequenced_mutation_defer_plan(second_fetch_id))),
    ...
    let request = supergraph::Request::fake_builder()
        .context(defer_context())
        .query("mutation { update1 { v0 ... @defer { v1 } } update2 { v1 ... @defer { v0 } } }")
        .build()
        .unwrap();
...
/// The first payload carries both primaries and each deferred payload
/// arrives once, whether or not the per-field fetch ids collide.
#[rstest::rstest]
#[case::distinct_fetch_ids("1")]
#[case::colliding_fetch_ids("0")]
#[tokio::test]
async fn sequenced_top_level_defers_on_mutation(#[case] second_fetch_id: &'static str) {
    let mut responses = execute_sequenced_mutation_defer(second_fetch_id).await;
    let primary = responses.remove(0);
    let mut incremental: Vec<_> = responses
        .iter()
        .flat_map(|r| r.incremental.iter())
        .map(|i| serde_json::to_value(i).unwrap())
        .collect();
    incremental.sort_by_key(|i| i["path"].to_string());
    assert_eq!(
        serde_json::to_value(&primary).unwrap(),
        serde_json::json!({
            "data": {"update1": {"v0": "a"}, "update2": {"v1": "c"}},
            "hasNext": true,
        })
    );
    assert_eq!(
        incremental,
        vec![
            serde_json::json!({"data": {"v1": "b"}, "path": ["update1"]}),
            serde_json::json!({"data": {"v0": "d"}, "path": ["update2"]}),
        ]
    );
    assert_eq!(responses.last().and_then(|r| r.has_next), Some(false));
}
