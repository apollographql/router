...
Use the `request_header` and `response_header` selectors on `http_client` spans to capture headers from HTTP requests sent to subgraphs and REST services. Because `http_client` executes after Rhai scripts, these selectors observe headers added by scripts.
