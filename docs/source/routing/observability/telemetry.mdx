...
You can set conditions for instruments and events to only collect telemetry data [when necessary](#collect-exactly-the-telemetry-you-need). This configuration snippet enables only collecting the configured telemetry data when the `request_header` is equal to "example-value":
...
The router sends [operation](/graphos/platform/insights/sending-operation-metrics#from-the-apollo-router-or-apollo-server) and [field usage metrics](/graphos/platform/insights/sending-operation-metrics#from-the-apollo-router) metrics to GraphOS. Additionally, metric exporters can consume and monitor other metrics.
