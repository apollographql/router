...
Configure subgraph timeouts to be less than the router timeout. This allows the router to properly handle subgraph timeouts and return appropriate error responses to clients.
...
When a subgraph timeout is hit, a [GraphQL-compliant HTTP 200 status code](https://graphql.github.io/graphql-over-http/) is returned to the client. Because the router can handle multiple subgraph requests for one operation, the timeout of one subgraph doesn't immediately invalidate other requests in the query plan. Returning a 200 status code enables the router to return partial data from other requests that didn't timeout.
