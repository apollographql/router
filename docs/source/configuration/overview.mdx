...
The timeout for each of the polls to Apollo Uplink.
...
By default, the Apollo Router starts an HTTP server that listens on `127.0.0.1:4000`, but you can specify a different address:
...
The Apollo Router can also listen on a Unix socket (not supported on Windows):
...
It is enabled by default, but can be disabled from the configuration:
...
Subgraph requests from the Router can use APQ (disabled by default). Enabling APQ on a subgraph can be done either globally or on a per-subgraph request. The following example shows how to use a mixture of both global and per-subgraph configuration to accomplish enabling APQ on the `products` subgraph, while otherwise disabling it:
...
TLS connections to subgraphs are verified using the list of certificate authorities provided by the system. It is possible to override the list, for all subgraphs or per subgraph:
