...
- [**External co-processing**](/router/customizations/coprocessor/) ([Enterprise feature](../enterprise-features/))
...
The Apollo Router Core supports customization only through [Rhai scripts](/router/customizations/rhai/).

Because [Rhai scripts](/router/customizations/rhai/) are easier to deploy, we recommend using them if they support your use case. Use external co-processing if your customization needs to do any of the following (which Rhai scripts _don't_ support):
...
To learn how to hook in to the various lifecycle stages, including examples customizations, refer to the [Rhai scripts](/router/customizations/rhai/) and [external coprocessing](/router/customizations/coprocessor/) docs.
