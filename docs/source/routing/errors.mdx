...
The router stopped planning because the operation exceeded a query-planning complexity limit. The response has a 400 HTTP status code. For the non-local selections limit, see [`max_non_local_selections`](/graphos/routing/security/request-limits#max_non_local_selections).
