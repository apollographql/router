...
- `apollo.router.operations.subscriptions.rejected` (attributes: `reason`, `subgraph.service.name`): Incremented when a subscription request is rejected. The `reason` attribute indicates why (`max_opened_subscriptions_limit_reached` or `subgraph`), and `subgraph.service.name` identifies the subgraph when applicable.
- `apollo.router.operations.subscriptions.ended.subgraph` (attribute: `subgraph.service.name`): Incremented once per subgraph WebSocket closure. Each deduplicated client sharing that connection will also emit a corresponding `apollo.router.operations.subscriptions.terminated` event with `reason=server_close`.
