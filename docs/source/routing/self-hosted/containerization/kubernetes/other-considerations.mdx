...
Apollo Router does not currently provide a configurable drain timeout or parameter similar to `stopGracePeriodMillis` found in Apollo Server. Upon receiving a `SIGTERM` in Kubernetes, the Router immediately stops accepting new requests and waits for all active/in-flight requests to complete before exiting. There is no internal time limit controlling this process: the shutdown will take as long as it takes for ongoing requests to either finish or time out naturally at the request level.

If you need to set any termination duration for your service mesh, this should be just slightly more than your longest subgraph timeout configuration.
