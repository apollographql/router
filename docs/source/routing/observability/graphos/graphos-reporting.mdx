...
This includes the errors a connector's response mapping declares with `->withError`, which clients read in `extensions.connectorErrors` rather than in `errors`. Their `service` is the connector's subgraph, and their `code` is the one the mapping author supplied. They are counted at the connector, so `include_subgraph_errors` withholding one from a *client* does not suppress its metric, while a subgraph's `send: false` does withhold it from Studio. Note that `send: false` is not a blanket off switch: like every other error, a declared error is still counted by [`apollo.router.graphql_error`](/graphos/routing/observability/telemetry/instrumentation/standard-instruments), which no telemetry setting gates.
