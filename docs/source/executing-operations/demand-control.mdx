...
subtitle: Configure your router with operation cost limits to reject high cost operations sent by malicious or demanding clients
description: Protect your Apollo Router from malicious or demanding clients by configuring demand control to reject high cost operations.
---
...
To calculate the total cost of an operation, the router assigns a cost or weight for each sub-request, based on fixed costs for each sub-request's operation kind and the types of GraphQL elements of its fields.

The cost of each operation kind:
...
The cost of each GraphQL element type, per operation kind:
...
The router's demand control supports a measurement mode that enables you to gather this information without impacting your running services. You can define telemetry instruments to monitor your operations and decide on their maximum cost threshold.  
...
| static_estimated.max        | integer                 |               | The maximum cost of an accepted operation. An operation with cost higher than `strategy.max` is rejected. |
...
When analyzing the costs of operations, if your histograms are not granular enough or don't cover sufficient range, you can modify the views in your telemetry configuration:
...
A chart of the example histogram:
...
1. The previous step identified a noticeable difference between actual and estimated costs with the example operations. You can better understand the difference—and consequently tune the configured list size—by adding telemetry instruments for fields in your GraphQL schema.

For example, you can generate a histogram for every field in your GraphQL schema: 

```yaml title="Telemetry to generate histogram for every field in GraphQL schema"
telemetry:
  exporters:
    metrics:
      common:
...
```yaml title="Example instrument for list sizes of specific operation"
telemetry:
  instrumentation:
    instruments:
      graphql:
...
This output shows that the actual list sizes for the `allProducts` field. You should configure `static_estimated.list_size` to be the maximum of the range of actual costs, which is 100.
...
preview_demand_control:
  ...
  strategy:
    static_estimated:
      list_size: 100 # Updated list size
      max: 1000
```
...
Looking at the top N operations, you can see that the estimated costs have been updated:
...
From the previous step, you can set the maximum cost to a value that allows all operations except `ExceptAll`:

```yaml title="Example enforcing max cost"
preview_demand_control:
  enabled: true
