...
- Constructor options passed to `new ApolloServer({ ... })` (TODO — is this one important? Is plugins most important?)
...
Environment variables have implicit effect on some Apollo features, like Managed Federation and usage reporting.  Take note of any `APOLLO_` prefixed environment variables that are being set when starting the host runtime you run your Apollo Gateway and Apollo Server on.

All official Apollo-related environment variables are prefixed with `APOLLO_`.  Common cases are:

- `APOLLO_KEY`
- `APOLLO_GRAPH_REF`
- TODO More?

In most cases, the Apollo Router supports the same environment variables, though the following are not yet supported:

- `TODO` (probably customizing the uplink endpoints)
