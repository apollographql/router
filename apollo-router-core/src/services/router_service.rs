    ...
    fn call(&mut self, request: RouterRequest) -> Self::Future {
            ...
            .get(&request.http_request.body().query)
        {
            return Box::pin(async move {
                Ok(RouterResponse {
                    response: http::Response::new(ResponseBody::GraphQL(response)),
        ...
        let fut = async move {
            ...
            if let Some(err) = query.as_ref().and_then(|q| {
                ...
                Ok(RouterResponse {
                    response: http::Response::new(ResponseBody::GraphQL(err)),
                    context: Context::new().with_request(Arc::new(request.http_request)),
                })
            } else {
                ...
                response.map(|execution_reqponse| RouterResponse {
                    response: execution_reqponse.response.map(ResponseBody::GraphQL),
                    context: execution_reqponse.context,
                })
            }
        };
