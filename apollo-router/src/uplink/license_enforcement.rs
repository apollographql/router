...
impl License {
    pub(crate) fn jwks() -> &'static JwkSet {
        JWKS.get_or_init(|| {
            ...
            #[cfg(not(any(test, feature = "test-jwks")))]
            let jwks = re.replace(include_str!("license.jwks.json"), "");
            #[cfg(any(test, feature = "test-jwks"))]
...
mod test {
    ...
    fn test_license_parse() {
        let license = License::from_str("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..Ca51677VReD0cAjKre1i7Pfpqb42FZxupWI3xGsFjCA").expect("must be able to decode JWT");
        ...
        assert_eq!(
            ...
            Some(Claims {
                ...
                tps: Default::default(),
                allowed_features: Some(vec![AllowedFeature::Coprocessors])
            }),
        );
    }
