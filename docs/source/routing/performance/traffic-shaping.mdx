...
#### How compression headers work

When you configure compression for subgraph traffic, the router automatically manages HTTP compression headers:

- **`content-encoding` header**: When compression is enabled via `traffic_shaping.all.compression` or `traffic_shaping.subgraphs.<name>.compression`, the router sets this header on outgoing requests to subgraphs. This header indicates that the router has compressed the request body using the specified algorithm (e.g., `content-encoding: gzip`).

- **`accept-encoding` header**: The router automatically sets this header on all subgraph requests to indicate it can accept compressed responses. The router always accepts responses compressed with `gzip`, `br`, or `deflate`, regardless of whether you've configured outgoing compression.

In other words, the router always tells subgraphs "I can handle compressed responses" (via `accept-encoding`), but only compresses its own request bodies when you explicitly configure the `compression` option (indicated via `content-encoding`).

<Note>

These headers are added to requests after the requests are added to the debug stack, so the headers won't appear in the Connectors Debugger. However, they are present in the actual HTTP requests sent to subgraphs.
