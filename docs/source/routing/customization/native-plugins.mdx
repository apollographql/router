...
use apollo_router::plugin::Plugin;
use apollo_router::services::http_layer;
use apollo_router::services::*;
...
impl Plugin for HelloWorld {
    ...
    // http_service runs at the raw HTTP boundary (before conversion to router::Request).
    // Request/response use http_layer types: method, URI, headers, body as Bytes.
    // Use for custom auth, body/header inspection or mutation before GraphQL parsing.
    fn http_service(
        &self,
...
The `http_service` hook receives [`http_layer::HttpRequest`](https://docs.rs/apollo-router/latest/apollo_router/services/http_layer/type.HttpRequest.html) and [`http_layer::HttpResponse`](https://docs.rs/apollo-router/latest/apollo_router/services/http_layer/type.HttpResponse.html) (body is `Bytes`), so layers like `map_request` and `map_response` work on raw HTTP. The inner service eventually converts to `router::Request` and back to an HTTP response.
