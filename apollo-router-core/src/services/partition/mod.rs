...
impl PartitionService<(), (), (), ()> {
    ...
    /// );
    /// ```
    pub fn with_cache_size<K, Req, P, F, S>(partition_fn: P, make_service: F, cache_size: usize) -> PartitionService<K, F, Req, S>
    where
        P: Fn(&Req) -> K + Send + Sync + 'static,
        F: Fn(K) -> S + Clone + Send + Sync + 'static,
        S: Service<Req> + Send + Clone + 'static,
    ...
    Req: Send + 'static,
{
    ...
    fn call(&mut self, req: Req) -> Self::Future {
        ...
        let service = self.cache.get_or_insert_with(&partition, || {
