...
The **HTTP layer** (`http_service`) runs first and receives the raw HTTP request/response (method, URI, headers, body as string). Use it when you need to customize at the HTTP boundary before the request is converted to a GraphQL/router request. See [HTTP layer request and response](#http-layer-request-and-response) for the object shapes.
...
When you define `fn http_service(service)` and register callbacks with `service.map_request` and `service.map_response`, those callbacks receive raw HTTP objects:

**Request** (in `map_request` callbacks):

- `request.method` — HTTP method (read/write)
- `request.uri` — full URI (read/write)
- `request.headers` — HTTP headers (read/write)
- `request.body` — request body as a **string** (read/write), e.g. JSON

**Response** (in `map_response` callbacks):

- `response.status_code` — HTTP status code (read/write)
- `response.headers` — HTTP headers (read/write)
- `response.body` — response body as a **string** (read/write), e.g. JSON

At this stage the body is not yet parsed as GraphQL; you can read or replace it as a string (for example, to validate or rewrite JSON before it reaches the router pipeline). Throwing from an HTTP layer callback produces an HTTP error response (status and optional JSON body), same as at other stages.
...
Your Rhai script can terminate the associated client request that triggered it. To do so, throw an exception from a callback in **http_service**, **router_service**, or **supergraph_service**. This returns an error response to the client (by default an `Internal Server Error` with a `500` status code).

When choosing where to terminate:

- **HTTP layer** (`http_service`): Runs first; request and response bodies are raw strings (e.g. JSON). Use to reject or short-circuit before any GraphQL parsing, or when you only need to inspect or mutate raw HTTP.
...
- **Supergraph service**: Use whenever you need to examine GraphQL request bodies to enforce termination.
...
The following describes the `request` object for **router_service**, **supergraph_service**, **execution_service**, and **subgraph_service**. For the HTTP layer (`http_service`), the request shape is different (e.g. `body` is a string, no `context`); see [HTTP layer request and response](#http-layer-request-and-response).

All callback functions registered via `map_request` (in those services) are passed a `request` object that represents the request sent by the client. This object provides the following fields:
...
The following describes the `response` object for **router_service**, **supergraph_service**, **execution_service**, and **subgraph_service**. For the HTTP layer (`http_service`), the response shape is different (e.g. `body` is a string, no GraphQL `body.data`/`body.errors`); see [HTTP layer request and response](#http-layer-request-and-response).

All callback functions registered via `map_response` (in those services) are passed a `response` object that represents an HTTP response.
