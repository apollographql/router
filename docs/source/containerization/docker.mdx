...
Note: The [docker documentation](https://docs.docker.com/engine/reference/run/) for the run command will be generally helpful.
...
docker run -p 4000:4000 \
  ...
  --rm \
  ghcr.io/apollographql/router:v0.1.0-preview.1
```
...
docker run -p 4000:4000 \
  ...
  --rm \
  ghcr.io/apollographql/router:v0.1.0-preview.1 -c /tmp/router.yaml
```
...
docker run -p 4000:4000 \
  ...
  --tty \
  --entrypoint=sh
  ghcr.io/apollographql/router:v0.1.0-preview.1-debug
/dist # pwd
/dist
/dist # ls
...
docker run -p 4000:4000 \
  ...
  --rm \
  ghcr.io/apollographql/router:v0.1.0-preview.1 -c config/router.yaml -s schema/local.graphql
```
