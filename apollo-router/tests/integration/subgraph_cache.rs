...
async fn subgraph_cache_basic() -> Result<(), BoxError> {
    if !graph_os_enabled() {
        return Ok(());
    }
...
#[tokio::test(flavor = "multi_thread")]
async fn subgraph_cache_with_nested_field_set() -> Result<(), BoxError> {
    if !graph_os_enabled() {
        return Ok(());
    }
...
#[tokio::test(flavor = "multi_thread")]
async fn subgraph_cache_authorization() -> Result<(), BoxError> {
    if !graph_os_enabled() {
        return Ok(());
    }
