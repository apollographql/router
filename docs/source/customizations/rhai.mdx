...
All request/response callback functions are provided with a typed Request/Response which may be interacted with to manipulate headers and context. In addition, request details (query, operation name, variables and extensions) are available to be read for all requests as sub-fields of the body. For example to read a request query, simply access "request.body.query". The examples illustrate how to interact with request/response objects.
