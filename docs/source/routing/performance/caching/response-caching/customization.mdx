...
<Note>
  ...
  - **Connector root fields** are cached at the connector HTTP request level, so the coprocessor's
    `connector_request`/`connector_response` stages still run on a cache hit. Because no HTTP call happened, the
    `connector_response` payload carries `cacheHit: true` and no `statusCode` or `headers`, and a status or headers
    returned by the coprocessor for a cache hit are ignored.
