...
Assuming each review has exactly one author, the total cost of the query is 26.
...
```text disableCopy=true showLineNumbers=false
 ...
 + 6 product objects (6)
 + 6 name scalars (0)
 + 10 review objects (10)
 + 10 author objects (10)
 + 10 name scalars (0)
 = total cost (26)
...
For an operation with list fields, the router must run the operation to get the actual number of items in its lists. Without actual list sizes, the router can only estimate the operation cost before it's executed by assuming list sizes. 

After an operation is executed, the router can calculate the actual cost per operation with the actual list sizes.

<Note>

The difference between estimated and actual operation costs is due only to the difference between assumed and actual sizes of list fields.

</Note>

### Measurement and enforcement modes

When implementing demand control, you must first gather information about the operations being executed against your graph. With this information, you can accurately decide when to reject requests.

The router's demand control supports a _measurement_ mode that enables you to gather this information without impacting your running services. You can define telemetry instruments to monitor your operations and decide on their maximum cost threshold.  

After gathering enough data, you can then configure your router with maximum cost and list size limits and set demand control to _enforcement_ mode, where it rejects operations with costs exceeding the limit.
...
- Generate log messages whenever the delta between estimated and actual cost is greater than an arbitrary value.
