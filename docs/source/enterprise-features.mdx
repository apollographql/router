...
1. Fetch an offline license by running [rover fetch license](/rover/commands/license/#fetch-license) with the ID of the graph from which you want to fetch a license:
...
1. Provide the offline license to your router on startup. The router accepts an offline license in a few ways:
    ...
    The router checks the CLI option and environment variables in the listed order, and it uses the value of the first option or variable that is set.
...
1. (**Recommended**) Configure the router to report usage metrics to GraphOS in a best-effort basis by setting both the `APOLLO_KEY` and `APOLLO_GRAPH_REF` environment variables. 
