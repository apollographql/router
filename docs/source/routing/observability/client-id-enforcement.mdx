...
Understanding client usage is critical for maintaining an evolving GraphQL schema. 
...
The router supports client awareness by default. If the client sets the headers `apollographql-client-name` and `apollographql-client-version` in its HTTP requests, GraphOS Studio can separate the metrics and operations per client. This client name is also used by the persisted queries feature.

Clients can (and should) [name their GraphQL operations](/react/data/operation-best-practices/#name-all-operations), which provides more context around how and where data is being used.
...
Apollo strongly recommends requiring client details (name and version) and operation names in all incoming GraphQL requests.
