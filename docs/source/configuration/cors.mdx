---
title: Configuring CORS in the GraphOS Router
subtitle: Control browser access to your router
description: Manage browser access to your GraphOS Router with CORS configuration options, including origin whitelisting, wildcard origins, and credential passing.
---

<Note> 

**This article describes CORS configuration that's specific to the GraphOS Router**. For a more general introduction to CORS and common considerations, see the following sections:
...
By default, the GraphOS Router enables _only_ GraphOS Studio to initiate browser connections to it. If your supergraph serves data to other browser-based applications, you need to do one of the following in the `cors` section of your router's [YAML config file](./overview/#yaml-config-file):
...
To allow browsers to pass credentials to the GraphOS Router, set `allow_credentials` to `true`, like so:
...
The following snippet shows all CORS configuration defaults for the GraphOS Router:
