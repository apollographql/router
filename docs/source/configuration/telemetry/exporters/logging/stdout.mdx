...
The `telemetry.logging.stdout.format.json.display_resource` option configures whether resources configured in `router.yaml` are attached to and displayed in log messages. By default, `display_resource` is `true`.
...
telemetry:
  exporters:
    logging:
      stdout:
        format: json
          display_resource: true #highlight-line
      common:
