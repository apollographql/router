...
async fn mock_subgraph_service_withf_panics_should_be_reported_as_service_closed() {
    ...
    let reason: String =
        serde_json_bytes::from_value(result.0.errors[0].extensions.get("reason").unwrap().clone())
            .unwrap();
...
async fn defer_if_condition() {
    ...
    // shouldDefer: true
    insta::assert_json_snapshot!(defer_primary_response.0);
    ...
    let defer_disabled = query_plan
        ...
        .await;
    insta::assert_json_snapshot!(defer_disabled.0);
