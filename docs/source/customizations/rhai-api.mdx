...
Your Rhai customization can access environment variables using the `router_env` module. Use the `router_env::get()` function.

```rhai
fn router_service(service) {
    try {
        print(`HOME: ${router_env::get("HOME")}`);
        print(`LANG: ${router_env::get("LANG")}`);
    } catch(err) {
        print(`exception: ${err}`);
    }
}
...
Note: You don't need to import the "router_env" module. It is imported in the router.
