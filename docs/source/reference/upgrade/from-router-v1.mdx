...
```bash
router config upgrade --diff --config-path router.yaml
```

Then apply the changes using:
```bah
router config upgrade --config-path router.yaml > router.next.yaml
...
<!-- TODO(@goto-bus-stop): start using the ?live and ?ready things -->
