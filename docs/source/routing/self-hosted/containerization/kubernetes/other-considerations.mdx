...
Apollo Router does not currently provide a configurable drain timeout or parameter similar to <code>stopGracePeriodMillis</code> found in Apollo Server. Upon receiving a <code>SIGTERM</code> in Kubernetes, the Router immediately stops accepting new requests and waits for all active/in-flight requests to complete before exiting. There is no internal time limit controlling this process: the shutdown process continues until all ongoing requests either finish or time out at the request level.

Set the termination duration for your service mesh to be slightly longer than your longest subgraph timeout configuration.
