...
Query validation errors can potentially reveal information about your schema structure, such as field names, types, and required arguments. While this is not typically a significant security concern (especially with persisted queries), you may want to redact these error details in production environments.

By default, the router returns detailed validation error messages. To redact validation errors and return a generic error message instead, set the `supergraph.redact_query_validation_errors` flag:
...
When enabled, any query validation errors are replaced with a single generic error:
