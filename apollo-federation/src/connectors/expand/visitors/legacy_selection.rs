// Legacy visitor-based selection code for ConnectSpec v0.2 compatibility
// This code is frozen and will be removed when v0.2 support is dropped
            ...
            if !extended_field_type.is_built_in() {
                match field_type {
                    ...
                    TypeDefinitionPosition::Enum(enum_type) => {
                        let def = enum_type.get(self.original_schema.schema())?;
                        let def = EnumType {
                            description: def.description.clone(),
                            name: def.name.clone(),
                            directives: filter_directives(
                        ...
                        try_pre_insert!(self.to_schema, enum_type)?;
                        try_insert!(self.to_schema, enum_type, Node::new(def))
                    }
                    _ => Ok(()),
                }?;
            }
    ...
    for SchemaVisitor<'_, ObjectTypeDefinitionPosition, ObjectType>
{
    fn try_get_group_for_field(
        ...
        match field.names().first() {
            Some(field_name) => {
                ...
                // Get the subselection for this field
    ...
    fn enter_group(
        &mut self,
        group: &JSONSelectionGroup,
    ) -> Result<Vec<NamedSelection>, Self::Error> {
        let (group_type, sub) = group;
        let sub_type = group_type.get(self.original_schema.schema())?;
        try_pre_insert!(self.to_schema, group_type)?;

        let sub_type = ObjectType {
            description: sub_type.description.clone(),
            name: sub_type.name.clone(),
            implements_interfaces: sub_type.implements_interfaces.clone(),
            directives: filter_directives(self.directive_deny_list, &sub_type.directives),
