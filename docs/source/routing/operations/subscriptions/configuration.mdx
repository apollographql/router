...
### Limiting subscription lifetime

By default, subscriptions can remain open indefinitely. You can set a maximum duration using `max_lifetime`, after which the router closes the subscription and sends a terminal error to the client:
...
This is useful for enforcing resource limits, rotating authentication tokens, or preventing stale long-lived connections.
...
| `max_lifetime` | The subscription was terminated because the configured [`max_lifetime`](#limiting-subscription-lifetime) duration was reached. Clients can reconnect to resume the subscription. |
