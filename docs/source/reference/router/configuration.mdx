...
When the Router schema or configuration updates all connections must be closed for the existing resources to be freed.
To ensure that long-lived connections do hang on to these resources a maximum graceful shutdown timeout can be configured.
