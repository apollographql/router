...
Conditions and the `static` selector are supported for the `router`, `supergraph`, `subgraph`, and `connector` services. They are **not** supported for `http_client` span attributes. See [`http_client` span attributes](#http_client-span-attributes) for details.
...
`http_client` span attributes only support the `request_header` and `response_header` selectors. Conditions and the `static` selector are **not** supported. Attempting to use them causes a router startup failure with an error similar to:
