...
A `views[]` entry without an explicit `aggregation` preserves the instrument's native kind. Before router v2.16.0, such entries converted counters and gauges into histograms; if your dashboards depend on those `_bucket`/`_sum`/`_count` series, add `aggregation: histogram` with your buckets.
...
OpenTelemetry metrics support up to 2000 distinct attribute combinations by default. When this limit is reached, additional attribute combinations are grouped into a single overflow series tagged `otel_metric_overflow="true"` to maintain system stability.

Use `cardinality_limit` to adjust this cap globally, or override it for a specific metric:
...
| `cardinality_limit` | `2000`                   | Maximum number of distinct attribute combinations per metric. Per-metric overrides available via `views[].cardinality_limit`. For details, see [Metric cardinality limit](#cardinality_limit). |
