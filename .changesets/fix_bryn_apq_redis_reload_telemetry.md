...
The automatic-persisted-query cache connected to Redis after a hot reload had already swapped the global telemetry providers. A failed connection (with `required_to_start: true`) therefore left the router serving with broken metrics. The connection attempt now happens before the swap; when it fails, the reload fails cleanly and the router keeps serving the previous configuration with working telemetry.
