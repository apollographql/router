...
A [_selector_](/router/configuration/telemetry/instrumentation/selectors) is used to extract data from the GraphOS Router's request lifecycle (pipeline) services and attach them to telemetry like spans, metrics and logs. For response cache we have different selectors available on the subgraph service to create either custom metrics or also add custom attributes to your telemetry data.
...
+ If you want a metric to know what's the values of computed `max-age` at the subgraph level to know the TTLs set in your cache you can create your own custom instrument using `response_cache_control` selector. (cf `subgraph.response.cache_control.max_age` instrument).

+ If you want to know for a subgraph request duration using the standard otel instrument `http.client.request.duration` if it's a request coming from cache or not. You can use the selector `response_cache_status` and set it to `status` to know if it's a partial hit, a cache hit or a cache miss. (cf `response.cache.status` attribute on `http.client.request.duration`).
    ...
    instruments:
      subgraph:
        ...
        subgraph.response.cache_control.max_age:
          ...
          type: histogram
          unit: s # Seconds
          description: Histogram to know what's the TTL computed for a subgraph response
  #highlight-end
```
