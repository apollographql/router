...
description: Enable and configure the Datadog exporter for metrics via OpenTelemetry in the Apollo Router
---

Enable and configure the [OTLP exporter](./opentelemetry) for metrics in the Apollo Router for use with [Datadog](https://www.datadoghq.com/).
...
To configure the router, enable the [OTLP exporter](./opentelemetry#configuration) and set both `temporality: delta` and `endpoint: <datadog-agent-endpoint>`. For example: 
