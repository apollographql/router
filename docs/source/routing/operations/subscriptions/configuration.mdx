...
The stability threshold is `5 × reconnect_delay` with a minimum of 500 ms. For example, with the default `reconnect_delay: 1s`, a connection must stay open at least 5 seconds to be considered stable and reset the budget. With `reconnect_delay: 10s`, the threshold rises to 50 seconds. A subgraph that accepts the handshake but drops the connection more quickly than this threshold is treated as flapping and continues to consume the current budget.
