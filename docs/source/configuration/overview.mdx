...
You can pass the `--dev` flag when starting up the Apollo Router to enable plugins that make developing easier. For now, this flag enables [subgraph error inclusion](/subgraph-error-inclusion), but in the future it may enable other plugins or modify other behavior. You should never run a production router with this flag enabled.
