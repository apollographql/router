...
* `map_request` callbacks are called in each service as execution proceeds "to the right" from the router receiving a client request:
    ...
    For reference, see the fields of [`request`](/router/customizations/rhai-api/#request-interface).
...
If your router customization needs to do any of these, you can instead use [external co-processing](/router/customizations/coprocessor/) (this is an [Enterprise feature](../enterprise-features)).
...
For tracking down runtime errors, insert [logging](/router/customizations/rhai-api#logging) statements to narrow down the issue.
