...
async fn test_layer_converts_apollo_router_errors_properly() {
    ...
    assert!(
        error["message"]
            .as_str()
            .unwrap()
            .contains("Test service connection failed")
    );
    assert!(
        error["message"]
            .as_str()
...
async fn test_layer_converts_nested_apollo_router_errors() {
    ...
    let response =
        result.expect("Layer should convert nested Apollo Router error to GraphQL response");
    ...
    assert!(
        error["message"]
            .as_str()
            .unwrap()
            .contains("Invalid nested configuration")
    );
