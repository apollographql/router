...
- `apollo.router.oci.manifest.count` - Number of requests made to get the manifest for a large Graph Artifact blob, attributes:
  - `url`: The URL that was polled
  - `status`: (`success`, `failure`)
- `apollo.router.oci.manifest.duration.seconds` - Duration of request to get OCI manifest of larger blob, attributes:
    - `url`: The URL that was polled
    - `kind`: (`got_manifest`, `oci_error`, `new`, `unchanged`)
- `apollo.router.oci.blob.count` - Number of fetches made to get entire blob of a Graph Artifact, attributes:
  - `status`: (`success`, `failure`)
  - `url`: The URL that was polled
- `apollo.router.oci.manifest.blob.seconds` - Duration of requests to get entire Graph Artifact blob, attributes:
  - `url`: The URL that was polled
  - `kind`: (`oci_error`, `new`)
