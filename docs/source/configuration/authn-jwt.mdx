...
- To be compatible with Apollo Router supported JWT authentication, your IdP (or whatever service issues JWTs to authenticated clients) must use one of the [signature algorithms](https://crates.io/crates/jsonwebtoken#algorithms) supported by the router. Most popular signing algorithms are supported.
