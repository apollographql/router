...
The router configuration necessary to call the coprocessor for authorization claims looks like this:

```yaml title="router.yaml"
coprocessor:
  url: http://127.0.0.1:8081 # Required. Replace with the URL of your coprocessor's HTTP endpoint.
  router: # By including this key, a coprocessor can hook into the `RouterService`. You can also use `supergraph` for authorization.
