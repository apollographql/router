...
Limits how many errors a connector's response mapping can add to the GraphQL `errors` array using the [`->withError` method](/graphos/connectors/responses/error-handling#report-errors-without-failing-a-field).
...
There is no default limit; every declared error is reported unless you configure this option, which aligns with how Apollo Router passes through subgraph errors.

Identify sources by `subgraph_name.source_name`, the same key format used by `http_max_response_size`. Set a global default and optionally override it per source:
...
This limit applies only to errors the mapping defines with `->withError`. Mapping *problems* — the mapping language's own diagnostics, such as a path that matched nothing — are never sent to clients and are unaffected.

### Monitoring exceeded limits

When a connector response's mapping errors are truncated, Apollo Router increments the `apollo.router.limits.connector_mapping_errors.exceeded` counter with a `connector.source` attribute identifying the affected source.
