...
Amazon API Gateway now supports response streaming for REST APIs, so you can use HTTP multipart subscriptions when the router is fronted by AWS API Gateway. Previously, AWS API Gateway did not support streaming of HTTP data; support was added in November 2025. For details, see the [Amazon API Gateway response streaming for REST APIs announcement](https://aws.amazon.com/about-aws/whats-new/2025/11/api-gateway-response-streaming-rest-apis/).

Enable response streaming for your REST API (for example, when using HTTP proxy integration to the router) so that multipart responses are streamed instead of buffered. For configuration, see [Response transfer mode](https://docs.aws.amazon.com/apigateway/latest/developerguide/response-transfer-mode.html) in the Amazon API Gateway documentation.
