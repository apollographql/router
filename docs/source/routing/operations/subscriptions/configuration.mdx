...
- `apollo.router.operations.subscriptions.ended.subgraph` (attribute: `subgraph.name`): Incremented once per subgraph WebSocket closure. Each deduplicated client sharing that connection will also emit a corresponding `apollo.router.operations.subscriptions.terminated` event with `reason=server_close`.

The `apollo.router.operations.subscriptions.terminated` metric includes `reason` and `subgraph.name` attributes by default. You can optionally enable `client.name` through the telemetry instruments configuration. See [standard instruments](/router/configuration/telemetry/instrumentation/standard-instruments#subscriptions) for configuration details.

#### Understanding metric multiplicity

The `apollo.router.operations.subscriptions.ended.subgraph` and `apollo.router.operations.subscriptions.terminated` metrics operate at different granularities. `ended.subgraph` increments once per subgraph WebSocket closure, while `terminated` increments once per client that was listening on that connection. For example, if 5 clients are deduplicated onto a single subgraph WebSocket and the subgraph closes the connection, you'll see 1 `ended.subgraph` event and 5 `terminated` events with `reason=server_close`. This means the `terminated` count can be significantly higher than `ended.subgraph` — the ratio reflects your deduplication factor.
...
- The `apollo.router.operations.subscriptions.ended.subgraph` metric helps track how often subgraphs are closing connections, which can inform capacity planning and subgraph stability monitoring.
