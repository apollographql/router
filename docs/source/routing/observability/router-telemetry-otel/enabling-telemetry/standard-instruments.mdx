...
- `apollo.router.operations.subscriptions.rejected.limit` - Number of subscription requests rejected because the router has reached its [`max_opened_subscriptions`](/router/executing-operations/subscription-support/#limiting-the-number-of-client-connections) limit.
