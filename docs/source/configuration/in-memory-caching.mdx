---
title: In-Memory Caching in the GraphOS Router
subtitle: Configure caching for query plans and automatic persisted queries
description: Configure the GraphOS Router's in-memory caching for improved performance. Configure query plans and automatic persisted queries caching.
---

The GraphOS Router uses an in-memory LRU cache to store the following data:
...
The GraphOS Router supports using APQ in its communications with both clients _and_ subgraphs:
...
The GraphOS Router enables APQ caching for client operations by default. In your router's [YAML config file](./overview/#yaml-config-file), you can configure the maximum number of APQ entries in the cache like so:
...
By default, the GraphOS Router does _not_ use APQ when sending queries to its subgraphs.
