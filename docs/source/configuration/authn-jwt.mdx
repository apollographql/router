...
The Apollo Router provides experimental support for request authentication via [JSON Web Token](https://www.rfc-editor.org/rfc/rfc7519) (**JWT**). This feature is implemented as a built-in [native Rust plugin](../customizations/native/).
...
5. The authentication plugin extracts all **claims** from the validated JWT and adds them to the request's context, making them available to your [router customizations](../customizations/overview/), such as Rhai scripts.
    - **Claims** are the individual details of a JWT's scope. These details might include the ID of the associated user, any roles assigned to that user, and the JWT's expiration time. [See the spec.](https://www.rfc-editor.org/rfc/rfc7519#section-4)
...
- `url`: **required** URL from which the JWKS file will be read. It can be an `https://` or a `file://` URL.
...
After the Apollo Router's authentication plugin validates a JWT, it adds that token's **claims** to the request's context at this key: `apollo_authentication::JWT::claims`

The authentication plugin performs validation in the `RouterService`, which is the _first_ service that executes in the Apollo Router's [request pipeline](../customizations/overview/#how-customizations-work). This means you can build [router customizations](../customizations/overview/) (usually [Rhai scripts](../customizations/rhai/)) that interact with claims by hooking into any _other_ service.
...
Below is an example [Rhai script](../customizations/rhai/) that checks the details of different JWT claims and throws different errors for different invalid claim details.
...
fn supergraph_service(service) {
    // Rhai mechanism for creating a function pointer
    const request_callback = Fn("process_request");
...
Below is an example [Rhai script](../customizations/rhai/) that forwards a JWT's claims to individual subgraphs via HTTP headers (one header for each claim). This enables each subgraph to define logic to handle (or potentially reject) incoming requests based on claim details.

> This script hooks into the router's `SubgraphService`, which executes _once for each subgraph_ that the router communicates with to resolve the operation. [Learn more about router services.](../customizations/overview#service-descriptions)
...
fn subgraph_service(service, subgraph) {
  ...
  service.map_request(f); // Register the callback
}
```
...
If you need to forward entire JWTs to subgraphs so that they can _also_ validate them (instead of only [forwarding claims](#example-forwarding-claims-to-subgraphs)), you can do so via the Apollo Router's general-purpose [HTTP header propagation settings](./header-propagation).
