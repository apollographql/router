...
| `response_cache_status`    | No          | `hit` \| `partial_hit`\| `miss`\| `status`    | Select if you want to know if it's a cache hit (all data coming from cache), miss (all data coming from subgraph) or partial_hit (not all entities are coming from cache for example)  |
| `response_cache_control`    | No          | `max_age` \| `public`\| `private`\| `public`\| `no_store`    | Select data you want from the computed cache control from response caching  |
    ...
    instruments:
      router:
        apollo.router.overhead:
          attributes:
            # Add the graphql operation name as an attribute
            operation.name:
