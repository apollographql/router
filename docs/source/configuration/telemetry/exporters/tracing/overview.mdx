...
If you are sending traces to the Datadog agent using the OTLP exporter `datadog_agent_sampling: true` in `router.yaml` to ensure that APM metrics display correctly.
This is automatically set if you are using the Datadog native exporter.
...
```
This setting will cause the router to send ALL spans to the Datadog agent. The agent will then sample the spans before sending them to datadog.
> [!IMPORTANT]
