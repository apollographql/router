...
Because `.sock` isn't required as a file extension, the Apollo Router can't tell if a path has been appended to the base URL. You can specify a custom HTTP path using the `path` query parameter:
