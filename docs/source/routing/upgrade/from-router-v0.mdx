...
The router supports three types of customizations that hook into the request-handling pipeline:

* [WebAssembly plugins](/graphos/routing/customization/wasm/)
    * WebAssembly components run portable request logic in a capability-restricted sandbox inside the router.
...
* [External coprocessing](/router/customizations/coprocessor/). 
    * Requires a [GraphOS Router with an Enterprise plan](/router/enterprise-features).
