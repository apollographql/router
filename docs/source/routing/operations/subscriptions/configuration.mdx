...
Each rejected subscription increments the `apollo.router.operations.subscriptions.rejected.limit` counter metric, which you can use to monitor how often clients hit the limit.
...
- `apollo.router.operations.subscriptions.rejected.subgraph` (attribute: `subgraph.service.name`): Incremented when a subscription request is rejected because the subgraph WebSocket connection failed, such as a connection refusal, protocol error, or failed subscription handshake.
...
- A high rate of `apollo.router.operations.subscriptions.rejected.subgraph` for a specific subgraph may indicate connectivity issues or misconfigured WebSocket endpoints.
