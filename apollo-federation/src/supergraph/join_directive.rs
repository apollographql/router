...
pub(super) fn extract(
    ...
    for object_pos in &join_directives.object_types {
        let obj = object_pos.get(supergraph_schema.schema())?;
        let directives = obj
            .directives
            .iter()
            .filter_map(|d| {
        ...
        for (directive, subgraph_enum_values) in directives {
            for subgraph_enum_value in subgraph_enum_values {
                ...
                if subgraph
                    .schema
                    .try_get_type(object_pos.type_name.clone())
                    .map(|t| matches!(t, TypeDefinitionPosition::Object(_)))
                    .unwrap_or_default()
                {
                    object_pos.insert_directive(
                        &mut subgraph.schema,
