...
| Value | Description |
|-------|-------------|
| `server_close` | The server (subgraph) closed the connection gracefully. This occurs when the subgraph ends the subscription. |
...
- Frequent `stream_end` values may indicate issues with your subgraph subscription sources closing unexpectedly.
