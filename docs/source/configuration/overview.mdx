...
By default, the router will compress subgraph requests by generating fragment
...
while forming subgraph requests. This behavior can be enabled by setting the
option to `true`:
