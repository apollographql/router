...
import Reload from '../../../shared/config/reload.mdx';
...
Learn more about [configuring reload retries](/graphos/routing/configuration/hot-reload-schema#configuring-reload-retries).
...
In Router 2.x and above, enhanced operation signature normalization is enabled by default. To configure the normalization algorithm, update `telemetry.apollo.signature_normalization_algorithm` in `router.yaml`:

```yaml title="router.yaml"
telemetry:
  apollo:
    signature_normalization_algorithm: enhanced # Default is `enhanced`; disable by setting to `legacy`
```
...
Configure extended reference reporting in `router.yaml` with the `telemetry.apollo.errors.extended_error_metrics` option. It is enabled by default; to opt out, set it to `disabled`:

```yaml title="router.yaml"
telemetry:
  apollo:
    errors:
      extended_error_metrics: enabled # Default is enabled
```

[Learn more about GraphOS extended error reporting](/graphos/routing/observability/graphos/graphos-reporting#enabling-extended-error-reporting).
...
Apollo's standard reference reporting doesn't include data about enum values and input object fields, meaning you can't view enum and input object field usage in GraphOS Studio.
Standard reporting can also cause [inaccurate operation checks](#enhanced-operation-checks).

Configure reference reporting by updating `telemetry.apollo.metrics_reference_mode` in `router.yaml`:

```yaml title="router.yaml"
telemetry:
  apollo:
    metrics_reference_mode: extended # Default is `extended`; disable by setting to `standard`
```
