...
All request/response callback functions are provided with a typed Request/Response which can be interacted with to manipulate headers and context. In addition, operation_name details are available for all requests.
...
There are three complete working examples (with tests) of rhai in the [examples directory](https://github.com/apollographql/router/tree/main/examples). The rhai examples are:
