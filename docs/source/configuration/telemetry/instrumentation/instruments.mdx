...
To learn about standard metric instruments for the router's request lifecycle, see [router instruments](./standard-instruments).
...
* `event_duration` - the duration of an event in the pipeline service.
* `event_unit` - the number of times an event in the pipeline service has been executed.
* `event_custom` - a custom value extracted from the event in the pipeline service. See [selectors](./selectors) for more information.
