    ...
    let (mapped_response, result) = match result {
        ...
        Err(error) => {
            let raw = RawResponse::Error {
                error: error
                    .to_raw_error(&connector, Some(response_key_to_path(&response_key))),
                key: response_key,
            };
    ...
    } = mapped_response
    {
        if let Some(Value::String(error_code)) = mapped_error.extensions.get("code") {
            emit_error_event(
                ...
                mapped_error.path.as_ref().map(|r| serde_json::from_str::<Path>(r.as_str()).unwrap_or_default())
            );
        }
    }
            ...
            .body(
                graphql::Response::builder()
                    ...
                    .errors(errors.into_iter().map(|error| {
                        graphql::Error::from_connectors_error(error, "TODO subgraph name")
                    }).collect())
                    .build(),
            )
            .unwrap(),
...
impl graphql::Error {
    fn from_connectors_error(
        ...
        Self::builder()
            ...
            .extension_code("X")
            .and_path(error.path.map(|r| serde_json::from_str::<Path>(r.as_str()).unwrap_or_default()))
            .build()
...
async fn deserialize_response<T: HttpBody>(
    ...
    let make_err = |path: Path| {
        RawError {
            ...
            code: Some("CONNECTOR_RESPONSE_INVALID".into()),
            extensions: Map::from_iter([
                ("service".into(), connector.id.subgraph_name.clone().into()),
                ("coordinate".into(), connector.id.coordinate().into()),
                ("http".into(),
            ...
            path: Some(serde_json::to_string(&path).expect("path is serializable").into())
        }
    };
