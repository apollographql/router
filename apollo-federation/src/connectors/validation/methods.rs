                ...
                .and_then(|value| value.as_str())
            {
                ...
                None => {
                    if let Some((field_name, _)) = fields.iter().find(|(_, field)| {
                        matches!(
                            schema.types.get(field.ty.inner_named_type()),
                            Some(
...
pub(super) fn validate_method_calls(schema: &SchemaInfo) -> Vec<Message> {
    ...
    for directive in applied_directives(schema) {
...
mod tests {
    ...
    /// Run the real validation path over a schema string.
    ///
    /// `validate` takes an expanded [`Subgraph`], so the parse/expand assembly
    /// happens here — mirroring `test_validate_source::validation_tests`, down
    /// to keeping the partial schema when the document is not valid GraphQL,
    /// since connector errors are reported for those too.
    fn validate_sdl(sdl: String) -> Vec<Message> {
        ...
        let subgraph = Subgraph::new("test", "http://test", parsed, orphan_extension_types)
...
"#
    ...
    fn codes(schema: String) -> Vec<Code> {
        validate_sdl(schema).into_iter().map(|m| m.code).collect()
    }
...
"#
        .to_string();
        let errors = validate_sdl(schema);
        assert!(
            errors.is_empty(),
