...
- `apollo.router.operations.subscriptions.ended.subgraph` (attribute: `subgraph.service.name`): Incremented when a subgraph closes its WebSocket connection, ending the subscription stream for all deduplicated clients listening on that connection.
