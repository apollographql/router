...
subtitle: Configure Apollo Router to trust your proxy's root certificate
...
If your organization uses a corporate proxy that performs TLS inspection (also known as SSL inspection or HTTPS interception), add the proxy's root certificate to your router container. Without this certificate, Apollo Router cannot establish secure connections to GraphOS or your subgraphs.

<ElasticNotice />

## Understanding proxy certificate requirements

Corporate proxies often intercept HTTPS traffic for security monitoring. They decrypt and re-encrypt traffic using their own certificate. For Apollo Router to trust these connections, install the proxy's root certificate authority (CA) certificate in its trust store.

Common symptoms of a missing proxy certificate:
...
Alternatively, build a custom Docker image with the certificate as described in [Build a custom image](#build-a-custom-image) and reference it in your Helm values.
...
- [Kubernetes deployment](/graphos/routing/self-hosted/containerization/kubernetes/quickstart): Deploy Apollo Router using Helm charts
