...
Cache tags derived from a subgraph response are written to Redis in a debugger‑readable form only when the cache‑populating request runs in debug mode. For performance reasons, storage differs between debug and non‑debug modes, so cache tags (coming from subgraph responses) written outside debug mode aren’t visible in the debugger.
