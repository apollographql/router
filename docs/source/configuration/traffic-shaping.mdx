...
traffic_shaping:
  ...
  all:
    deduplicate_query:
      enabled: true # Enable query deduplication for all subgraphs.
      ignored_headers:
        - "x-my-header" # Ignore certain headers when deduplicating queries
    compression: br # Enable brotli compression for all subgraphs.
  subgraphs: # Rules applied to requests from the router to individual subgraphs
    products:
      deduplicate_query:
        enabled: false # Disable query deduplication for the products subgraph.
...
You can use the `deduplicate_query_ignored_headers` option to provide a list of headers to remove from this deduplication calculation.
