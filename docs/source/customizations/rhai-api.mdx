...
**For `subgraph_service` callbacks only,** the `request` object provides the non modifiable field `request.subgraph.id` which is a unique ID corresponding to the subgraph request.
...
**For `subgraph_service` callbacks only,** the `response` object provides the non modifiable field `response.subgraph.id` which is a unique ID corresponding to the subgraph request, and the same id that can be obtained on the request side.
