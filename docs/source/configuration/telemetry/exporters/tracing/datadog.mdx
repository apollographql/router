...
OTLP is the [OpenTelemetry protocol](https://opentelemetry.io/docs/specs/otel/protocol/), and is the recommended protocol for transmitting telemetry, including traces, to Datadog.

To setup traces to Datadog via OTLP, you must do the following:

- Modify the default configuration of the Datadog Agent to accept OTLP traces submitted to it by the router.
...
Use the Datadog agent `probabalistic_sampling` option sampler and set the `sampler` to `always_on` to allow the Datadog agent to control the sampling rate.
