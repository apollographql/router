...
- `apollo.router.operations.subscriptions.rejected.limit` - Number of subscription requests rejected because Apollo Router has reached its [`max_opened_subscriptions`](/router/executing-operations/subscription-support/#limiting-the-number-of-client-connections) limit.
- `apollo.router.operations.subscriptions.rejected.subgraph` - Number of subscription requests rejected because the subgraph WebSocket connection failed (for example, connection refused, protocol error, or failed subscription handshake). Attributes:
  - `subgraph.service.name`: The name of the subgraph that rejected the connection.
- `apollo.router.operations.subscriptions.ended.subgraph` - Number of subscriptions ended by the subgraph closing the WebSocket connection, ending the subscription stream for all deduplicated clients listening on that connection. Attributes:
