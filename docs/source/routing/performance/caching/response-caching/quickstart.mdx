...
response_cache:
  ...
  connector:
    all:
      enabled: true
      ttl: 10m # Required: fallback TTL when responses don't include Cache-Control headers
      redis:
        urls: ["redis://localhost:6379"]
      invalidation:
        enabled: true
...
The `apollo.router.response.cache` metric tracks cache hits and misses for both subgraph and connector requests.
