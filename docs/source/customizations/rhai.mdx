...
The request query is accessible. If modified make sure to do this before query planning is performed (i.e.: `supergraph_service()` or `query_planner_service()`) or the modification will have no effect on the query. For example, let's modify the query at the supergraph_service stage and turn it into a completely invalid query.
